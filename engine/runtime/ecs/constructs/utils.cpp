...
namespace ecs {
  namespace utils {
    ...
    bool try_load_entity_from_file(const fs::path& full_path, runtime::entity& out_data) {
      ...
      if (!load_entities_from_file(full_path, out_data_vec)) return false;

      if (!out_data_vec.empty()) out_data = out_data_vec[0];

      return true;
    }

    void
    save_entities_to_file(const fs::path& full_path, const std::vector<runtime::entity>& data) {
      std::ofstream os(full_path.string(), std::fstream::binary | std::fstream::trunc);
      serialize_t<cereal::oarchive_associative_t>(os, data);
    }

    bool
    load_entities_from_file(const fs::path& full_path, std::vector<runtime::entity>& out_data) {
    ...
    runtime::entity clone_entity(const runtime::entity& data) {
      ...
      if (!vec_data.empty()) { return vec_data.front(); }
      return {};
    }
