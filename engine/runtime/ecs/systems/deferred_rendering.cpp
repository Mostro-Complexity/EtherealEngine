...
namespace runtime {
  ...
  bool update_lod_data(
    lod_data& data, const std::vector<urange32_t>& lod_limits, std::size_t total_lods,
    float transition_time, float dt, asset_handle<mesh> mesh, const math::transform& world,
    const camera& cam) {
    if (!mesh) return false;

    if (total_lods <= 1) return true;

    const auto& viewport = cam.get_viewport_size();
    irect32_t   rect = mesh->calculate_screen_rect(world, cam);

    float percent =
      math::clamp((float(rect.height()) / float(viewport.height)) * 100.0f, 0.0f, 100.0f);

    std::size_t lod = 0;
    for (size_t i = 0; i < lod_limits.size(); ++i) {
      const auto& range = lod_limits[i];
      if (range.contains(urange32_t::value_type(percent))) { lod = i; }
    }
    ...
    if (data.current_lod_index != data.target_lod_index) data.current_time += dt;
  ...
  bool should_rebuild_reflections(
    visibility_set_models_t& visibility_set, const reflection_probe& probe) {

    if (probe.method == reflect_method::environment) return false;
    ...
    for (auto& element : visibility_set) {
      ...
      if (!transform_comp_ptr || !model_comp_ptr) continue;
  ...
  visibility_set_models_t deferred_rendering::gather_visible_models(
    entity_component_system& ecs, camera* camera, bool dirty_only /* = false*/,
    bool static_only /*= true*/, bool require_reflection_caster /*= false*/) {
    ...
    for (auto entity : ecs.entities_with_components(transform_comp_handle, model_comp_handle)) {
      ...
      if (static_only && !model_comp_ptr->is_static()) { continue; }

      if (require_reflection_caster && !model_comp_ptr->casts_reflection()) { continue; }
  ...
  void deferred_rendering::build_reflections_pass(
    entity_component_system& ecs, std::chrono::duration<float> dt) {
  ...
  void deferred_rendering::build_shadows_pass(
    entity_component_system& ecs, std::chrono::duration<float> dt) {
    auto dirty_models = gather_visible_models(ecs, nullptr, true, true, true);
  ...
  void
  deferred_rendering::camera_pass(entity_component_system& ecs, std::chrono::duration<float> dt) {
  ...
  std::shared_ptr<gfx::frame_buffer> deferred_rendering::g_buffer_pass(
    std::shared_ptr<gfx::frame_buffer> input, camera& camera, gfx::render_view& render_view,
    visibility_set_models_t& visibility_set, std::unordered_map<entity, lod_data>& camera_lods,
    ...
    for (auto& element : visibility_set) {
      ...
      if (
        ...
        continue;
      const auto params =
        math::vec3{ 0.0f, -1.0f, (transition_time - current_time) / transition_time };
  ...
  std::shared_ptr<gfx::frame_buffer> deferred_rendering::lighting_pass(
    std::shared_ptr<gfx::frame_buffer> input, camera& camera, gfx::render_view& render_view,
    entity_component_system& ecs, std::chrono::duration<float> dt) {
    const auto& view = camera.get_view();
    const auto& proj = camera.get_projection();
    ...
    auto light_buffer = render_view.get_texture(
      "LBUFFER", viewport_size.width, viewport_size.height, false, 1, light_buffer_format);
    ...
    auto refl_buffer =
      render_view
        .get_texture(
          "RBUFFER", viewport_size.width, viewport_size.height, false, 1, light_buffer_format)
        .get();
    ...
    ecs.for_each<transform_component, light_component>(
      [this, &camera, &pass, &buffer_size, &view, &proj, g_buffer_fbo, refl_buffer](
        ...
        irect32_t rect(
          0,
          0,
          irect32_t::value_type(buffer_size.width),
          irect32_t::value_type(buffer_size.height));
        if (
          light_comp_ref.compute_projected_sphere_rect(
            rect, light_position, light_direction, view, proj)
          == 0)
          return;
        ...
        if (light.type == light_type::point && point_light_program_) {
          float light_data[4] = {
            light.point_data.range, light.point_data.exponent_falloff, 0.0f, 0.0f
          };
        ...
        if (program) {
          float light_color_intensity[4] = {
            light.color.value.r, light.color.value.g, light.color.value.b, light.intensity
          };
          auto camera_pos = camera.get_position();
          ...
          gfx::set_state(
            topology | BGFX_STATE_WRITE_RGB | BGFX_STATE_WRITE_A | BGFX_STATE_BLEND_ADD);
  ...
  std::shared_ptr<gfx::frame_buffer> deferred_rendering::reflection_probe_pass(
    std::shared_ptr<gfx::frame_buffer> input, camera& camera, gfx::render_view& render_view,
    entity_component_system& ecs, std::chrono::duration<float> dt) {
    const auto& view = camera.get_view();
    const auto& proj = camera.get_projection();
    ...
    auto refl_buffer = render_view.get_texture(
      "RBUFFER", viewport_size.width, viewport_size.height, false, 1, refl_buffer_format);
  ...
  std::shared_ptr<gfx::frame_buffer> deferred_rendering::atmospherics_pass(
    std::shared_ptr<gfx::frame_buffer> input, camera& camera, gfx::render_view& render_view,
    entity_component_system& ecs, std::chrono::duration<float> dt) {
    ...
    input =
      render_view.get_fbo("LBUFFER", { light_buffer, render_view.get_depth_buffer(viewport_size) });
    ...
    if ((surface != nullptr) && atmospherics_program_) {
      ...
      irect32_t rect(
        0, 0, irect32_t::value_type(output_size.width), irect32_t::value_type(output_size.height));
      gfx::set_scissor(rect.left, rect.top, rect.width(), rect.height());
      auto topology = gfx::clip_quad(1.0f);
      gfx::set_state(
        topology | BGFX_STATE_WRITE_RGB | BGFX_STATE_WRITE_A | BGFX_STATE_DEPTH_TEST_LEQUAL
  ...
  std::shared_ptr<gfx::frame_buffer> deferred_rendering::tonemapping_pass(
    std::shared_ptr<gfx::frame_buffer> input, camera& camera, gfx::render_view& render_view) {
  ...
  void deferred_rendering::receive(entity e) {
    lod_data_.erase(e);
    for (auto& pair : lod_data_) { pair.second.erase(e); }
  }
  deferred_rendering::deferred_rendering() {
    ...
    vs_clip_quad.wait();
    auto fs_deferred_point_light =
      am.load<gfx::shader>("engine:/data/shaders/fs_deferred_point_light.sc");
    fs_deferred_point_light.wait();
    auto fs_deferred_spot_light =
      am.load<gfx::shader>("engine:/data/shaders/fs_deferred_spot_light.sc");
    ...
    auto fs_box_reflection_probe =
      am.load<gfx::shader>("engine:/data/shaders/fs_box_reflection_probe.sc");
