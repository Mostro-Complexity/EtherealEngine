...
namespace runtime {
  ...
  void process_node(
    const std::unique_ptr<mesh::armature_node>& node, const skin_bind_data& bind_data,
    ...
    if (bone) { entity_nodes.push_back(entity_node); }
  ...
  bone_system::bone_system() { runtime::on_frame_update.connect(this, &bone_system::frame_update); }
