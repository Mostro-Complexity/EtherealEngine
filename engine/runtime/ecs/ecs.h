...
namespace runtime {
  ...
  template <typename C> using chandle = std::weak_ptr<C>;
  ...
  public:
    ...
    template <typename C, typename... Args> chandle<C> assign(Args&&... args);
    ...
    template <typename C> chandle<C> get_component() const;

    template <typename... Components> std::tuple<chandle<Components>...> components() const;
    ...
    template <typename A, typename... Args> void unpack(chandle<A>& a, chandle<Args>&... args);
  ...
  public:
    ...
    /// </summary>
    //-----------------------------------------------------------------------------
    component(const component& /*unused*/) { touch(); }
    ...
    /// </summary>
    //-----------------------------------------------------------------------------
    void touch() { last_touched_ = static_cast<std::uint32_t>(ecs::get_frame()); }
  ...
  template <typename T> class component_impl : public component {
  private:
    rtti::type_index_sequential_t::index_t runtime_id() const override { return static_id(); }
  ...
  public:
    ...
    chandle<T> handle() { return std::static_pointer_cast<T>(shared_from_this()); }
  ...
  public:
    ...
    protected:
      ...
      view_iterator(
        entity_component_system* manager, const component_mask_t mask, std::uint32_t index)
      ...
      void next() {
        while (i_ < capacity_ && !predicate()) { ++i_; }
    ...
    public:
      ...
      void for_each(typename identity<std::function<void(entity entity, Components&...)>>::type f) {
        for (auto it : *this) { f(it, *(it.template get_component<Components>().lock().get())...); }
      }
    ...
    template <typename... Components> class unpacking_view {
    public:
      struct unpacker {
        explicit unpacker(chandle<Components>&... handles)
          : handles(std::tuple<chandle<Components>&...>(handles...)) {}

        void unpack(entity& entity) const { unpack_<0, Components...>(entity); }

      private:
        template <int N, typename C> void unpack_(entity& entity) const {
      ...
      public:
        ...
        void next_entity(entity& entity) { unpacker_.unpack(entity); }
      ...
      iterator_type end() {
        return iterator_type(
          manager_, mask_, static_cast<std::uint32_t>(manager_->capacity()), unpacker_);
      }
    ...
    private:
      ...
      unpacking_view(
        entity_component_system* manager, component_mask_t mask, chandle<Components>&... handles)
     ...
     */
    template <typename C, typename... Args> chandle<C> assign(entity::id_t id, Args&&... args) {
     ...
     */
    template <typename C> void remove(entity::id_t id) {
     ...
     */
    template <typename C> bool has_component(entity::id_t id) const {
     ...
     * have that component.
     */
    template <typename C> chandle<C> get_component(entity::id_t id) {
      assert_valid(id);
      ...
      // anyway.
      if (family >= component_pools_.size()) { return chandle<C>(); }
      auto& pool = component_pools_[family];
      if (!pool || !entity_component_mask_[id.index()][family]) { return chandle<C>(); }
     ...
     * @endcode
     */
    template <typename... Components> view<Components...> entities_with_components() {
     ...
     */
    base_view<true> all_entities() { return base_view<true>(this); }

    template <typename C> void unpack(entity::id_t id, chandle<C>& a) {
      assert_valid(id);
  ...
  private:
    ...
    inline void assert_valid(entity::id_t id) const {
      expects(
        id.index() < entity_component_mask_.size() && "entity::Id ID outside entity vector range");
      expects(
        entity_version_[id.index()] == id.version()
    ...
    template <typename C> component_mask_t component_mask() {
      component_mask_t mask;
      mask.set(rtti::type_index_sequential_t::id<component, C>());
      return mask;
    }

    template <typename C1, typename C2, typename... Components> component_mask_t component_mask() {
      return component_mask<C1>() | component_mask<C2, Components...>();
    }

    template <typename C> component_mask_t component_mask(const chandle<C>& /*unused*/) {
      return component_mask<C>();
    }

    template <typename C1, typename... Components>
    component_mask_t
    component_mask(const chandle<C1>& /*unused*/, const chandle<Components>&... /*unused*/) {
    ...
    template <typename C> component_storage& accomodate_component() {
      auto family = rtti::type_index_sequential_t::id<component, C>();
      return accomodate_component(family);
    }

    component_storage& accomodate_component(rtti::type_index_sequential_t::index_t family) {
      if (component_pools_.size() <= family) { component_pools_.resize(family + 1); }
  ...
  template <typename C, typename... Args> chandle<C> entity::assign(Args&&... args) {
    expects(valid());
  ...
  template <typename C> chandle<C> entity::get_component() const {
    expects(valid());
    return manager_->get_component<C>(id_);
  }

  template <typename... Components> std::tuple<chandle<Components>...> entity::components() const {
    expects(valid());
  ...
  template <typename C> bool entity::has_component() const {
  ...
  inline bool entity::valid() const { return (manager_ != nullptr) && manager_->valid(id_); }
