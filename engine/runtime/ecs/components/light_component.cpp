...
int light_component::compute_projected_sphere_rect(
  ...
  if (light_.type == light_type::point) {
    ...
    float clamped_outer_cone_angle = math::clamp(
      math::radians(light_.spot_data.get_outer_angle()),
    ...
    return math::compute_projected_sphere_rect(
      rect.left, rect.right, rect.top, rect.bottom, center, radius, view, proj);
  } else {
    return 1;
  }
}
