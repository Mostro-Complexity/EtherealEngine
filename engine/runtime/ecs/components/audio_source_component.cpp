...
void audio_source_component::set_autoplay(bool on) {
  ...
  // Should this be here?
  if (auto_play_ && !is_playing()) { play(); }
}

bool audio_source_component::get_autoplay() const { return auto_play_; }

float audio_source_component::get_volume() const { return volume_; }

float audio_source_component::get_pitch() const { return pitch_; }

float audio_source_component::get_volume_rolloff() const { return volume_rolloff_; }

const frange_t& audio_source_component::get_range() const { return range_; }
...
void audio_source_component::stop() { source_.stop(); }

void audio_source_component::pause() { source_.pause(); }

bool audio_source_component::is_playing() const { return source_.is_playing(); }

bool audio_source_component::is_paused() const { return source_.is_paused(); }

bool audio_source_component::is_looping() const { return loop_; }
...
asset_handle<audio::sound> audio_source_component::get_sound() const { return sound_; }

bool audio_source_component::has_binded_sound() const { return source_.has_binded_sound(); }
...
bool audio_source_component::is_sound_valid() const { return sound_ && sound_->is_valid(); }
