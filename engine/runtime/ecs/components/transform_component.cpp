...
void transform_component::on_entity_set() {
  for (auto& child : children_) {
    if (child.valid()) {
      ...
      if (child_transform) { child_transform->parent_ = get_entity(); }
    }
  }
}
...
transform_component::~transform_component() {
  if (parent_.valid()) {
    ...
    if (parent_transform) { parent_transform->remove_child(get_entity()); }
  }
...
void transform_component::set_local_position(const math::vec3& position) {
  ...
  if (math::all(math::equal(this_pos, position, math::epsilon<float>()))) { return; }
...
math::vec3 transform_component::get_local_scale() { return local_transform_.get_scale(); }

const math::vec3& transform_component::get_local_position() {
  return local_transform_.get_position();
}

math::quat transform_component::get_local_rotation() { return local_transform_.get_rotation(); }

math::vec3 transform_component::get_local_x_axis() { return local_transform_.x_unit_axis(); }

math::vec3 transform_component::get_local_y_axis() { return local_transform_.y_unit_axis(); }

math::vec3 transform_component::get_local_z_axis() { return local_transform_.z_unit_axis(); }

const math::vec3& transform_component::get_position() { return get_transform().get_position(); }

math::quat transform_component::get_rotation() { return get_transform().get_rotation(); }

math::vec3 transform_component::get_x_axis() { return get_transform().x_unit_axis(); }

math::vec3 transform_component::get_y_axis() { return get_transform().y_unit_axis(); }

math::vec3 transform_component::get_z_axis() { return get_transform().z_unit_axis(); }

math::vec3 transform_component::get_scale() { return get_transform().get_scale(); }
...
void transform_component::look_at(const math::vec3& eye, const math::vec3& at) {
  math::transform m = math::lookAt(eye, at, math::vec3{ 0.0f, 1.0f, 0.0f });
  set_transform(math::inverse(m));
}

void transform_component::look_at(
  const math::vec3& eye, const math::vec3& at, const math::vec3& up) {
  math::transform m = math::lookAt(eye, at, up);
  set_transform(math::inverse(m));
}
...
void transform_component::set_scale(const math::vec3& scale) {
  ...
  if (math::all(math::equal(this_scale, scale, math::epsilon<float>()))) { return; }
...
void transform_component::set_rotation(const math::quat& rotation) {
  ...
  if (math::all(math::equal(this_rotation, rotation, math::epsilon<float>()))) { return; }
...
void transform_component::reset_rotation() { set_rotation(math::quat{}); }

void transform_component::reset_scale() { set_scale(math::vec3{ 1.0f, 1.0f, 1.0f }); }

void transform_component::reset_local_rotation() { set_local_rotation(math::quat{}); }

void transform_component::reset_local_scale() { set_local_scale(math::vec3{ 1.0f, 1.0f, 1.0f }); }
...
void transform_component::set_parent(runtime::entity parent) { set_parent(parent, true, false); }

bool check_parent(const runtime::entity& e, const runtime::entity& parent) {
  if (e == parent) return false;

  if (parent.valid() && (parent.has_component<transform_component>() == false)) return false;

  if (e.valid()) {
    auto e_transform = e.get_component<transform_component>().lock();
    if (e_transform) {
      for (const auto& child : e_transform->get_children()) {
        if (false == check_parent(child, parent)) return false;
      }
    }
  }
...
void transform_component::set_parent(
  runtime::entity parent, bool world_position_stays, bool local_position_stays) {
  // Skip if this is a no-op.
  if (check_parent(get_entity(), parent) == false) { return; }
  ...
  if (parent_.valid()) {
    auto parent_transform = parent_.get_component<transform_component>().lock();
    if (parent_transform) { parent_transform->attach_child(get_entity()); }
  }
  ...
  if (world_position_stays) {
    ...
    if (!local_position_stays) set_local_transform(math::transform::identity());
  }

  set_dirty(is_dirty());
}

const runtime::entity& transform_component::get_parent() const { return parent_; }
...
void transform_component::set_transform(const math::transform& tr) {
  if (world_transform_.is_equal(tr, math::epsilon<float>())) { return; }
...
void transform_component::set_local_transform(const math::transform& trans) {
  ...
  if (math::all(math::equal(this_matrix, rhs_matrix, math::epsilon<float>()))) { return; }
...
bool transform_component::is_dirty() const { return dirty_; }
...
void transform_component::set_dirty(bool dirty) {
  ...
  if (dirty_ == true) {
    ...
    for (const auto& child : children_) {
      if (child.valid()) {
        auto child_transform = child.get_component<transform_component>().lock();
        if (child_transform) { child_transform->set_dirty(dirty); }
      }
    }
  }
}

const std::vector<runtime::entity>& transform_component::get_children() const { return children_; }
