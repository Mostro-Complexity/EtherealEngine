...
int reflection_probe_component::compute_projected_sphere_rect(
  irect32_t& rect, const math::vec3& position, const math::transform& view,
  const math::transform& proj) {
  if (probe_.type == probe_type::sphere) {
    return math::compute_projected_sphere_rect(
      rect.left, rect.right, rect.top, rect.bottom, position, probe_.sphere_data.range, view, proj);
    ...
    return math::compute_projected_sphere_rect(
      rect.left, rect.right, rect.top, rect.bottom, position, d, view, proj);
  } else {
    return 1;
  }
}
...
void reflection_probe_component::update() {
  for (auto& view : render_view_) { view.release_unused_resources(); }
}

void reflection_probe_component::set_probe(const reflection_probe& probe) {
  if (probe == probe_) return;
