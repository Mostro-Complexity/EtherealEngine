...
namespace runtime {
  ...
  component_storage::component_storage(std::size_t size) { expand(size); }

  void component_storage::expand(std::size_t n) { data.resize(n); }

  void component_storage::reserve(std::size_t n) { data.reserve(n); }
  ...
  component_storage::set(unsigned int index, const std::shared_ptr<component>& component) {
  ...
  entity_component_system::~entity_component_system() { dispose(); }

  size_t entity_component_system::size() const {
    return entity_component_mask_.size() - free_list_.size();
  }

  size_t entity_component_system::capacity() const { return entity_component_mask_.size(); }
  ...
  void
  entity_component_system::remove(entity::id_t id, const std::shared_ptr<component>& component) {
    remove(id, component->runtime_id());
  }

  void
  entity_component_system::remove(entity::id_t id, rtti::type_index_sequential_t::index_t family) {
    assert_valid(id);
  ...
  bool entity_component_system::has_component(
    entity::id_t id, const std::shared_ptr<component>& component) const {
    return has_component(id, component->runtime_id());
  }

  bool entity_component_system::has_component(
    entity::id_t id, rtti::type_index_sequential_t::index_t family) const {
    assert_valid(id);
    // We don't bother checking the component mask, as we return a nullptr anyway.
    if (family >= component_pools_.size()) { return false; }
    auto& pool = component_pools_[family];
  ...
  std::vector<chandle<component>> entity_component_system::all_components(entity::id_t id) const {
    ...
    for (size_t i = 0; i < component_pools_.size(); ++i) {
      if (mask.test(i)) {
        ...
        if (pool) { components.push_back(chandle<component>(pool->get(id.index()))); }
      }
    }
    return components;
  }
  ...
  entity_component_system::all_components_shared(entity::id_t id) const {
    std::vector<std::shared_ptr<component>> components;
    auto                                    mask = component_mask(id);
    for (size_t i = 0; i < component_pools_.size(); ++i) {
      if (mask.test(i)) {
        auto& pool = component_pools_[i];
        if (pool) { components.push_back(pool->get(id.index())); }
      }
    }
    return components;
  }
  ...
  chandle<component>
  entity_component_system::assign(entity::id_t id, const std::shared_ptr<component>& comp) {
    assert_valid(id);
