...
namespace runtime {
  namespace asset_reader {
    ...
    template <>
    bool load_from_file<gfx::texture>(
      core::task_future<asset_handle<gfx::texture>>& output, const std::string& key) {
      asset_handle<gfx::texture> original;
      if (output.is_ready()) { original = output.get(); }
      ...
      if (!fs::exists(compiled_absolute_key, err)) {
        APPLOG_ERROR(
          "Asset with key {0} and absolute_path {1} does not exist!", key, compiled_absolute_key);
        output = ts.push_or_execute_on_worker_thread(create_resource_func_fallback);
        return true;
      }
      ...
      auto create_resource_func = [result = original, read_memory, key](bool read_result) mutable {
        if (!read_result) { return result; }
        // if someone destroyed our memory
        if (!read_memory) { return result; }
        // if nothing was read
        if (read_memory->empty()) { return result; }
    ...
    template <>
    bool load_from_file<gfx::shader>(
      core::task_future<asset_handle<gfx::shader>>& output, const std::string& key) {
    ...
    template <>
    bool
    load_from_file<mesh>(core::task_future<asset_handle<mesh>>& output, const std::string& key) {
      ...
      auto read_memory_func = [wrapper, compiled_absolute_key]() mutable {
        ...
        wrapper->mesh->set_vertex_source(
          &data.vertex_data[0], data.vertex_count, data.vertex_format);
    ...
    template <>
    bool load_from_file<audio::sound>(
      core::task_future<asset_handle<audio::sound>>& output, const std::string& key) {
    ...
    template <>
    bool load_from_file<runtime::animation>(
      core::task_future<asset_handle<runtime::animation>>& output, const std::string& key) {
    ...
    template <>
    bool load_from_file<material>(
      core::task_future<asset_handle<material>>& output, const std::string& key) {
    ...
    template <>
    bool load_from_file<prefab>(
      core::task_future<asset_handle<prefab>>& output, const std::string& key) {
      ...
      auto read_memory_func = [read_memory, compiled_absolute_key]() {
        ...
        auto stream = std::fstream{ compiled_absolute_key,
    ...
    template <>
    bool
    load_from_file<scene>(core::task_future<asset_handle<scene>>& output, const std::string& key) {
