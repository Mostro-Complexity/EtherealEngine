...
namespace runtime {
  ...
  public:
    ...
    /// </summary>
    //-----------------------------------------------------------------------------
    template <typename S, typename... Args> asset_storage<S>& add_storage(Args&&... args) {
      auto operation = storages_.emplace(
    ...
    load(const std::string& key, load_flags flags = load_flags::standard) {
      auto& storage = get_storage<T>();
      return load_asset_from_file_impl<T>(
        key, flags, storage.container_mutex, storage.container, storage.load_from_file);
    }
    ...
    core::task_future<asset_handle<T>> create_asset_from_memory(
      const std::string& key, const std::uint8_t* data, const std::uint32_t& size,
    ...
    template <typename T>
    core::task_future<asset_handle<T>>
    load_asset_from_instance(const std::string& key, std::shared_ptr<T> entry) {
      auto& storage = get_storage<T>();
      return load_asset_from_instance_impl(
        key, entry, storage.container_mutex, storage.container, storage.load_from_instance);
    }

    template <typename T> void rename_asset(const std::string& key, const std::string& new_key) {
      auto& storage = get_storage<T>();
    ...
    template <typename T> void clear_asset(const std::string& key) {
      auto& storage = get_storage<T>();
  ...
  private:
    ...
    core::task_future<asset_handle<T>> load_asset_from_file_impl(
      const std::string& key, load_flags flags, std::recursive_mutex& container_mutex,
    ...
    core::task_future<asset_handle<T>>& load_asset_from_instance_impl(
      const std::string& key, std::shared_ptr<T> entry, std::recursive_mutex& container_mutex,
      typename asset_storage<T>::request_container_t& container, F&& load_func) {
      std::lock_guard<std::recursive_mutex> lock(container_mutex);
      auto&                                 future = container[key];
      // Dispatch the loading
    ...
    core::task_future<asset_handle<T>> find_asset_impl(
      const std::string& key, std::recursive_mutex& container_mutex,
    ...
    /// </summary>
    //-----------------------------------------------------------------------------
    template <typename S> asset_storage<S>& get_storage() {
