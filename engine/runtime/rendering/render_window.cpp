...
void render_window::on_resize() { prepare_surface(); }
...
render_window::render_window(
  mml::video_mode mode, const std::string& title, std::uint32_t style /*= mml::style::default*/)
  : mml::window(mode, title, style) {
  id_ = s_next_id++;
  prepare_surface();
}

render_window::~render_window() { destroy_surface(); }
