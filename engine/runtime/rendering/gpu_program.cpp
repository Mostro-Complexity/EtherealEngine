...
gpu_program::gpu_program(
  asset_handle<gfx::shader> vertex_shader, asset_handle<gfx::shader> fragment_shader) {
...
void gpu_program::populate() {
  bool all_valid = std::all_of(std::begin(shaders_), std::end(shaders_), [](auto& shader) {
  ...
  if (all_valid) {
    if (shaders_.size() == 1) {
      ...
      program_ =
        std::make_unique<gfx::program>(vertex_shader.get_asset(), fragment_shader.get_asset());
    }

    shaders_cached_.clear();
    for (const auto& shader : shaders_) { shaders_cached_.push_back(shader->native_handle().idx); }
  }
}

void gpu_program::set_texture(
  uint8_t _stage, const std::string& _sampler, gfx::frame_buffer* _fbo, uint8_t _attachment,
  uint32_t _flags) {
  program_->set_texture(_stage, _sampler, _fbo, _attachment, _flags);
}

void gpu_program::set_texture(
  uint8_t _stage, const std::string& _sampler, gfx::texture* _texture, uint32_t _flags) {
...
gfx::program::handle_type_t gpu_program::native_handle() const { return program_->native_handle(); }

const std::vector<asset_handle<gfx::shader>>& gpu_program::get_shaders() const { return shaders_; }
