...
float camera::get_zoom_factor() const {
  if (viewport_size_.height == 0) { return 0.0f; }

  return ortho_size_ / (float(viewport_size_.height) / 2.0f);
}

float camera::get_ppu() const { return float(viewport_size_.height) / (2.0f * ortho_size_); }
...
void camera::set_fov(float fFOVY) {
  // Skip if no-op
  if (math::equal(fFOVY, fov_, math::epsilon<float>())) { return; }
...
void camera::set_projection_mode(projection_mode mode) {
  ...
  if (mode == projection_mode_) { return; }
...
void camera::set_near_clip(float distance) {
  ...
  if (math::equal(distance, near_clip_, math::epsilon<float>())) { return; }
  ...
  if (near_clip_ > far_clip_) { set_far_clip(near_clip_); }
}

void camera::set_far_clip(float distance) {
  // Skip if this is a no-op
  if (math::equal(distance, far_clip_, math::epsilon<float>())) { return; }
  ...
  // Make sure near clip is less than the far clip
  if (near_clip_ > far_clip_) { set_near_clip(far_clip_); }
}
...
math::vec3 camera::get_position() const { return math::inverse(view_).get_position(); }

math::vec3 camera::x_unit_axis() const { return math::inverse(view_).x_unit_axis(); }
math::vec3 camera::y_unit_axis() const { return math::inverse(view_).y_unit_axis(); }

math::vec3 camera::z_unit_axis() const { return math::inverse(view_).z_unit_axis(); }
...
const math::frustum& camera::get_frustum() const {
  ...
  if (frustum_dirty_ && !frustum_locked_) {
    ...
    clipping_volume_.planes[math::volume_plane::near_plane].data.w =
      ...
      -math::plane::dot_coord(
        clipping_volume_.planes[math::volume_plane::near_plane], get_position());
...
const math::frustum& camera::get_clipping_volume() const {
  // Recalculate frustum if necessary
  if (frustum_dirty_ && !frustum_locked_) { get_frustum(); }
...
math::volume_query
camera::bounds_in_frustum(const math::bbox& AABB, const math::transform& t) const {
  // Recompute the frustum as necessary.
  const math::frustum& f = get_frustum();
...
bool camera::viewport_to_ray(
  const math::vec2& point, math::vec3& vec_ray_start, math::vec3& vec_ray_dir) const {
  ...
  math::vec3 cursor;
  cursor.x =
    (((2.0f * (point.x - viewport_pos_.x)) / float(viewport_size_.width)) - 1.0f) / mtx_proj[0][0];
  cursor.y = -(((2.0f * (point.y - viewport_pos_.y)) / float(viewport_size_.height)) - 1.0f)
             / mtx_proj[1][1];
  cursor.z = 1.0f;
...
bool camera::viewport_to_world(
  const math::vec2& point, const math::plane& pl, math::vec3& world_pos, bool clip) const {
  ...
  if (!viewport_to_ray(point, pick_ray_origin, pick_ray_dir)) { return false; }
  ...
  if (math::abs<float>(proj_ray_length) < math::epsilon<float>()) { return false; }
...
bool camera::viewport_to_major_axis(
  const math::vec2& point, const math::vec3& Origin, math::vec3& world_pos,
  math::vec3& major_axis) const {
  return viewport_to_major_axis(point, Origin, z_unit_axis(), world_pos, major_axis);
}

bool camera::viewport_to_major_axis(
  const math::vec2& point, const math::vec3& Origin, const math::vec3& Normal,
  math::vec3& world_pos, math::vec3& major_axis) const {
  ...
  if (
    math::abs<float>(x - y) < math::epsilon<float>()
    && math::abs<float>(x - z) < math::epsilon<float>()) {
...
float camera::estimate_zoom_factor(const math::plane& pl) const {
  ...
  if (get_projection_mode() == projection_mode::orthographic) { return get_zoom_factor(); }

  math::vec3 world;
  // Otherwise, estimate is based on the distance from the grid plane.
  viewport_to_world(
    math::vec2(float(viewport_size_.width) / 2.0f, float(viewport_size_.height) / 2.0f),
    pl,
    world,
    false);
...
math::vec3 camera::estimate_pick_tolerance(
  float wire_tolerance, const math::vec3& pos, const math::transform& object_transform) const {
...
void camera::set_aa_data(
  const usize32_t& viewport_size, std::uint32_t current_subpixel_index,
  ...
  if (temporal_aa_samples > 1) {
    ...
    aa_data_ =
      math::vec4(float(current_subpixel_index), float(temporal_aa_samples), SampleX, SampleY);
