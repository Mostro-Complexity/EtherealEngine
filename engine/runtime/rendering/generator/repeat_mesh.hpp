...
namespace generator {
   ...
   */
  template <typename mesh_t> class repeat_mesh_t {
  public:
    class triangles_t {
    public:
      bool done() const noexcept { return index_ >= repeat_mesh_->instances_; }
    ...
    private:
      ...
      explicit triangles_t(const repeat_mesh_t* repeatMesh) noexcept
        : repeat_mesh_{ repeatMesh }, triangles_{ repeatMesh->mesh_.triangles() },
          index_{ repeatMesh->vertex_count_ > 0 ? 0 : repeatMesh->instances_ }, delta_{ 0 } {}

      int counttriangles_t() const noexcept {
        if (repeat_mesh_->instances_ < 1) return 0;
    ...
    private:
      explicit vertices_t(const repeat_mesh_t* repeatMesh)
        : repeat_mesh_{ repeatMesh }, vertices_{ repeatMesh->mesh_.vertices() },
          index_{ repeatMesh->vertex_count_ > 0 ? 0 : repeatMesh->instances_ }, delta_{} {}
      ...
      int countvertices_t() const noexcept {
        ...
        return repeat_mesh_->vertex_count_ * (repeat_mesh_->instances_ - index_ - 1)
               + count(vertices_);
      }
    ...
    explicit repeat_mesh_t(mesh_t mesh, int instances, const gml::dvec3& delta) noexcept
      : mesh_{ std::move(mesh) }, instances_{ instances }, delta_{ delta }, vertex_count_{ count(
                                                                              mesh_.vertices()) } {}
