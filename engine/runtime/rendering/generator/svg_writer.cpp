...
svg_writer_t::LineElem::LineElem(
  const gml::dvec3& p1, const gml::dvec3& p2, const gml::dvec3& color)
  : BaseElem{ (p1[2] + p2[2]) / 2.0, color }, p1_(p1), p2_(p2) {}
...
svg_writer_t::TriangleElem::TriangleElem(
  const gml::dvec3& p1, const gml::dvec3& p2, const gml::dvec3& p3, const gml::dvec3& color)
  : BaseElem{ (p1[2] + p2[2] + p3[2]) / 3.0, color }, p_{ { p1, p2, p3 } } {}

void svg_writer_t::TriangleElem::stream(std::ostream& os) const {
  os << "<polygon "
     << "points=\"";
  for (gml::dvec3 p : p_) { os << p[0] << "," << p[1] << " "; }
  os << "\" ";
...
svg_writer_t::svg_writer_t(int width, int height)
  : size_{ width, height }, viewMatrix_{ 1.0 }, projMatrix_{ 1.0 }, viewProjMatrix_{ 1.0 },
    viewportOrigin_{ 0, 0 },
    viewportSize_{ width, height }, lightDir_{ 1.0, 2.0, 3.0 }, cullface_{ true }, elems_{} {
...
void svg_writer_t::cullface(bool cullface) { cullface_ = cullface; }
...
void svg_writer_t::writeTriangle(
  const gml::dvec3& p1, const gml::dvec3& p2, const gml::dvec3& p3, const gml::dvec3& color) {
