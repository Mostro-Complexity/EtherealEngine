...
namespace generator {
  ...
  template <typename generator_t> class generated_type {
  public:
  ...
  template <typename primitive_t> class edge_generator_type {
  public:
  ...
  template <typename primitive_t> class triangle_generator_type {
  public:
  ...
  template <typename primitive_t> class vertex_generator_type {
  public:
    using type = decltype(static_cast<const primitive_t*>(nullptr)->vertices());
  };

  /// Counts the number of steps left in the generator.
  template <typename generator_t> int count(const generator_t& generator) noexcept {
