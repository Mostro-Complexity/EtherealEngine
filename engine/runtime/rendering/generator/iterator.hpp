...
namespace generator {
  ...
  template <typename generator_t> class iterator_t {
  private:
  ...
  public:
    ...
    /// generator.
    bool operator==(const iterator_t& that) const noexcept { return generator_ == that.generator_; }

    bool operator!=(const iterator_t& that) const noexcept { return generator_ != that.generator_; }
  };

  /// Will return an iterator to the generator.
  template <typename generator_t> iterator_t<generator_t> begin(generator_t& generator) noexcept {
    return iterator_t<generator_t>{ generator };
  }

  /// Returns a dummy end iterator
  template <typename generator_t> iterator_t<generator_t> end(const generator_t&) noexcept {
