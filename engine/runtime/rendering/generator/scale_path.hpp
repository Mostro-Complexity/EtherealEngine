...
namespace generator {
  ...
  template <typename path_t> class scale_path_t {
  private:
  ...
  public:
    ...
    edges_t edges() const noexcept { return transform_path_.edges(); }

    using vertices_t = typename impl_t::vertices_t;

    vertices_t vertices() const noexcept { return transform_path_.vertices(); }
  };

  template <typename path_t> scale_path_t<path_t> scale_path(path_t path, const gml::dvec3& scale) {
