...
namespace generator {
  ...
  template <typename path_t> class transform_path_t {
  private:
    using impl_t = path_t;
    impl_t path_;
  ...
  public:
    ...
    private:
      explicit vertices_t(const transform_path_t& path)
        : path_{ &path }, vertices_{ path.path_.vertices() } {}
  ...
  transform_path_t<path_t>
  transform_path(path_t path, const std::function<void(path_vertex_t&)>& mutate) {
