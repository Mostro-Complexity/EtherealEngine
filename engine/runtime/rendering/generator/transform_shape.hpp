...
namespace generator {
  ...
  template <typename shape_t> class transform_shape_t {
  private:
  ...
  public:
    ...
    private:
      ...
      explicit vertices_t(const transform_shape_t& shape)
        : shape_{ &shape }, vertices_{ shape.shape_.vertices() } {}
    ...
    transform_shape_t(shape_t shape, const std::function<void(shape_vertex_t&)>& mutate)
      : shape_{ std::move(shape) }, mutate_{ mutate } {}

    using edges_t = typename impl_t::edges_t;

    edges_t edges() const noexcept { return shape_.edges(); }
  ...
  transform_shape(shape_t shape, const std::function<void(shape_vertex_t&)>& mutate) {
