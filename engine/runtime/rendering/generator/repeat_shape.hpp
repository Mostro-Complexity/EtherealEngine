...
namespace generator {
   ...
   */
  template <typename shape_t> class repeat_shape_t {
  public:
    class edges_t {
    public:
      bool done() const noexcept { return index_ >= repeat_shape_->instances_; }
    ...
    private:
      ...
      explicit edges_t(const repeat_shape_t* repeatShape) noexcept
        : repeat_shape_{ repeatShape }, edges_{ repeatShape->shape_.edges() },
          index_{ repeatShape->vertex_count_ > 0 ? 0 : repeatShape->instances_ }, delta_{ 0 } {}

      int countEdges() const noexcept {
        if (repeat_shape_->instances_ < 1) return 0;

        return count(repeat_shape_->shape_.edges()) * (repeat_shape_->instances_ - index_ - 1)
               + count(edges_);
      }

      friend int count(const edges_t& generator) noexcept { return generator.countEdges(); }
    ...
    private:
      explicit vertices_t(const repeat_shape_t* repeatShape)
        : repeat_shape_{ repeatShape }, vertices_{ repeatShape->shape_.vertices() },
          index_{ repeatShape->vertex_count_ > 0 ? 0 : repeatShape->instances_ }, delta_{} {}
      ...
      int countvertices_t() const noexcept {
        ...
        return repeat_shape_->vertex_count_ * (repeat_shape_->instances_ - index_ - 1)
               + count(vertices_);
      }

      friend int count(const vertices_t& generator) noexcept { return generator.countvertices_t(); }
    ...
    explicit repeat_shape_t(shape_t shape, int instances, const gml::dvec2& delta) noexcept
      : shape_{ std::move(shape) }, instances_{ instances }, delta_{ delta }, vertex_count_{
  ...
  repeat_shape_t<shape_t>
  repeat_shape(shape_t shape, int instances, const gml::dvec2& delta) noexcept {
