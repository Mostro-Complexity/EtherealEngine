...
namespace generator {
  ...
  template <typename mesh_t, int Iterations> class subdivide_mesh_t {
  ...
  public:
    ...
    triangles_t triangles() const noexcept { return subdivide_mesh_.triangles(); }

    using vertices_t = typename impl_t::vertices_t;

    vertices_t vertices() const noexcept { return subdivide_mesh_.vertices(); }
  };

  template <typename mesh_t> class subdivide_mesh_t<mesh_t, 0> {
  private:
    using impl_t = mesh_t;
    impl_t mesh_;
  ...
  public:
    ...
    triangles_t triangles() const noexcept { return mesh_.triangles(); }

    using vertices_t = typename impl_t::vertices_t;

    vertices_t vertices() const noexcept { return mesh_.vertices(); }
  };

  /// Subdivides each triangle to 4 parts by splitting edges.
  template <typename mesh_t> class subdivide_mesh_t<mesh_t, 1> {
  public:
    class triangles_t {
    public:
    ...
    private:
      ...
      explicit triangles_t(const subdivide_mesh_t& mesh)
        : mesh_{ &mesh }, i_{ 0 }, triangles_{ mesh.mesh_.triangles() }, triangle_{} {}
    ...
    public:
      bool done() const noexcept {
        return vertex_index_ == mesh_->vertex_cache_.size()
               && edge_index_ == mesh_->edge_cache_.size();
      }

      mesh_vertex_t generate() const {
        if (vertex_index_ < mesh_->vertex_cache_.size()) return mesh_->vertex_cache_[vertex_index_];
    ...
    private:
      ...
      explicit vertices_t(const subdivide_mesh_t& mesh)
        : mesh_{ &mesh }, edge_index_{ 0 }, vertex_index_{ 0 } {}

      friend class subdivide_mesh_t;
    };

    subdivide_mesh_t(mesh_t mesh) : mesh_{ std::move(mesh) } {
      for (const mesh_vertex_t& vertex : mesh_.vertices()) { vertex_cache_.push_back(vertex); }
      ...
      for (const triangle_t& triangle : mesh_.triangles()) {
        for (int i = 0; i < 3; ++i) {
          ...
          if (e.vertices[0] > e.vertices[1]) std::swap(e.vertices[0], e.vertices[1]);
    ...
    triangles_t triangles() const noexcept { return triangles_t{ *this }; }

    vertices_t vertices() const noexcept { return vertices_t{ *this }; }
