...
namespace generator {
  ...
  template <typename... Mesh> class merge_mesh_t;  // undefined

  template <> class merge_mesh_t<> : public empty_mesh {};

  template <typename Head, typename... Tail> class merge_mesh_t<Head, Tail...> {
  public:
    class triangles_t {
    public:
      ...
      bool done() const noexcept { return all_done_; }
    ...
    private:
      ...
      explicit triangles_t(const merge_mesh_t& mesh)
        : head_{ mesh.head_.triangles() }, tail_(mesh.tail_.triangles()),
          head_vertex_count_{ count(mesh.head_.vertices()) }, all_done_{ tail_.done()
                                                                         && head_.done() } {}
    ...
    private:
      ...
      explicit vertices_t(const merge_mesh_t& mesh)
        : head_{ mesh.head_.vertices() },
          tail_(mesh.tail_.vertices()), all_done_{ tail_.done() && head_.done() } {}
  ...
  template <typename... Mesh> merge_mesh_t<Mesh...> merge_mesh(Mesh... meshes) {
