...
convex_polygon_mesh_t::triangles_t::triangles_t(const convex_polygon_mesh_t& mesh) noexcept
  : mesh_{ &mesh }, odd_{ false }, segment_index_{ 0 }, side_index_{ 0 }, ring_index_{ 0 } {}
...
void convex_polygon_mesh_t::triangles_t::next() {
  ...
  if (ring_index_ == mesh_->rings_ - 1) {
    ...
    if (segment_index_ == mesh_->segments_) {
      ...
      if (side_index_ == static_cast<int>(mesh_->vertices_.size())) { ++ring_index_; }
    }
  } else {
...
convex_polygon_mesh_t::vertices_t::vertices_t(const convex_polygon_mesh_t& mesh) noexcept
  : mesh_{ &mesh }, center_done_{ false }, segment_index_{ 0 }, side_index_{ 0 }, ring_index_{ 0 } {
}
...
convex_polygon_mesh_t::convex_polygon_mesh_t(
  ...
  : vertices_{ std::move(vertices) }, segments_{ segments }, rings_{ rings }, center_{ calcCenter(
                                                                                vertices_) },
    normal_{ calcNormal(center_, vertices_) }, tangent_{}, bitangent_{}, tex_delta_{} {
