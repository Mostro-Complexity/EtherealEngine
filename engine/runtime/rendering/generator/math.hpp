...
namespace gml {
  ...
  template <typename T> T angle(const glm::tvec2<T>& v1, const glm::tvec2<T>& v2) {
    using std::acos;
  ...
  template <typename T> glm::tvec2<T> cross(const glm::tvec2<T>& v) {
    return glm::tvec2<T>(-v.y, v.x);
  }

  template <typename T> glm::tvec3<T> transform(const glm::tquat<T>& q, const glm::tvec3<T>& v) {
    const glm::tvec3<T> temp = T{ 2.0 } * cross(glm::tvec3<T>(q.x, q.y, q.z), v);
    return v + q.w * temp + cross(glm::tvec3<T>(q.x, q.y, q.z), temp);
  }

  template <typename T> glm::tquat<T> qrotate(const T& angle, const glm::tvec3<T>& axis) {
    using std::cos;
    using std::sin;
  ...
  glm::tvec3<T> project(
    const glm::tvec3<T>& v, const glm::tmat4x4<T>& modelViewProj,
  ...
  template <typename T> glm::tmat4x4<T> rotate(const glm::tvec3<T>& angle) {
    using std::cos;
    using std::sin;
  ...
  template <typename T> glm::tmat3x3<T> rotate(const T& angle) {
    using std::cos;
    using std::sin;
  ...
  template <typename T> glm::tvec2<T> transform(const glm::tmat3x3<T>& m, const glm::tvec2<T>& v) {
  ...
  template <int D, typename T> glm::tvec2<T> bezier(const glm::tvec2<T> (&p)[D], T t) {
    static_assert(D > 0, "At least one control point needed.");
    return detail::bezierImpl(&p[0], D, static_cast<T>(1) - t, t);
  }

  template <int D, typename T> glm::tvec3<T> bezier(const glm::tvec3<T> (&p)[D], T t) {
    static_assert(D > 0, "At least one control point needed.");
    return detail::bezierImpl(&p[0], D, static_cast<T>(1) - t, t);
  }
  ...
  glm::tvec3<T> bezier2(const glm::tvec3<T> (&p)[D1][D0], const glm::tvec2<T>& t) {
    ...
    glm::tvec3<T> temp[D1];
    for (int i = 0; i < D1; ++i) { temp[i] = bezier(p[i], t[0]); }
  ...
  namespace detail {
    ...
    template <int O, int D, typename VecT, typename T> struct bezierDerivativeImpl {
      static VecT calc(const VecT (&p)[D], T t) {
        VecT temp[D - 1];
        for (int i = 0; i < D - 1; ++i) { temp[i] = static_cast<T>(D - 1) * (p[i + 1] - p[i]); }
  ...
  glm::tmat2x3<T> bezier2Jacobian(const glm::tvec3<T> (&p)[D1][D0], const glm::tvec2<T>& t) {
    ...
    for (int i = 0; i < D1; ++i) { temp1[i] = bezier(p[i], t[0]); }
