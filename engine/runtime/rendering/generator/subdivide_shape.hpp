...
namespace generator {
  ...
  template <typename shape_t> class subdivide_shape_t {
  public:
    class edges_t {
    public:
      ...
      edge_t generate() const {
        ...
        if (i_ % 2 == 0)
          return edge_t{ edge_.vertices[0],
                         static_cast<int>(shape_->vertex_cache_.size()) + i_ / 2 };
    ...
    private:
      ...
      explicit edges_t(const subdivide_shape_t& shape)
        : shape_{ &shape }, edges_{ shape.shape_.edges() }, i_{ 0 } {}
    ...
    private:
      ...
      explicit vertices_t(const subdivide_shape_t& shape)
        : shape_{ &shape }, vertex_index_{ 0 }, edges_{ shape.shape_.edges() } {}

      friend class subdivide_shape_t;
    };

    subdivide_shape_t(shape_t shape) : shape_(std::move(shape)), vertex_cache_{} {
      for (const shape_vertex_t& vertex : shape_.vertices()) { vertex_cache_.push_back(vertex); }
    }
