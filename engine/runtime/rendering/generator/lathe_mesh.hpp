...
namespace generator {
  ...
  template <typename shape_t> class lathe_mesh_t {
  public:
    class triangles_t {
    public:
      ...
      bool done() const noexcept { return shape_edges_.done(); }
    ...
    private:
      ...
      explicit triangles_t(const lathe_mesh_t& mesh)
        : mesh_{ &mesh }, shape_edges_{ mesh.shape_.edges() }, i_{ 0 } {}
    ...
    public:
      ...
      bool done() const noexcept { return shape_vertices_.done(); }
    ...
    private:
      ...
      explicit vertices_t(const lathe_mesh_t& mesh)
        : mesh_{ &mesh }, shape_vertices_{ mesh.shape_.vertices() }, i_{ 0 } {}
    ...
    lathe_mesh_t(
      shape_t shape, const gml::dvec2& axis, int slices = 32, double start = 0.0,
      double sweep = gml::radians(360.0))
      : axis_{ axis, 0.0 }, shape_{ std::move(shape) }, slices_{ slices }, start_{ start }, sweep_{
          sweep
        } {}
  ...
  lathe_mesh_t<shape_t> lathe(
    shape_t shape, axis_t axis = axis_t::X, int slices = 32, double start = 0.0,
    double sweep = gml::radians(360.0)) {
