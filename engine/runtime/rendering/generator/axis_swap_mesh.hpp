...
namespace generator {
  ...
  template <typename mesh_t> class axis_swap_mesh_t {
  private:
  ...
  public:
    ...
    public:
      triangle_t generate() const {
        ...
        if (mesh_->flip_) std::swap(triangle.vertices[0], triangle.vertices[2]);
        return triangle;
      }
    ...
    private:
      ...
      explicit triangles_t(const axis_swap_mesh_t& mesh)
        : mesh_{ &mesh }, triangles_{ mesh.transform_mesh_.triangles() } {}
    ...
    axis_swap_mesh_t(mesh_t mesh, axis_t x, axis_t y, axis_t z)
      : transform_mesh_{ std::move(mesh),
        ...
        flip_{ true } {
      if (x != axis_t::X) flip_ = !flip_;
      if (y != axis_t::Y) flip_ = !flip_;
      if (z != axis_t::Z) flip_ = !flip_;
    }
    ...
    vertices_t vertices() const noexcept { return transform_mesh_.vertices(); }
