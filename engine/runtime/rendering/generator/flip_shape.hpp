...
namespace generator {
  ...
  template <typename shape_t> class flip_shape_t {
  private:
  ...
  public:
    ...
    flip_shape_t(shape_t shape)
      : transform_shape_{ std::move(shape),
                          [](shape_vertex_t& vertex) { vertex.tangent *= -1.0; } } {}
  ...
  template <typename shape_t> flip_shape_t<shape_t> flip_shape(shape_t shape) {
