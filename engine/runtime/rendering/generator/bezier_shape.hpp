...
namespace generator {
  ...
  template <int D> class bezier_shape_t {
  private:
    ...
    struct array_wrapper {
      ...
      explicit array_wrapper(const gml::dvec2 (&p)[D]) { std::copy(&p[0], &p[0] + D, &data[0]); }
    };
  ...
  public:
    ...
    edges_t edges() const noexcept { return parametric_shape_.edges(); }

    using vertices_t = typename impl_t::vertices_t;

    vertices_t vertices() const noexcept { return parametric_shape_.vertices(); }
  };
}  // namespace generator
