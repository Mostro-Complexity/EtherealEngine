...
namespace generator {
  ...
  template <typename shape_t, typename path_t> class extrude_mesh_t {
  public:
    class triangles_t {
    public:
      ...
      bool done() const noexcept { return path_edges_.done(); }
    ...
    private:
      ...
      explicit triangles_t(const extrude_mesh_t& mesh)
        : mesh_{ &mesh }, shape_edges_{ mesh.shape_.edges() },
          path_edges_{ mesh.path_.edges() }, odd_{ true } {}
    ...
    public:
      ...
      bool done() const noexcept { return path_vertices_.done(); }
    ...
    private:
      ...
      explicit vertices_t(const extrude_mesh_t& mesh)
        : mesh_{ &mesh }, shape_vertices_{ mesh.shape_.vertices() }, path_vertices_{
            mesh.path_.vertices()
          } {}
    ...
    extrude_mesh_t(shape_t shape, path_t path)
      : shape_{ std::move(shape) }, path_{ std::move(path) }, shape_vertex_count_{ count(
                                                                shape_.vertices()) } {}
