...
namespace generator {
  ...
  public:
    ...
    /// Write one line.
    void writeLine(
      const gml::dvec3& p1, const gml::dvec3& p2, const gml::dvec3& color = { 0.0, 0.0, 0.0 });
    ...
    void writeShape(const shape_t& shape, bool writeVertices = false, bool writeAxis = false) {
      ...
      for (const auto& vertex : shape.vertices()) { vertices.push_back(vertex); }
      ...
      if (writeVertices) {
        for (auto v : shape.vertices()) { writePoint(gml::dvec3{ v.position, 0.0 }); }
      }
    }
    ...
    void writePath(const Path& path, bool writeVertices = false, bool writeAxis = false) {
      ...
      for (const auto& temp : path.vertices()) { vertices.push_back(temp); }
      ...
      if (writeVertices) {
        for (const auto& v : path.vertices()) { writePoint(v.position + 0.001 * v.normal); }
      }
    ...
    void writeMesh(const Mesh& mesh, bool writeVertices = false, bool writeNormals = false) {
      ...
      for (const mesh_vertex_t& vertex : mesh.vertices()) { vertices.push_back(vertex); }
      ...
      if (writeVertices) {
        for (const auto& v : vertices) { writePoint(v.position); }
      }
