...
namespace generator {
  ...
  template <typename path_t> class subdivide_path_t {
  public:
    class edges_t {
    public:
      ...
      edge_t generate() const {
        ...
        if (i_ % 2 == 0)
          return edge_t{ { edge_.vertices[0],
                           static_cast<int>(path_->vertex_cache_.size()) + i_ / 2 } };

        return edge_t{ { static_cast<int>(path_->vertex_cache_.size()) + i_ / 2,
                         edge_.vertices[1] } };
      }
    ...
    private:
      ...
      explicit edges_t(const subdivide_path_t& path)
        : path_{ &path }, edges_{ path.path_.edges() }, i_{ 0 } {}
    ...
    public:
      ...
      path_vertex_t generate() const {
        if (vertex_index_ < path_->vertex_cache_.size()) return path_->vertex_cache_[vertex_index_];
    ...
    private:
      ...
      explicit vertices_t(const subdivide_path_t& path)
        : path_{ &path }, vertex_index_{ 0 }, edges_{ path.path_.edges() } {}

      friend class subdivide_path_t;
    };

    subdivide_path_t(path_t path) : path_(std::move(path)), vertex_cache_{} {
      for (const path_vertex_t& vertex : path_.vertices()) { vertex_cache_.push_back(vertex); }
    }

    edges_t edges() const { return edges_t{ *this }; }

    vertices_t vertices() const { return vertices_t{ *this }; }
  ...
  template <typename path_t> subdivide_path_t<path_t> subdivide_path(path_t path) {
