...
namespace generator {
  ...
  template <typename T> class any_generator {
  public:
  ...
  private:
    ...
    template <typename generator> class derived : public base {
    public:
      ...
      T generate() const override { return generator_.generate(); }

      bool done() const noexcept override { return generator_.done(); }

      void next() override { generator_.next(); }
