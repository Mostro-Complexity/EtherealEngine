...
namespace generator {
   ...
   */
  template <typename path_t> class repeat_path_t {
  public:
    class edges_t {
    public:
      bool done() const noexcept { return index_ >= repeat_path_->instances_; }
    ...
    private:
      ...
      explicit edges_t(const repeat_path_t* repeatPath) noexcept
        : repeat_path_{ repeatPath }, edges_{ repeatPath->path_.edges() },
          index_{ repeatPath->vertex_count_ > 0 ? 0 : repeatPath->instances_ }, delta_{ 0 } {}
    ...
    private:
      explicit vertices_t(const repeat_path_t* repeatPath)
        : repeat_path_{ repeatPath }, vertices_{ repeatPath->path_.vertices() },
          index_{ repeatPath->vertex_count_ > 0 ? 0 : repeatPath->instances_ }, delta_{} {}
      ...
      int countvertices_t() const noexcept {
        if (repeat_path_->instances_ < 1) return 0;

        return repeat_path_->vertex_count_ * (repeat_path_->instances_ - index_ - 1)
               + count(vertices_);
      }
    ...
    explicit repeat_path_t(path_t path, int instances, const gml::dvec3& delta) noexcept
      : path_{ std::move(path) }, instances_{ instances }, delta_{ delta }, vertex_count_{ count(
                                                                              path_.vertices()) } {}
