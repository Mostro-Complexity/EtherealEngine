...
namespace generator {
  ...
  template <typename mesh_t> class uv_flip_mesh_t {
  private:
  ...
  public:
    ...
    uv_flip_mesh_t(mesh_t mesh, bool u, bool v)
      : transform_mesh_{ std::move(mesh), [u, v](mesh_vertex_t& vertex) {
                          if (u) vertex.tex_coord[0] = 1.0 - vertex.tex_coord[0];
                          if (v) vertex.tex_coord[1] = 1.0 - vertex.tex_coord[1];
                        } } {}

    using triangles_t = typename impl_t::triangles_t;

    triangles_t triangles() const noexcept { return transform_mesh_.triangles(); }
  ...
  template <typename mesh_t> uv_flip_mesh_t<mesh_t> uv_flip_mesh(mesh_t mesh) {
