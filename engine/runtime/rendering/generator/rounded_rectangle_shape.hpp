...
namespace generator {
  ...
  private:
    using impl_t = merge_shape_t<
      line_shape_t, translate_shape_t<circle_shape_t>, line_shape_t,
  ...
  public:
    ...
    rounded_rectangle_shape_t(
      double radius = 0.25, const gml::dvec2& size = { 0.75, 0.75 }, int slices = 4,
      const gml::ivec2& segments = { 8, 8 });

    using edges_t = typename impl_t::edges_t;

    edges_t edges() const noexcept { return merge_shape_.edges(); }

    using vertices_t = typename impl_t::vertices_t;

    vertices_t vertices() const noexcept { return merge_shape_.vertices(); }
  };
}  // namespace generator
