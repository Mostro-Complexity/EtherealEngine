...
void model::set_material(asset_handle<material> material, std::uint32_t index) {
  if (index >= mesh_lods_.size()) { mesh_lods_.resize(index + 1); }

  materials_[index] = material;
}

const std::vector<asset_handle<mesh>>& model::get_lods() const { return mesh_lods_; }
...
void model::set_lods(const std::vector<asset_handle<mesh>>& lods) {
  ...
  if (sz1 != sz2) { recalulate_lod_limits(); }
...
const std::vector<asset_handle<material>>& model::get_materials() const { return materials_; }
...
asset_handle<material> model::get_material_for_group(const size_t& group) const {
  if (materials_.size() <= group) { return {}; }

  return materials_[group];
}

void model::set_lod_limits(const std::vector<urange32_t>& limits) { lod_limits_ = limits; }

void model::render(
  gfx::view_id id, const math::transform& world_transform,
  ...
  auto render_subset = [this, &mesh](
    ...
    if (mat) {
      mat->skinned = skinned;
      if (user_program == nullptr) { program = mat->get_program(); }
    }

    if (program != nullptr) {
      valid_program = program->begin();
      if (valid_program) { setup_params(*program); }
    }

    if (valid_program) {
      if (mat) {
        if (user_program == nullptr) { mat->submit(); }
      ...
      if (!matrices.empty()) {
        ...
        for (const auto& m : matrices) { mats.emplace_back(m.get_matrix()); }
    ...
    if (program != nullptr) { program->end(); }
  };
...
void model::recalulate_lod_limits() {
  ...
  for (size_t i = 0; i < mesh_lods_.size(); ++i) {
    ...
    if (mesh_lods_.size() - 1 != i) { lower_limit = upper_limit * (0.5f - ((i) *0.1f)); }
