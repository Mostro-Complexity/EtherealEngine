...
void mesh::dispose() {
  ...
  if (preparation_data_.owns_source) checked_array_delete(preparation_data_.vertex_source);
...
bool mesh::bind_skin(const skin_bind_data& bind_data) {
  ...
  if (prepare_status_ == mesh_status::prepared) prepare_mesh(vertex_format_);

  face_influences used_bones;
  // Build a list of all bone indices and associated weights for each vertex.
  skin_bind_data_.build_vertex_table(
    preparation_data_.vertex_count, preparation_data_.vertex_records, vertex_table);
    ...
    for (; !bone_combinations.empty();) {
      ...
      for (it_combination = bone_combinations.begin(); it_combination != bone_combinations.end();
        ...
        for (size_t i = 0; i < bone_palettes_.size(); ++i) {
          ...
          // all
          palette.compute_palette_fit(
            influences_ptr->bones, remaining_space, common_bones, additional_bones);
      ...
      // source material
      if (it_largest_combination == bone_combinations.end()) break;
  ...
  for (size_t i = 0; i < vertex_table.size(); ++i) {
    ...
    if (i >= original_vertex_count) {
      ...
      memcpy(
        src_vertices_ptr + (i * vertex_stride),
...
bool mesh::prepare_mesh(const gfx::vertex_layout& format) {
  ...
  if (prepare_status_ == mesh_status::prepared) {
    ...
    if (format.m_hash == vertex_format_.m_hash)
      ...
      gfx::vertex_convert(
        format, &preparation_data_.vertex_data[0], vertex_format_, system_vb_, vertex_count_);
    ...
    if (source_has_normals) vertex_flags |= preparation_data::source_contains_normal;
    if (source_has_binormal) vertex_flags |= preparation_data::source_contains_binormal;
    if (source_has_tangent) vertex_flags |= preparation_data::source_contains_tangent;
      ...
      for (const auto& palette : bone_palettes_) {
        ...
        for (std::int32_t face = subset_ptr->face_start;
          ...
          for (size_t i = 0; i < 3; ++i) {
            float weights[4];
            gfx::vertex_unpack(
              weights, gfx::attribute::Weight, vertex_format_, vertex_data_ptr, tri.indices[i]);
            float indices[4];
            gfx::vertex_unpack(
    ...
    for (auto subset : mesh_subsets_) { checked_delete(subset); }
...
bool mesh::prepare_mesh(
  const gfx::vertex_layout& format, void* vertices_ptr, std::uint32_t vertex_count,
...
bool mesh::set_vertex_source(
  void* source_ptr, std::uint32_t vertex_count, const gfx::vertex_layout& source_format) {
  ...
  if (source_ptr == nullptr) return true;
  ...
  if (
    !source_format.has(gfx::attribute::Bitangent) && vertex_format_.has(gfx::attribute::Bitangent))
...
bool mesh::add_primitives(const triangle_array_t& triangles) {
  ...
  for (const auto& src_tri : triangles) {
    ...
    if (has_position) {
      ...
      if (
        (math::equal(v1, v2, math::epsilon<float>()) == math::tvec3<bool>{ true, true, true })
...
static void create_mesh(
  const gfx::vertex_layout& format, const generator::any_mesh& mesh, mesh::preparation_data& data,
  math::bbox& bbox) {
  ...
  for (const auto& v : mesh.vertices()) {
    ...
    if (has_position)
      gfx::vertex_pack(
        &position.x, false, gfx::attribute::Position, format, current_vertex_ptr, std::uint32_t(i));
    if (has_normals)
      gfx::vertex_pack(
        &normal.x, true, gfx::attribute::Normal, format, current_vertex_ptr, std::uint32_t(i));
    if (has_texcoord0)
      gfx::vertex_pack(
        &texcoords0.x,
        true,
...
bool mesh::create_cylinder(
  const gfx::vertex_layout& format, float radius, float height, std::uint32_t stacks,
...
bool mesh::create_sphere(
  const gfx::vertex_layout& format, float radius, std::uint32_t stacks, std::uint32_t slices,
...
bool mesh::create_torus(
  const gfx::vertex_layout& format, float outer_radius, float inner_radius, std::uint32_t bands,
  std::uint32_t sides, mesh_create_origin origin, bool hardware_copy /* = true */) {
  // Clear out old data.
  dispose();
...
bool mesh::create_icosphere(
  const gfx::vertex_layout& format, int tesselation_level, bool hardware_copy /*= true*/) {
  // Clear out old data.
  dispose();
...
bool mesh::create_cone(
  const gfx::vertex_layout& format, float radius, float radius_tip, float height,
...
bool mesh::create_plane(
  const gfx::vertex_layout& format, float width, float height, std::uint32_t width_segments,
  std::uint32_t height_segments, mesh_create_origin origin, bool hardware_copy /* = true */) {
  // Clear out old data.
  dispose();
...
bool mesh::create_cube(
  const gfx::vertex_layout& format, float width, float height, float depth,
  std::uint32_t width_segments, std::uint32_t height_segments, std::uint32_t depth_segments,
  mesh_create_origin origin, bool hardware_copy /* = true */) {
  // Clear out old data.
  dispose();
...
bool mesh::end_prepare(
  bool hardware_copy /* = true */, bool weld /* = true */, bool optimize /* = true */,
  ...
  for (std::uint32_t i = 0; i < preparation_data_.triangle_count; ++i) {
    ...
    if (math::length2(c) < (4.0f * 0.000001f * 0.000001f)) tri.flags |= triangle_flags::degenerate;
  ...
  if (!generate_vertex_components(weld)) return false;
  ...
  if (build_buffers) build_vb(hardware_copy);
  ...
  if (!sort_mesh_data(optimize, hardware_copy, build_buffers)) return false;
    ...
    if (!hardware_ib_) {
      ...
      if (!ib->is_valid()) {
        const gfx::memory_view* mem =
          gfx::copy(system_ib_, static_cast<std::uint32_t>(buffer_size));
        hardware_ib_ = std::make_shared<gfx::index_buffer>(mem, BGFX_BUFFER_INDEX32);
      }
    }
...
bool mesh::sort_mesh_data(bool optimize, bool hardware_copy, bool build_buffer) {
  ...
  for (std::uint32_t i = 0; i < face_count_; ++i) {
    ...
    if (index > sub->vertex_count) sub->vertex_count = index;
  ...
  for (auto subset : new_subsets) {
    ...
    subset->vertex_count =
      (subset->vertex_count - static_cast<std::uint32_t>(subset->vertex_start)) + 1;
  ...
  // Hardware versions of the final buffer were required?
  if (build_buffer) build_ib(hardware_copy);
...
float mesh::find_vertex_optimizer_score(const optimizer_vertex_info* vertex_info_ptr) {
  ...
  if (vertex_info_ptr->unused_triangle_references == 0) return -1.0f;
  ...
  float valence_boost = math::pow(
    static_cast<float>(vertex_info_ptr->unused_triangle_references),
  ...
  for (std::uint32_t i = 0; i < subset->face_count; ++i) {
    ...
    for (std::uint32_t j = 0; j < 3; ++j) {
      ...
      *dest_buffer_ptr++ = index;
      ...
      auto itReference = std::find(
        vert_ptr->triangle_references.begin(), vert_ptr->triangle_references.end(), triangle_index);
      ...
      if (vert_ptr->cache_position == -1) {
        ...
        if (vertex_cache_size > 0) {
          ...
          memmove(
            &vertex_cache_ptr[1], &vertex_cache_ptr[0], vertex_cache_size * sizeof(std::uint32_t));
    ...
    for (std::uint32_t j = 0; j < vertex_cache_size; ++j) {
      ...
      for (std::uint32_t k = 0; k < vert_ptr->unused_triangle_references; ++k) {
        ...
        score +=
          vertex_info_ptr[src_buffer_ptr[(triangle_index * 3) + 1] - min_vertex].vertex_score;
        score +=
          vertex_info_ptr[src_buffer_ptr[(triangle_index * 3) + 2] - min_vertex].vertex_score;
...
void mesh::bind_render_buffers_for_subset(std::uint32_t data_group_id) {
  ...
  if (it == subset_lookup_.end()) return;
  ...
  if (face_count > 0) bind_mesh_data(face_start, face_count, vertex_start, vertex_count);
}

void mesh::bind_mesh_data(
  std::uint32_t face_start, std::uint32_t face_count, std::uint32_t vertex_start,
...
bool mesh::generate_vertex_components(bool weld) {
  ...
  // provided?)
  if (
    force_tangent_generation_ || preparation_data_.compute_binormals
    || preparation_data_.compute_tangents) {
    // Requires normals
...
bool mesh::generate_vertex_normals(
  ...
  if (remap_array_ptr) {
    ...
    for (i = 0; i < preparation_data_.vertex_count; ++i) (*remap_array_ptr)[i] = i;
  ...
  for (i = 0; i < preparation_data_.triangle_count; ++i) {
    triangle& tri = preparation_data_.triangle_data[i];
    if (tri.flags & triangle_flags::degenerate) continue;
    ...
    for (j = 0; j < 3; ++j) {
      ...
      for (;;) {
        ...
        if (current_tri == start_tri || current_tri == 0xFFFFFFFF) break;

        // Find the edge in the adjacency list that we came in through
        for (k = 0; k < 3; ++k) {
          if (adjacency_ptr[(current_tri * 3) + k] == previous_tri) break;
      ...
      if (ref_normal.x == 0.0f && ref_normal.y == 0.0f && ref_normal.z == 0.0f) {
        ...
        if (
          math::abs(ref_normal.x - vec_normal.x) >= 1e-3f
          || math::abs(ref_normal.y - vec_normal.y) >= 1e-3f
          || math::abs(ref_normal.z - vec_normal.z) >= 1e-3f) {
          // Make room for new vertex data.
          preparation_data_.vertex_data.resize(
            preparation_data_.vertex_data.size() + vertex_stride);
          ...
          memcpy(
            src_vertices_ptr + (preparation_data_.vertex_count * vertex_stride),
          ...
          // Record the split
          if (remap_array_ptr) (*remap_array_ptr)[index] = preparation_data_.vertex_count;
...
bool mesh::generate_vertex_tangents() {
  ...
  if (!force_tangent_generation_ && !requires_bitangents && !requires_tangents) return true;
  ...
  for (i = 0; i < num_faces; ++i) {
    ...
    // are not invalid.
    float r = (s1 * t2 - s2 * t1);
    if (math::abs(r) < math::epsilon<float>()) continue;
    r = 1.0f / r;
  ...
  for (i = 0; i < num_verts; i++, src_vertices_ptr += vertex_stride) {
    ...
    if (!force_tangent_generation_ && has_bitangent && has_tangent) continue;
    ...
    if (force_tangent_generation_ || (!has_tangent && requires_tangents))
      gfx::vertex_pack(
        &math::vec4(T, 1.0f)[0], true, gfx::attribute::Tangent, vertex_format_, src_vertices_ptr);
    ...
    if (force_tangent_generation_ || (!has_bitangent && requires_bitangents)) {
      ...
      // Store.
      gfx::vertex_pack(
        &math::vec4(B, 1.0f)[0], true, gfx::attribute::Bitangent, vertex_format_, src_vertices_ptr);
...
bool mesh::generate_adjacency(std::vector<std::uint32_t>& adjacency) {
  ...
  if (prepare_status_ != mesh_status::prepared) {
    // Validate requirements
    if (preparation_data_.triangle_count == 0) return false;
    ...
    for (std::uint32_t i = 0; i < preparation_data_.triangle_count; ++i) {
      ...
      if (it_edge != edge_tree.end()) adjacency[(i * 3)] = it_edge->second;
      ...
      // Find the matching adjacent edge
      it_edge = edge_tree.find(edge);
      if (it_edge != edge_tree.end()) adjacency[(i * 3) + 1] = it_edge->second;
      ...
      // Find the matching adjacent edge
      it_edge = edge_tree.find(edge);
      if (it_edge != edge_tree.end()) adjacency[(i * 3) + 2] = it_edge->second;
...
bool mesh::weld_vertices(
  ...
  for (std::uint32_t i = 0; i < preparation_data_.vertex_count; ++i) {
    ...
    if (it_key == vertex_tree.end()) {
      ...
      if (vertex_remap_ptr) (*vertex_remap_ptr)[i] = new_vertex_count;
      ...
      if (vertex_remap_ptr) (*vertex_remap_ptr)[i] = 0xFFFFFFFF;
  ...
  if (preparation_data_.vertex_count == new_vertex_count) {
    ...
    if (vertex_remap_ptr) vertex_remap_ptr->clear();
    return true;
...
std::uint8_t* mesh::get_system_vb() { return system_vb_; }

std::uint32_t* mesh::get_system_ib() { return system_ib_; }

const gfx::vertex_layout& mesh::get_vertex_format() const { return vertex_format_; }

const mesh::subset* mesh::get_subset(std::uint32_t data_group_id /* = 0 */) const {
  auto it = subset_lookup_.find(mesh_subset_key(data_group_id));
  if (it == subset_lookup_.end()) return nullptr;
  return it->second;
}

const skin_bind_data& mesh::get_skin_bind_data() const { return skin_bind_data_; }

const mesh::bone_palette_array_t& mesh::get_bone_palettes() const { return bone_palettes_; }

const std::unique_ptr<mesh::armature_node>& mesh::get_armature() const { return root_; }
...
///////////////////////////////////////////////////////////////////////////////
void skin_bind_data::add_bone(const bone_influence& bone) { bones_.push_back(bone); }
...
void skin_bind_data::clear_vertex_influences() {
  for (size_t i = 0; i < bones_.size(); ++i) { bones_[i].influences.clear(); }
}

void skin_bind_data::clear() { bones_.clear(); }
...
void skin_bind_data::build_vertex_table(
  std::uint32_t vertex_count, const std::vector<std::uint32_t>& vertex_remap,
...
std::vector<skin_bind_data::bone_influence>& skin_bind_data::get_bones() { return bones_; }

bool skin_bind_data::has_bones() const { return !get_bones().empty(); }

const skin_bind_data::bone_influence*
skin_bind_data::find_bone_by_id(const std::string& name) const {
  auto it = std::find_if(std::begin(bones_), std::end(bones_), [name](const auto& bone) {
    return name == bone.bone_id;
  });
  if (it != std::end(bones_)) { return &(*it); }
...
bone_palette::bone_palette(std::uint32_t palette_size)
  : data_group_id_(0), maximum_size_(palette_size), maximum_blend_index_(-1) {}
...
bone_palette::bone_palette(const bone_palette& rhs)
  : bones_lut_(rhs.bones_lut_), bones_(rhs.bones_), faces_(rhs.faces_),
    data_group_id_(rhs.data_group_id_), maximum_size_(rhs.maximum_size_),
...
std::vector<math::transform> bone_palette::get_skinning_matrices(
  const std::vector<math::transform>& node_transforms, const skin_bind_data& bind_data,
...
void bone_palette::compute_palette_fit(
  bone_index_map_t& input, std::int32_t& current_space, std::int32_t& common_bones,
  std::int32_t& additional_bones) {
  // Reset values
  current_space =
    static_cast<std::int32_t>(maximum_size_ - static_cast<std::uint32_t>(bones_.size()));
  common_bones = 0;
  additional_bones = 0;
...
std::uint32_t bone_palette::translate_bone_to_palette(std::uint32_t bone_index) const {
  ...
  if (it_bone == bones_lut_.end()) return 0xFFFFFFFF;
  return it_bone->second;
}

std::uint32_t bone_palette::get_data_group() const { return data_group_id_; }

void bone_palette::set_data_group(std::uint32_t group) { data_group_id_ = group; }

std::int32_t bone_palette::get_maximum_blend_index() const { return maximum_blend_index_; }

void bone_palette::set_maximum_blend_index(int nIndex) { maximum_blend_index_ = nIndex; }

std::uint32_t bone_palette::get_maximum_size() const { return maximum_size_; }

std::vector<std::uint32_t>& bone_palette::get_influenced_faces() { return faces_; }

void bone_palette::clear_influenced_faces() { faces_.clear(); }

const std::vector<std::uint32_t>& bone_palette::get_bones() const { return bones_; }
...
bool operator<(const mesh::bone_combination_key& key1, const mesh::bone_combination_key& key2) {
  ...
  if (key1.data_group_id != key2.data_group_id) return key1.data_group_id < key2.data_group_id;

  const mesh::face_influences* p1 = key1.influences;
  const mesh::face_influences* p2 = key2.influences;

  // The bone count must match.
  if (p1->bones.size() != p2->bones.size()) return p1->bones.size() < p2->bones.size();
  ...
  for (; it_bone1 != p1->bones.end() && it_bone2 != p2->bones.end(); ++it_bone1, ++it_bone2) {
    if (it_bone1->first != it_bone2->first) return it_bone1->first < it_bone2->first;
