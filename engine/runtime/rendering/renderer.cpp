...
namespace runtime {
  ...
  render_window* renderer::get_focused_window() const {
    ...
    if (it != std::end(windows)) { focused_window = it->get(); }
  ...
  const std::unique_ptr<render_window>& renderer::get_window(uint32_t id) const {
    auto it = std::find_if(std::begin(windows_), std::end(windows_), [id](const auto& window) {
  ...
  void renderer::hide_all_secondary_windows() {
    ...
    for (auto& window : windows_) {
      if (i++ != 0) { window->set_visible(false); }
    }
  }
  ...
  void renderer::show_all_secondary_windows() {
    std::size_t i = 0;
    for (auto& window : windows_) {
      if (i++ != 0) { window->set_visible(true); }
    }
  }

  void renderer::process_pending_windows() {
    std::move(
      std::begin(windows_pending_addition_),
      std::end(windows_pending_addition_),
      std::back_inserter(windows_));
    windows_pending_addition_.clear();
  }

  void renderer::platform_events(
  ...
  bool renderer::init_backend(cmd_line::parser& parser) {
    ...
    if (novsync) { init_data.resolution.reset = 0; }
