...
namespace nonstd {
  ...
  template <typename T, std::size_t N> struct policy_std_array {
    ...
    static buffer_t create_with_size(size_type /*n*/) { return {}; }
  };

  template <typename T> struct policy_std_vector {
    using buffer_t = typename std::vector<T>;
    using size_type = typename buffer_t::size_type;
    using value_type = typename buffer_t::value_type;

    static buffer_t create_with_size(size_type n) { return buffer_t(n); }
  };

  template <typename policy_t> class ring_buffer : public ring_span<typename policy_t::value_type> {
  public:
    ...
    ring_buffer(ring_buffer&& rhs) noexcept(
      ...
      : ring_buffer(
        &*std::begin(rhs) - &*std::begin(rhs.container_), rhs.size(), std::move(rhs.container_)) {}
    ...
    ring_buffer& operator=(const ring_buffer& rhs) noexcept(
      std::is_nothrow_copy_assignable<container_type>::value) {
      const auto first_idx = (&*std::begin(rhs) - &*std::begin(rhs.container_));
      container_ = rhs.container_;
      static_cast<view_type&>(*this) = view_type(
        std::begin(container_),
        std::end(container_),
        std::begin(container_) + first_idx,
        rhs.size());
      return *this;
    }

    ring_buffer&
    operator=(ring_buffer&& rhs) noexcept(std::is_nothrow_copy_assignable<container_type>::value) {
      const auto first_idx = (&*std::begin(rhs) - &*std::begin(rhs.container_));
  ...
  private:
    void reset(size_type first_idx, size_type size) {
      static_cast<view_type&>(*this) = view_type(
        std::begin(container_), std::end(container_), std::begin(container_) + first_idx, size);
    }

    template <class... Args>
    ring_buffer(size_type first_idx, size_type size, Args&&... args) noexcept(
      noexcept(container_type(std::forward<Args>(args)...)))
  ...
  template <typename T, size_t N> using stack_ringbuffer = ring_buffer<policy_std_array<T, N>>;

  template <typename T> using heap_ringbuffer = ring_buffer<policy_std_vector<T>>;
