...
namespace nonstd {
  ...
  public:
    ...
    any(const any& rhs) : vtable(rhs.vtable) {
      if (!rhs.empty()) { rhs.vtable->copy(rhs.storage, this->storage); }
    }
    ...
    template <
      typename ValueType, typename = typename std::enable_if<
  ...
  private:  // Storage and Virtual Method Table
    ...
    template <typename T> struct vtable_dynamic {
      static const rtti::type_index_t& type() noexcept { return rtti::type_id<T>(); }
  ...
  protected:
    ...
    bool is_typed(const rtti::type_index_t& t) const { return is_same(this->type(), t); }
    ...
    static bool is_same(const rtti::type_index_t& a, const rtti::type_index_t& b) { return a == b; }
...
namespace std {
  inline void swap(nonstd::any& lhs, nonstd::any& rhs) noexcept { lhs.swap(rhs); }
}  // namespace std
#endif
