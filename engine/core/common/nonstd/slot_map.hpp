...
namespace nonstd {
  ...
  template <
    class T, class Key = std::pair<unsigned, unsigned>,
  ...
  class slot_map {
    ...
    template <class Integral> static void set_index(Key& k, Integral value) {
      auto& [idx, gen] = k;
      idx = value;
    }
  ...
  public:
    ...
    static_assert(
      std::is_same<value_type, mapped_type>::value,
    ...
    reference at(const key_type& key) {
      ...
      if (value_iter == this->end()) { SLOT_MAP_THROW_EXCEPTION(std::out_of_range, "at"); }
      return *value_iter;
    }
    ...
    // O(1) time and space complexity.
    //
    reference       operator[](const key_type& key) { return *find_unchecked(key); }
    const_reference operator[](const key_type& key) const { return *find_unchecked(key); }
    ...
    iterator find(const key_type& key) {
      ...
      if (slot_index >= slots_.size()) { return end(); }
      auto slot_iter = std::next(slots_.begin(), get_index(key));
      if (get_generation(*slot_iter) != get_generation(key)) { return end(); }
    ...
    size_type capacity() const { return values_.capacity(); }
    ...
    size_type capacity_slots() const { return slots_.capacity(); }
    ...
    //
    key_type insert(const mapped_type& value) { return this->emplace(value); }
    key_type insert(mapped_type&& value) { return this->emplace(std::move(value)); }

    template <typename... Args> key_type emplace(Args&&... args) {
    ...
    iterator erase(const_iterator first, const_iterator last) {
      // Must use indexes, not iterators, because Container iterators might be invalidated by
      // pop_back
    ...
    //
    void clear() {
      // This resets the generation counters, which "undefined-behavior-izes" at() and find() for
      // the old keys.
      slots_.clear();
      values_.clear();
      reverse_map_.clear();
  ...
  protected:
    ...
    Container<mapped_type>&        c() & noexcept { return values_; }
    const Container<mapped_type>&  c() const& noexcept { return values_; }
    Container<mapped_type>&&       c() && noexcept { return std::move(values_); }
    const Container<mapped_type>&& c() const&& noexcept { return std::move(values_); }
  ...
  private:
    ...
    iterator erase_slot_iter(slot_iterator slot_iter) {
      ...
      if (value_iter != value_back_iter) {
        ...
        *reverse_map_iter =
          static_cast<key_size_type>(std::distance(slots_.begin(), slot_back_iter));
      }
      values_.pop_back();
