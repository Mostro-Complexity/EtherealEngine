...
template <typename T> struct range {
  ...
  template <
    typename U, typename = typename std::enable_if<std::is_convertible<U, T>::value, void>::type>
  explicit range(const std::array<U, 2>& data) : min(data[0]), max(data[1]) {}
  T min = 0;
  T max = 0;

  inline bool contains(const T& val) const { return min <= val && val <= max; }

  inline bool operator==(const range& b) const { return (min == b.Min && max == b.Max); }
  inline bool operator!=(const range& b) const { return (min != b.Min || max != b.Max); }
...
template <typename T> struct size {
  ...
  template <
    typename U, typename = typename std::enable_if<std::is_convertible<U, T>::value, void>::type>
  explicit size(const std::array<U, 2>& data) : width(data[0]), height(data[1]) {}
  T width = 0;
  T height = 0;

  // Inline operators
  inline bool operator==(const size& b) const { return (width == b.width && height == b.height); }
  inline bool operator!=(const size& b) const { return (width != b.width || height != b.height); }
...
template <typename T> struct point {
  ...
  template <
    typename U, typename = typename std::enable_if<std::is_convertible<U, T>::value, void>::type>
  explicit point(const std::array<U, 2>& data) : x(data[0]), y(data[1]) {}
  T x = 0;
  T y = 0;
...
template <typename T> struct rect {
  ...
  template <typename T1 = T> inline point<T1> center() const {
    return { left + width() / 2, top + height() / 2 };
  }

  template <typename T1 = T> inline point<T1> tl() const { return { left, top }; }
  template <typename T1 = T> inline point<T1> tr() const { return { right, top }; }

  template <typename T1 = T> inline point<T1> bl() const { return { left, bottom }; }
  template <typename T1 = T> inline point<T1> br() const { return { right, bottom }; }
  ...
  inline static rect intersect(const rect& a, const rect& b) {
    ...
    if (c.left > c.right || c.top > c.bottom) { return rect(); }
