...
namespace uuids {
  ...
  template <typename random_generator_t = ::std::mt19937> class random_uuid_generator {
    ...
    typename engine_type::result_type gen() { return distribution_(generator_); }

  public:
    explicit random_uuid_generator(result_type seed = engine_type::default_seed)
      : generator_(seed),
        distribution_(
          std::numeric_limits<result_type>::min(), std::numeric_limits<result_type>::max()) {}
