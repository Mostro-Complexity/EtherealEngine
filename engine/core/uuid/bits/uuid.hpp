...
namespace uuids {
  namespace detail {
    template <typename T> struct has_overloaded_addressof {
      template <typename U> static constexpr bool has_overload(...) { return false; }
  ...
  struct uuid {
    ...
    constexpr uuid(char const* /*unused*/, size_type /*unused*/) : uuid() {
      // std::isxdigit(ch)..
    }
    ...
    template <typename InIter> uuid(InIter first, InIter last) { ::std::copy(first, last, data_); }
    ...
    constexpr const_iterator begin() const noexcept { return detail::static_addressof(data_[0]); }

    constexpr const_iterator end() const noexcept {
      return detail::static_addressof(data_[static_size - 1]);
    }

    constexpr const_iterator cbegin() const noexcept { return detail::static_addressof(data_[0]); }
    ...
    operator<<(::std::basic_ostream<CharT, Traits>& os, uuid const& u) {
      ...
      if (ok) {
        for (std::size_t i = 0; i != u.size(); ++i) {
          ...
          if (i == 3 || i == 5 || i == 7 || i == 9) { os << os.widen('-'); }
        }
      }
  ...
  inline bool operator!=(uuid const& lhs, uuid const& rhs) { return !(lhs == rhs); }

  inline bool operator<=(uuid const& lhs, uuid const& rhs) { return !(rhs < lhs); }
