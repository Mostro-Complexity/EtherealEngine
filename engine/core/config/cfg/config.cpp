...
namespace cfg {
  ...
  bool config::has_value(const std::string& section, const std::string& name) const {
    ...
    if (si != values_.end()) {
      ...
      if (vi != si->second.end()) { return true; }
    }
    return false;
  }
  ...
  const std::string& config::get(const std::string& section, const std::string& name) const {
    ...
    if (si != values_.end()) {
      auto vi = si->second.find(name);
      if (vi != si->second.end()) { return vi->second; }
    }
  ...
  const std::string& config::get_value(
    const std::string& section, const std::string& name, const std::string& defaultValue) {
    if (!has_value(section, name)) { return defaultValue; }

    return get(section, name);
  }

  void config::set(const std::string& section, const std::string& name, const std::string& value) {
    if (!isalnum(section)) { throw std::invalid_argument("Section is not an identifier."); }
    if (!isalnum(name)) { throw std::invalid_argument("Name is not an identifier."); }
  ...
  void config::save(const std::string& file) {
    ...
    if (out.bad()) { throw std::runtime_error("Failed to open " + file + " for writing."); }
  ...
  void config::load(const std::string& file) {
    ...
    if (in.bad()) { throw std::runtime_error("Failed to open " + file + " for reading."); }
