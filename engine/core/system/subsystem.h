...
namespace core {
  ...
  namespace details {
    enum class internal_status : uint8_t { idle, running, disposed };
  ...
  template <typename S, typename... Args> S& subsystem_context::add_subsystem(Args&&... args) {
  ...
  template <typename S> S& subsystem_context::get_subsystem() {
  ...
  template <typename S> void subsystem_context::remove_subsystem() {
    expects(has_subsystems<S>() && "failed to find system");
    const auto index = rtti::type_id<S>().hash_code();
    _subsystems.erase(index);
    _orders.erase(std::remove_if(
      std::begin(_orders),
      std::end(_orders),
      [index](const auto& el) { return index == el; },
      std::end(_orders)));
  }

  template <typename S> bool subsystem_context::has_subsystems() const {
    const auto index = rtti::type_id<S>().hash_code();
  ...
  template <typename S> S& get_subsystem() {
    expects(
      details::status() == details::internal_status::running
      && "details::context must be initialized");
