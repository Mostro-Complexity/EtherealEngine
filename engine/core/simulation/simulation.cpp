...
namespace core {
  ...
  simulation::simulation() {
    if (max_inactive_fps_ == 0) { max_inactive_fps_ = std::max(max_inactive_fps_, max_fps_); }
  }

  void simulation::run_one_frame(bool is_active) {
    // perform waiting loop if maximum fps set
    auto max_fps = max_fps_;
    if (!is_active && max_fps > 0) { max_fps = std::min(max_inactive_fps_, max_fps); }
    ...
    if (smoothing_step_ > 0) {
      ...
      if (previous_timesteps_.size() > smoothing_step_) {
        ...
        for (auto step : previous_timesteps_) { timestep_ += step; }
  ...
  std::uint32_t simulation::get_fps() const {
    auto dt =
      std::chrono::duration_cast<std::chrono::duration<float, std::milli>>(timestep_).count();
