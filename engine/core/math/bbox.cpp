...
namespace math {
  ...
  bool bbox::is_degenerate() const {
    return (
      glm::abs<float>(max.x - min.x) < glm::epsilon<float>()
  ...
  //-----------------------------------------------------------------------------
  bool bbox::intersect(
    const vec3& origin, const vec3& velocity, float& t, bool restrict_range /* = true */) const {
    ...
    if (t < 0.0f || (restrict_range && t > 1.0f)) { return false; }
  ...
  //-----------------------------------------------------------------------------
  bool bbox::intersect(
    const vec3& v_tri0, const vec3& v_tri1, const vec3& v_tri2, const bbox& tri_bounds) const {
    ...
    if (
      tri_bounds.min.x > max.x || tri_bounds.max.x < min.x || tri_bounds.min.y > max.y
    ...
    // plane
    if (glm::dot(vNormal, vNearPoint - vCenter) + fPlaneDistance > 0.0f) { return false; }
...
#define AXISTEST(vEdge, vP0, vP1, nComponent0, nComponent1)                                        \
  ...
  fTemp0 =                                                                                         \
    vAbsEdge[nComponent1] * vExtents[nComponent0] + vAbsEdge[nComponent0] * vExtents[nComponent1]; \
...
#define AXISTEST2(vEdge, vP0, vP1, nComponent0, nComponent1)                                       \
  ...
  /// </summary>
  //-----------------------------------------------------------------------------
  bbox bbox::operator*(float scale) const { return bbox(min * scale, max * scale); }
