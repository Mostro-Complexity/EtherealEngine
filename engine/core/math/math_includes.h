...
namespace math {
  ...
  log_space(std::size_t start, std::size_t end, std::size_t count) {
  ...
  inline bool compute_projected_sphere_shaft(
    float light_x, float light_z, float radius, const transform& proj, const vec3& axis,
    float axis_sign, std::int32_t& in_out_min_x, std::int32_t& in_out_max_x) {
    ...
    if (discriminant >= 0) {
      ...
      if (Pza > 0) {
        ...
        if (is_negative_float(Nxa) ^ is_negative_float(axis_sign)) {
          ...
          in_out_min_x = glm::max<std::int32_t>(
            std::int32_t(glm::floor(view_size_x * X + view_x)), in_out_min_x);
        }
      }
  ...
  inline std::uint32_t compute_projected_sphere_rect(
    std::int32_t& left, std::int32_t& right, std::int32_t& top, std::int32_t& bottom,
    const vec3& sphere_center, float radius, const transform& view, const transform& proj) {
    ...
    if (math::length2(sphere_center - view_origin) > math::square(radius)) {
      ...
      if (!compute_projected_sphere_shaft(
            lv.x, lv.z, radius, proj, vec3(1.0f, 0.0f, 0.0f), 1.0f, left, right)) {
        return 0;
      }

      if (!compute_projected_sphere_shaft(
            lv.y, lv.z, radius, proj, vec3(0.0f, 1.0f, 0.0f), -1.0f, top, bottom)) {
        return 0;
      }
