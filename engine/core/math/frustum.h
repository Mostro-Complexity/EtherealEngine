...
namespace math {
  ...
  public:
    ...
    volume_query
    classify_aabb(const bbox& bounds, unsigned int& frustumBits, int& lastOutside) const;
    ...
    static volume_query classify_obb(
      frustum f, const bbox& bounds, const transform& t, unsigned int& frustumBits,
      int& lastOutside);
    //-------------------------------------------------------------------------
    // Public Operators
    //-------------------------------------------------------------------------
  ...
  private:
    ...
    //-------------------------------------------------------------------------
    static bool swept_sphere_intersect_plane(
      float& t0, float& t1, const plane& plane, const vec3& center, float radius,
