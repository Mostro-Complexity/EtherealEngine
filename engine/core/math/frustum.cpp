...
namespace math {
  ...
  void frustum::update(const transform& view, const transform& proj, bool _oglNDC) {
    ...
    for (auto& plane : planes) { plane.data *= -1.0f; }
  ...
  void frustum::set_planes(const std::array<plane, 6>& new_planes) {
    ...
    for (auto& plane : planes) { plane = plane::normalize(plane); }
  ...
  void frustum::recompute_points() {
    ...
    for (std::size_t i = 0; i < 8; ++i) {
      const plane& p0 = plane::normalize(
        (i & 1) != 0u ? planes[volume_plane::near_plane] : planes[volume_plane::far_plane]);
      const plane& p1 =
  ...
  volume_query frustum::classify_aabb(const bbox& AABB) const {
    ...
    for (const auto& plane : planes) {
      ...
      // frustum
      if (plane::dot_coord(plane, NearPoint) > 0.0f) { return volume_query::outside; }

      // If far extreme point is outside, then the AABB is intersecting the
      // frustum
      if (plane::dot_coord(plane, FarPoint) > 0.0f) { Result = volume_query::intersect; }
  ...
  /// </summary>
  //-----------------------------------------------------------------------------
  volume_query frustum::classify_obb(
    frustum frustum, const bbox& AABB, const transform& t, unsigned int& FrustumBits,
    int& LastOutside) {
    transform invTransform = inverse(t);
  ...
  bool frustum::test_aabb(const bbox& AABB) const {
    ...
    for (const auto& plane : planes) {
      ...
      // If near extreme point is outside, then the AABB is totally outside the
      // frustum
      if (plane::dot_coord(plane, NearPoint) > 0.0f) { return false; }
  ...
  bool frustum::test_extruded_aabb(const bbox_extruded& Box) const {
    ...
    if (!test_sphere(Box.projection_point, Box.projection_range)) { return false; }
    ...
    bIntersect1 =
      ...
      || (Box.test_line(
        points[volume_geometry_point::left_top_near], points[volume_geometry_point::left_top_far]))
      || (Box.test_line(
        points[volume_geometry_point::left_top_far], points[volume_geometry_point::right_top_near]))
      || (Box.test_line(
        points[volume_geometry_point::right_top_far],
        points[volume_geometry_point::right_top_near]))
      || (Box.test_line(
        points[volume_geometry_point::right_top_near],
        points[volume_geometry_point::left_top_near]));
  ...
  volume_query frustum::classify_sphere(const vec3& vecCenter, float fRadius) const {
    ...
    for (const auto& plane : planes) {
      ...
      if (fDot >= fRadius) { return volume_query::outside; }

      // Sphere spans plane
      if (fDot >= -fRadius) { Result = volume_query::intersect; }
  ...
  /// </summary>
  //-----------------------------------------------------------------------------
  bool frustum::swept_sphere_intersect_plane(
    float& t0, float& t1, const plane& plane, const vec3& vecCenter, float fRadius,
  ...
  bool frustum::test_swept_sphere(
    ...
    for (const auto& plane : planes) {
      ...
      if (swept_sphere_intersect_plane(t0, t1, plane, vecCenter, fRadius, vecSweepDirection)) {
        ...
        if (t0 >= 0.0f) { pDisplacements[nCount++] = t0; }
        if (t1 >= 0.0f) { pDisplacements[nCount++] = t1; }
    ...
    for (i = 0; i < nCount; ++i) {
      ...
      if (test_sphere(vDisplacedCenter, fDisplacedRadius)) { return true; }
  ...
  /// </summary>
  //-----------------------------------------------------------------------------
  bool frustum::test_point(const vec3& vecPoint) const { return test_sphere(vecPoint, 0.0f); }
  ...
  bool frustum::test_line(const vec3& v1, const vec3& v2) const {
    ...
    for (i = 0; i < 6; ++i) {
      ...
      if (nSide1 == 1 && nSide2 == 1) { return false; }
      ...
      if ((nSide1 ^ nSide2) != 0) {
        ...
        if ((t >= 0.0f) && (t <= 1.0f)) {
          ...
          if (test_sphere(vIntersect, 0.01f)) { return true; }
  ...
  volume_query frustum::classify_plane(const plane& plane) const {
    ...
    if (nInFrontCount == 8) { return volume_query::outside; }

    // frustum entire behind plane
    if (nBehindCount == 8) { return volume_query::inside; }
  ...
  bool frustum::test_frustum(const frustum& f) const {
    ...
    bIntersect2 =
      ...
      || f.test_line(
        ...
        points[volume_geometry_point::right_top_near])
      || f.test_line(
        points[volume_geometry_point::left_bottom_far], points[volume_geometry_point::left_top_far])
      || f.test_line(
        points[volume_geometry_point::left_bottom_near],
        points[volume_geometry_point::left_top_near])
      || f.test_line(
      ...
      || f.test_line(
        points[volume_geometry_point::left_top_far], points[volume_geometry_point::right_top_near])
      || f.test_line(
        points[volume_geometry_point::right_top_far], points[volume_geometry_point::right_top_near])
      || f.test_line(
        points[volume_geometry_point::right_top_near],
        points[volume_geometry_point::left_top_near]);
  ...
  frustum& frustum::mul(const transform& t) {
    ...
    // transform points
    for (auto& point : points) { point = transform::transform_coord(point, mtx); }
  ...
  /// </summary>
  //-----------------------------------------------------------------------------
  frustum frustum::mul(frustum f, const transform& t) { return f.mul(t); }
