...
namespace core {
  ...
  template <typename T> class task_future {
  public:
    ...
    template <typename F> decltype(auto) then_on_worker(F&& f);
    template <typename F> decltype(auto) then_on_owner(F&& f);
  ...
  template <typename T> using is_future = async::detail::is_future<T>;

  class task {
    template <typename T> using decay_future_t = async::detail::decay_future_t<T>;

    template <class T>
    using decay_if_future_t =
      typename std::conditional<is_future<T>::value, decay_future_t<T>, T>::type;
  ...
  public:
    ...
    template <class F, class... Args> static decltype(auto) make_ready_task(F&& f, Args&&... args) {
  ...
  private:
    ...
    task(awaitable_task_tag /*unused*/, F&& f, Args&&... args) noexcept
      : t_(
        new awaitable_task_model<invoke_result_t<F, Args...>(decay_if_future_t<Args>...), Args...>(
          std::forward<F>(f), std::forward<Args>(args)...)) {}
    ...
    /// </summary>
    //-----------------------------------------------------------------------------
    template <class R, class... Args> struct ready_task_model<R(Args...)> : task_concept {
      template <class F>
      ...
      void invoke_() override { nonstd::apply(f_, args_); }
    ...
    struct awaitable_task_model<R(CallArgs...), FutArgs...> : task_concept {
      ...
      void invoke_() override {
        ...
        } catch (const std::future_error& e) { (void) e; }
      }
    ...
    private:
      ...
      template <std::size_t... I> inline void do_invoke_(std::index_sequence<I...> /*unused*/) {
        try {
  ...
  class task_system {
    ...
    template <typename T> friend class task_future;
  ...
  public:
    ...
    std::size_t get_most_busy_queue_idx(bool skip_owner) const {
      if (threads_count_ == 1) { return get_owner_thread_idx(); }
      ...
      const std::size_t idx =
        static_cast<std::size_t>(std::distance(std::begin(info.queue_infos), min_el));
      return idx;
    }
    ...
    decltype(auto) push_on_thread(const std::size_t idx, F&& f, Args&&... args) {
      ...
      return push_impl(
        is_ready_task(), idx, false, std::forward<F>(f), std::forward<Args>(args)...);
    }
    ...
    /// </summary>
    //-----------------------------------------------------------------------------
    template <class F, class... Args> decltype(auto) push_on_worker_thread(F&& f, Args&&... args) {
    ...
    /// </summary>
    //-----------------------------------------------------------------------------
    template <class F, class... Args> decltype(auto) push_on_owner_thread(F&& f, Args&&... args) {
  ...
  private:
    ...
    //-----------------------------------------------------------------------------
    template <class F, class... Args>
    decltype(auto) push_impl(
      std::true_type /*unused*/, std::size_t idx, bool execute_if_ready, F&& f, Args&&... args) {
      return push_task(
        task::make_ready_task(std::forward<F>(f), std::forward<Args>(args)...),
        idx,
        execute_if_ready);
    }
    ...
    template <class F, class... Args>
    decltype(auto) push_impl(
      std::false_type /*unused*/, std::size_t idx, bool execute_if_ready, F&& f, Args&&... args) {
      return push_task(
        task::make_awaitable_task(std::forward<F>(f), std::forward<Args>(args)...),
        idx,
        execute_if_ready);
    }
    ...
    auto push_task(T&& t, std::size_t idx, bool execute_if_ready) ->
      ...
      if (
        execute_if_ready && t.first.ready()
    ...
    /// </summary>
    //-----------------------------------------------------------------------------
    template <typename T> bool processing_wait(const task_future<T>& t) {
      ...
      if (queue_index == invalid_index) { return false; }
    ...
    //-----------------------------------------------------------------------------
    void run(
      std::size_t idx, const std::function<bool()>& condition,
  ...
  template <typename T> inline void task_future<T>::wait() const {
    if (!future_.valid()) { return; }

    if (!is_ready()) {
      if (executor_) {
        if (!executor_->processing_wait(*this)) { future_.wait(); }
      } else {
        future_.wait();
      }
    }
  ...
  template <typename T> inline void task_future<T>::cancel() const {
    if (!future_.valid()) { return; }
