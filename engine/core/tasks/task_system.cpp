...
namespace core {
  ...
  void task_system::task_queue::sort() {
    if (tasks_.size() > 1) {
      std::stable_partition(
        tasks_.begin(), tasks_.end(), [](const auto& task1) { return task1.ready(); });
    }
  }
  ...
  bool task_system::task_queue::is_done() const { return done_.load(); }

  std::pair<bool, task> task_system::task_queue::try_pop() {
    std::unique_lock<std::mutex> lock(mutex_, std::try_to_lock);

    if (!lock || tasks_.empty()) { return std::make_pair(false, task{}); }
  ...
  std::pair<bool, task> task_system::task_queue::pop(duration_t pop_timeout) {
    ...
    if (tasks_.empty()) { return std::make_pair(false, task{}); }
  ...
  void task_system::task_queue::wake_up() { cv_.notify_all(); }
  ...
  void task_system::run(
    ...
    while (condition()) {
      ...
      if (!p.first) { p = queues_[queue_index].pop(pop_timeout); }
  ...
  std::size_t task_system::get_thread_queue_idx(std::size_t idx, std::size_t seed) {
    ...
    if (idx == get_owner_thread_idx()) { return get_owner_thread_idx(); }

    auto queue_index = ((idx + seed) % threads_count_);

    if (queue_index == get_owner_thread_idx()) { queue_index++; }
  ...
  task_system::task_system(bool wait_on_destruct, std::size_t nthreads)
    : threads_count_{ nthreads }, wait_on_destruct_(wait_on_destruct) {
    queues_.reserve(threads_count_);
    queues_.emplace_back();
    for (std::size_t th = 1; th < threads_count_; ++th) { queues_.emplace_back(); }
    ...
    for (std::size_t th = 1; th < threads_count_; ++th) {
      threads_.emplace_back(
        &task_system::run, this, th, []() { return true; }, 50ms);
