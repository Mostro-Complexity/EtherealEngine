...
namespace audio {
  ...
  bool sound::is_valid() const { return impl_ && impl_->is_valid(); }

  const sound_info& sound::get_info() const { return info_; }

  bool sound::load_buffer() { return impl_ && impl_->load_buffer(); }

  uintptr_t sound::uid() const { return reinterpret_cast<uintptr_t>(impl_.get()); }
