...
namespace audio {
  namespace utils {
    ...
    convert_to_mono(const std::vector<std::uint8_t>& input, std::uint8_t bytes_per_sample) {
      ...
      if (bytes_per_sample == 1) {
        ...
        for (std::size_t i = 0, sz = input.size(); i < sz; i += 2 * bytes_per_sample) {
          ...
          const std::int16_t right =
            *reinterpret_cast<const std::int16_t*>(&input[i + bytes_per_sample]);
    ...
    std::vector<std::uint8_t>
    convert_to_stereo(const std::vector<std::uint8_t>& input, std::uint8_t bytes_per_sample) {
      if (bytes_per_sample > 2) {
