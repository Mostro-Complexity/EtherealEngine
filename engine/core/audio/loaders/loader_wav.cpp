...
namespace audio {
  ...
  namespace detail {
    template <class T> static void endian_swap(T* objp) {
  ...
  static bool read_header(wav_header& header, const std::uint8_t* data) {
    ...
    if (offset_to_data < format_header::spec_sz) { return false; }
  ...
  bool load_wav_from_memory(
    const std::uint8_t* data, std::size_t data_size, sound_data& result, std::string& err) {
    if (!data) {
    ...
    result.info.duration = sound_info::duration_t(
      sound_info::duration_t::rep(header.data.data_bytes)
