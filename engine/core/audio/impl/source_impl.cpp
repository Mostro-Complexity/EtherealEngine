...
namespace audio {
  namespace priv {
    source_impl::source_impl() {
      if (!create()) { throw audio::exception("Cannot create source."); }
    }

    source_impl::~source_impl() { purge(); }

    bool source_impl::create() {
      if (handle_ != 0u) { return true; }
    ...
    bool source_impl::bind(sound_impl* sound) {
      ...
      if (channels > 1) { log_info("Sound is not mono. 3D Attenuation will not work."); }
    ...
    float source_impl::get_playing_duration() const {
      if (bound_sound_) { return float(bound_sound_->buf_info_.get_duration()); }
      return 0;
    }

    void source_impl::play() const { al_check(alSourcePlay(handle_)); }

    void source_impl::stop() const { al_check(alSourceStop(handle_)); }

    void source_impl::pause() const { al_check(alSourcePause(handle_)); }
    ...
    void source_impl::set_volume(float volume) { al_check(alSourcef(handle_, AL_GAIN, volume)); }
    ...
    bool source_impl::is_valid() const { return handle_ != 0; }
    ...
    void source_impl::update_stream() {
      if (bound_sound_) { bound_sound_->load_buffer(); }
    }
    ...
    source_impl::native_handle_type source_impl::native_handle() const { return handle_; }
