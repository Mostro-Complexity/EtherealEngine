  ...
  namespace priv {
    ...
    namespace openal {
      ...
      static std::string al_version() {
        ...
        if (version != nullptr) { ss << "OpenAL version: " << version; }
        return ss.str();
      }
      ...
      static std::string al_vendor() {
        ...
        if (vendor != nullptr) { ss << "OpenAL vendor: " << vendor; }
        return ss.str();
      }
      ...
      static std::string al_extensions() {
        ...
        if (extensions != nullptr) { ss << "OpenAL extensions: " << extensions; }
        return ss.str();
      }
    ...
    device_impl::device_impl(int devnum) {
      ...
      for (const auto& dev : playback_devices) { log_info("-- " + dev); }
      auto capture_devices = enumerate_capture_devices();
      log_info("Supported audio capture devices:");
      for (const auto& dev : capture_devices) { log_info("-- " + dev); }
    ...
    void device_impl::enable() { al_check(alcMakeContextCurrent(context_.get())); }

    void device_impl::disable() { al_check(alcMakeContextCurrent(nullptr)); }

    bool device_impl::is_valid() const { return (device_ != nullptr) && (context_ != nullptr); }

    const std::string& device_impl::get_device_id() const { return device_id_; }

    const std::string& device_impl::get_version() const { return version_; }

    const std::string& device_impl::get_vendor() const { return vendor_; }

    const std::string& device_impl::get_extensions() const { return extensions_; }
    ...
    std::vector<std::string> device_impl::enumerate_capture_devices() {
      ...
      if (can_enumerate) { return openal::al_get_strings(nullptr, ALC_CAPTURE_DEVICE_SPECIFIER); }
    ...
    std::vector<std::string> device_impl::enumerate_playback_devices() {
      ...
      if (can_enumerate) { return openal::al_get_strings(nullptr, ALC_ALL_DEVICES_SPECIFIER); }

      return {};
    }

    void device_impl::deleter::operator()(ALCdevice* obj) {
      if (obj != nullptr) { alcCloseDevice(obj); }
    }
