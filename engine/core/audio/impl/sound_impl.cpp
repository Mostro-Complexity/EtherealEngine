...
namespace audio {
  namespace priv {
    ...
    namespace detail {
      static ALenum
      get_format_for_channels(std::uint32_t channels, std::uint32_t bytes_per_sample) {
        ALenum format = 0;
        switch (channels) {
        case 1: {
          switch (bytes_per_sample) {
          case 1: format = AL_FORMAT_MONO8; break;
          case 2: format = AL_FORMAT_MONO16; break;
          default:
        ...
        case 2: {
          switch (bytes_per_sample) {
          case 1: format = AL_FORMAT_STEREO8; break;
        ...
        default: log_error("Unsupported channel count : " + std::to_string(channels)); break;
        }
    ...
    sound_impl::sound_impl(
      std::vector<std::uint8_t>&& buffer, const sound_info& info, bool stream /*= false*/)
      : buf_(std::move(buffer)), buf_info_(info) {
    ...
    bool sound_impl::load_buffer() { return load_buffer(CHUNK_SIZE); }

    bool sound_impl::load_buffer(const size_t chunk_size) {
      if (buf_.empty()) return false;

      ALenum format =
        detail::get_format_for_channels(buf_info_.channels, buf_info_.bytes_per_sample);
      ...
      al_check(alBufferData(
        h, format, buf_.data() + buf_ptr_, ALsizei(size), ALsizei(buf_info_.sample_rate)));
      ...
      for (auto source : bound_to_sources_) { source->enqueue_buffer(h); }
    ...
    bool sound_impl::is_valid() const { return !handles_.empty(); }
    ...
    void sound_impl::unbind_from_source(source_impl* source) {
      std::lock_guard<std::mutex> lock(mutex_);
      bound_to_sources_.erase(
        std::remove_if(
          std::begin(bound_to_sources_),
    ...
    void sound_impl::unbind_from_all_sources() {
      ...
      for (auto& source : all_sources) {
        if (source != nullptr) { source->unbind(); }
      }
    }
