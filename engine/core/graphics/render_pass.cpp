...
namespace gfx {
  ...
  void render_pass::bind(const frame_buffer* fb) const {
    ...
    if (fb != nullptr) {
      ...
      set_view_rect(
        id, std::uint16_t(0), std::uint16_t(0), std::uint16_t(width), std::uint16_t(height));
      set_view_scissor(
        id, std::uint16_t(0), std::uint16_t(0), std::uint16_t(width), std::uint16_t(height));
  ...
  void render_pass::touch() const { gfx::touch(id); }

  void render_pass::clear(
  ...
  void render_pass::set_view_proj(const float* v, const float* p) { set_view_transform(id, v, p); }
  ...
  gfx::view_id render_pass::get_pass() {
    auto counter = get_counter();
    if (counter == 0) { counter = MAX_RENDER_PASSES; }
    return counter - 1;
  }
}  // namespace gfx
