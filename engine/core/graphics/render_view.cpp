...
namespace gfx {
  ...
  std::shared_ptr<texture> render_view::get_texture(
    const std::string& id, std::uint16_t _width, std::uint16_t _height, bool _hasMips,
    std::uint16_t _numLayers, texture_format _format, std::uint64_t _flags,
  ...
  std::shared_ptr<texture> render_view::get_texture(
    const std::string& id, std::uint16_t _width, std::uint16_t _height, std::uint16_t _depth,
    bool _hasMips, texture_format _format, std::uint64_t _flags, const memory_view* _mem) {
    texture_key key;
  ...
  std::shared_ptr<texture> render_view::get_texture(
    const std::string& id, std::uint16_t _size, bool _hasMips, std::uint16_t _numLayers,
    texture_format _format, std::uint64_t _flags, const memory_view* _mem) {
    texture_key key;
  ...
  std::shared_ptr<frame_buffer> render_view::get_fbo(
    const std::string& id, const std::vector<std::shared_ptr<texture>>& bind_textures) {
    fbo_key key;
    key.id = id;
  ...
  std::shared_ptr<texture> render_view::get_depth_stencil_buffer(const usize32_t& viewport_size) {
    static auto format = get_best_format(
      BGFX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER,
      format_search_flags::requires_depth | format_search_flags::requires_stencil);
    return get_texture(
      "DEPTH_STENCIL", viewport_size.width, viewport_size.height, false, 1, format);
  }
  ...
  std::shared_ptr<frame_buffer> render_view::get_g_buffer_fbo(const usize32_t& viewport_size) {
    static auto format = get_best_format(
      BGFX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER,
      format_search_flags::four_channels | format_search_flags::requires_alpha);
    static auto normal_format = get_best_format(
      BGFX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER,
      format_search_flags::four_channels | format_search_flags::requires_alpha
        | format_search_flags::half_precision_float);
    auto depth_buffer = get_depth_buffer(viewport_size);
    auto buffer0 =
      get_texture("GBUFFER0", viewport_size.width, viewport_size.height, false, 1, format);
    auto buffer1 =
      get_texture("GBUFFER1", viewport_size.width, viewport_size.height, false, 1, normal_format);
    auto buffer2 =
      get_texture("GBUFFER2", viewport_size.width, viewport_size.height, false, 1, format);
    auto buffer3 =
      get_texture("GBUFFER3", viewport_size.width, viewport_size.height, false, 1, format);
