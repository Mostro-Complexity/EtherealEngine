...
namespace gfx {
  ...
  texture_format get_best_format(std::uint16_t type_flags, std::uint32_t search_flags) {
    ...
    if (!is_depth) {
      ...
      if (
        ((search_flags & format_search_flags::prefer_compressed) != 0u)
        ...
        if ((search_flags & format_search_flags::requires_alpha) != 0u) {
          if (is_format_supported(type_flags, texture_format::BC2)) { return texture_format::BC2; }
          if (is_format_supported(type_flags, texture_format::BC3)) { return texture_format::BC3; }

        }  // End if alpha required
        else {
          if (is_format_supported(type_flags, texture_format::BC1)) { return texture_format::BC1; }
  ...
  uint64_t get_default_rt_sampler_flags() {
    static std::uint64_t sampler_flags =
