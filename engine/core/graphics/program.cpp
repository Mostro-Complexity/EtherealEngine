...
namespace gfx {
  program::program(const std::shared_ptr<shader>& compute_shader) {
    if (compute_shader) {
      ...
      for (auto& uniform : compute_shader->uniforms) { uniforms[uniform->info.name] = uniform; }
    }
  }
  ...
  program::program(
    ...
    if (vertex_shader && fragment_shader) {
      ...
      for (auto& uniform : vertex_shader->uniforms) { uniforms[uniform->info.name] = uniform; }

      for (auto& uniform : fragment_shader->uniforms) { uniforms[uniform->info.name] = uniform; }
    }
  }
  ...
  void program::set_texture(
    ...
    gfx::set_texture(
      _stage, get_uniform(_sampler, true)->native_handle(), _texture->native_handle(), _flags);
  }

  void program::set_uniform(const std::string& _name, const void* _value, uint16_t _num) {
    auto uniform = get_uniform(_name);

    if (uniform) { gfx::set_uniform(uniform->native_handle(), _value, _num); }
  }
