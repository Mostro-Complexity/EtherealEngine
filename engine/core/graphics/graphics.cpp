...
namespace gfx {
  ...
  void log(const std::string& category, const std::string& log_msg) {
    if (s_loggers[category]) { s_loggers[category](log_msg); }
  }
  ...
  struct gfx_callback : public bgfx::CallbackI {
    ...
    void traceVargs(
      const char* /*_filePath*/, std::uint16_t /*_line*/, const char* /*_format*/,
      va_list /*_argList*/) final {}

    void profilerBegin(
      const char* /*_name*/, std::uint32_t /*_abgr*/, const char* /*_filePath*/,
    ...
    void fatal(
      const char* _filePath, uint16_t _line, bgfx::Fatal::Enum /*_code*/, const char* _str) final {
      log("error", std::string(_filePath) + "[" + std::to_string(_line) + "]" + _str);
    }

    std::uint32_t cacheReadSize(std::uint64_t /*_id*/) final { return 0; }
    ...
    void screenShot(
      const char* /*_filePath*/, std::uint32_t /*_width*/, std::uint32_t /*_height*/,
      std::uint32_t /*_pitch*/, const void* /*_data*/, std::uint32_t /*_size*/,
      bool /*_yflip*/) final {}

    void captureBegin(
      std::uint32_t /*_width*/, std::uint32_t /*_height*/, std::uint32_t /*_pitch*/,
  ...
  void set_platform_data(const platform_data& _data) { bgfx::setPlatformData(_data); }

  void shutdown() {
    if (s_initted) { bgfx::shutdown(); }
  }
  ...
  void vertex_pack(
    const float _input[4], bool _inputNormalized, attribute _attr, const vertex_layout& _decl,
    void* _data, uint32_t _index) {
    bgfx::vertexPack(_input, _inputNormalized, _attr, _decl, _data, _index);
  }

  void vertex_unpack(
    float _output[4], attribute _attr, const vertex_layout& _decl, const void* _data,
    uint32_t _index) {
    bgfx::vertexUnpack(_output, _attr, _decl, _data, _index);
  }

  void vertex_convert(
    const vertex_layout& _destDecl, void* _destData, const vertex_layout& _srcDecl,
    const void* _srcData, uint32_t _num) {
    bgfx::vertexConvert(_destDecl, _destData, _srcDecl, _srcData, _num);
  }

  uint16_t weld_vertices(
    uint16_t* _output, const vertex_layout& _decl, const void* _data, uint16_t _num,
    float _epsilon) {
    return bgfx::weldVertices(_output, _decl, _data, _num, _epsilon);
  }

  uint32_t topology_convert(
    topology_conversion _conversion, void* _dst, uint32_t _dstSize, const void* _indices,
  ...
  void topology_sort_tri_list(
    topology_sort _sort, void* _dst, uint32_t _dstSize, const float _dir[], const float _pos[],
    const void* _vertices, uint32_t _stride, const void* _indices, uint32_t _numIndices,
    bool _index32) {
    bgfx::topologySortTriList(
      _sort, _dst, _dstSize, _dir, _pos, _vertices, _stride, _indices, _numIndices, _index32);
  }

  shader_handle
  create_embedded_shader(const embedded_shader* _es, renderer_type _type, const char* _name) {
  ...
  const char* get_renderer_name(renderer_type _type) { return bgfx::getRendererName(_type); }

  void reset(uint32_t _width, uint32_t _height, uint32_t _flags) {
    bgfx::reset(_width, _height, _flags);
  }

  encoder* begin() { return bgfx::begin(); }

  void end(encoder* _encoder) { bgfx::end(_encoder); }

  uint32_t frame(bool _capture) { return bgfx::frame(_capture); }

  renderer_type get_renderer_type() { return bgfx::getRendererType(); }

  const caps* get_caps() { return bgfx::getCaps(); }

  const stats* get_stats() { return bgfx::getStats(); }

  const memory_view* alloc(uint32_t _size) { return bgfx::alloc(_size); }

  const memory_view* copy(const void* _data, uint32_t _size) { return bgfx::copy(_data, _size); }
  ...
  const memory_view*
  make_ref(const void* _data, uint32_t _size, release_fn _releaseFn, void* _userData) {
    return bgfx::makeRef(_data, _size, _releaseFn, _userData);
  }

  void set_debug(uint32_t _debug) { bgfx::setDebug(_debug); }

  void dbg_text_clear(uint8_t _attr, bool _small) { bgfx::dbgTextClear(_attr, _small); }
  ...
  void
  dbg_text_vprintf(uint16_t _x, uint16_t _y, uint8_t _attr, const char* _format, va_list _argList) {
    bgfx::dbgTextPrintfVargs(_x, _y, _attr, _format, _argList);
  }

  void dbg_text_image(
    uint16_t _x, uint16_t _y, uint16_t _width, uint16_t _height, const void* _data,
    uint16_t _pitch) {
  ...
  void destroy(index_buffer_handle _handle) { bgfx::destroy(_handle); }

  vertex_buffer_handle
  create_vertex_buffer(const memory_view* _mem, const vertex_layout& _decl, uint16_t _flags) {
    return bgfx::createVertexBuffer(_mem, _decl, _flags);
  }

  void destroy(vertex_buffer_handle _handle) { bgfx::destroy(_handle); }
  ...
  create_dynamic_index_buffer(const memory_view* _mem, uint16_t _flags) {
  ...
  void destroy(dynamic_index_buffer_handle _handle) { bgfx::destroy(_handle); }

  dynamic_vertex_buffer_handle
  create_dynamic_vertex_buffer(uint32_t _num, const vertex_layout& _decl, uint16_t _flags) {
    return bgfx::createDynamicVertexBuffer(_num, _decl, _flags);
  }

  dynamic_vertex_buffer_handle create_dynamic_vertex_buffer(
    const memory_view* _mem, const vertex_layout& _decl, uint16_t _flags) {
    return bgfx::createDynamicVertexBuffer(_mem, _decl, _flags);
  }

  void
  update(dynamic_vertex_buffer_handle _handle, uint32_t _startVertex, const memory_view* _mem) {
    bgfx::update(_handle, _startVertex, _mem);
  }

  void destroy(dynamic_vertex_buffer_handle _handle) { bgfx::destroy(_handle); }
  ...
  void alloc_transient_vertex_buffer(
    transient_vertex_buffer* _tvb, uint32_t _num, const vertex_layout& _decl) {
    bgfx::allocTransientVertexBuffer(_tvb, _num, _decl);
  }

  bool alloc_transient_buffers(
    transient_vertex_buffer* _tvb, const vertex_layout& _decl, uint32_t _numVertices,
  ...
  void destroy(indirect_buffer_handle _handle) { bgfx::destroy(_handle); }

  shader_handle create_shader(const memory_view* _mem) { return bgfx::createShader(_mem); }
  ...
  void set_name(shader_handle _handle, const char* _name) { bgfx::setName(_handle, _name); }

  void destroy(shader_handle _handle) { bgfx::destroy(_handle); }
  ...
  void destroy(program_handle _handle) { bgfx::destroy(_handle); }

  bool is_texture_valid(
    uint16_t _depth, bool _cubeMap, uint16_t _numLayers, texture_format _format, uint64_t _flags) {
    return bgfx::isTextureValid(_depth, _cubeMap, _numLayers, _format, _flags);
  }

  void calc_texture_size(
    texture_info& _info, uint16_t _width, uint16_t _height, uint16_t _depth, bool _cubeMap,
  ...
  texture_handle
  create_texture(const memory_view* _mem, uint64_t _flags, uint8_t _skip, texture_info* _info) {
    return bgfx::createTexture(_mem, _flags, _skip, _info);
  }

  texture_handle create_texture_2d(
    uint16_t _width, uint16_t _height, bool _hasMips, uint16_t _numLayers, texture_format _format,
    uint64_t _flags, const memory_view* _mem) {
    return bgfx::createTexture2D(_width, _height, _hasMips, _numLayers, _format, _flags, _mem);
  }

  texture_handle create_texture_2d(
    backbuffer_ratio _ratio, bool _hasMips, uint16_t _numLayers, texture_format _format,
    uint64_t _flags) {
    return bgfx::createTexture2D(_ratio, _hasMips, _numLayers, _format, _flags);
  }

  texture_handle create_texture_3d(
    uint16_t _width, uint16_t _height, uint16_t _depth, bool _hasMips, texture_format _format,
    uint64_t _flags, const memory_view* _mem) {
    return bgfx::createTexture3D(_width, _height, _depth, _hasMips, _format, _flags, _mem);
  }

  texture_handle create_texture_cube(
    uint16_t _size, bool _hasMips, uint16_t _numLayers, texture_format _format, uint64_t _flags,
    const memory_view* _mem) {
    return bgfx::createTextureCube(_size, _hasMips, _numLayers, _format, _flags, _mem);
  }

  void update_texture_2d(
    texture_handle _handle, uint16_t _layer, uint8_t _mip, uint16_t _x, uint16_t _y,
  ...
  void update_texture_3d(
    texture_handle _handle, uint8_t _mip, uint16_t _x, uint16_t _y, uint16_t _z, uint16_t _width,
    uint16_t _height, uint16_t _depth, const memory_view* _mem) {
    bgfx::updateTexture3D(_handle, _mip, _x, _y, _z, _width, _height, _depth, _mem);
  }

  void update_texture_cube(
    texture_handle _handle, uint16_t _layer, uint8_t _side, uint8_t _mip, uint16_t _x, uint16_t _y,
    uint16_t _width, uint16_t _height, const memory_view* _mem, uint16_t _pitch) {
  ...
  void set_name(texture_handle _handle, const char* _name) { bgfx::setName(_handle, _name); }

  void destroy(texture_handle _handle) { bgfx::destroy(_handle); }

  frame_buffer_handle create_frame_buffer(
    uint16_t _width, uint16_t _height, texture_format _format, uint64_t _textureFlags) {
    return bgfx::createFrameBuffer(_width, _height, _format, _textureFlags);
  }

  frame_buffer_handle
  create_frame_buffer(backbuffer_ratio _ratio, texture_format _format, uint64_t _textureFlags) {
    return bgfx::createFrameBuffer(_ratio, _format, _textureFlags);
  }

  frame_buffer_handle
  create_frame_buffer(uint8_t _num, const texture_handle* _handles, bool _destroyTextures) {
    return bgfx::createFrameBuffer(_num, _handles, _destroyTextures);
  }

  frame_buffer_handle
  create_frame_buffer(uint8_t _num, const attachment* _attachment, bool _destroyTextures) {
    return bgfx::createFrameBuffer(_num, _attachment, _destroyTextures);
  }

  frame_buffer_handle create_frame_buffer(
    void* _nwh, uint16_t _width, uint16_t _height, texture_format _format,
  ...
  void destroy(frame_buffer_handle _handle) { bgfx::destroy(_handle); }
  ...
  void destroy(uniform_handle _handle) { bgfx::destroy(_handle); }

  occlusion_query_handle create_occlusion_query() { return bgfx::createOcclusionQuery(); }

  occlusion_query_result get_result(occlusion_query_handle _handle, int32_t* _result) {
    return bgfx::getResult(_handle, _result);
  }

  void destroy(occlusion_query_handle _handle) { bgfx::destroy(_handle); }

  void set_palette_color(uint8_t _index, uint32_t _rgba) { bgfx::setPaletteColor(_index, _rgba); }
  ...
  void set_view_name(view_id _id, const char* _name) { bgfx::setViewName(_id, _name); }
  ...
  void
  set_view_clear(view_id _id, uint16_t _flags, uint32_t _rgba, float _depth, uint8_t _stencil) {
    bgfx::setViewClear(_id, _flags, _rgba, _depth, _stencil);
  }

  void set_view_clear(
    view_id _id, uint16_t _flags, float _depth, uint8_t _stencil, uint8_t _0, uint8_t _1,
  ...
  void set_view_mode(view_id _id, view_mode _mode) { bgfx::setViewMode(_id, _mode); }
  ...
  void reset_view(view_id _id) { bgfx::resetView(_id); }

  void set_marker(const char* _marker) { bgfx::setMarker(_marker); }

  void set_state(uint64_t _state, uint32_t _rgba) { bgfx::setState(_state, _rgba); }
  ...
  void set_scissor(uint16_t _cache) { bgfx::setScissor(_cache); }

  uint32_t set_transform(const void* _mtx, uint16_t _num) { return bgfx::setTransform(_mtx, _num); }

  uint32_t alloc_transform(transform* _transform, uint16_t _num) {
    return bgfx::allocTransform(_transform, _num);
  }

  void set_transform(uint32_t _cache, uint16_t _num) { bgfx::setTransform(_cache, _num); }
  ...
  void set_index_buffer(
    dynamic_index_buffer_handle _handle, uint32_t _firstIndex, uint32_t _numIndices) {
    bgfx::setIndexBuffer(_handle, _firstIndex, _numIndices);
  }

  void
  set_index_buffer(const transient_index_buffer* _tib, uint32_t _firstIndex, uint32_t _numIndices) {
    bgfx::setIndexBuffer(_tib, _firstIndex, _numIndices);
  }

  void set_vertex_buffer(
    uint8_t _stream, vertex_buffer_handle _handle, uint32_t _startVertex, uint32_t _numVertices) {
    bgfx::setVertexBuffer(_stream, _handle, _startVertex, _numVertices);
  }

  void set_vertex_buffer(
    uint8_t _stream, dynamic_vertex_buffer_handle _handle, uint32_t _startVertex,
    uint32_t _numVertices) {
    bgfx::setVertexBuffer(_stream, _handle, _startVertex, _numVertices);
  }

  void set_vertex_buffer(
    uint8_t _stream, const transient_vertex_buffer* _tvb, uint32_t _startVertex,
    uint32_t _numVertices) {
  ...
  void
  set_instance_data_buffer(vertex_buffer_handle _handle, uint32_t _startVertex, uint32_t _num) {
    bgfx::setInstanceDataBuffer(_handle, _startVertex, _num);
  }

  void set_instance_data_buffer(
    dynamic_vertex_buffer_handle _handle, uint32_t _startVertex, uint32_t _num) {
    bgfx::setInstanceDataBuffer(_handle, _startVertex, _num);
  }

  void
  set_texture(uint8_t _stage, uniform_handle _sampler, texture_handle _handle, uint32_t _flags) {
    bgfx::setTexture(_stage, _sampler, _handle, _flags);
  }

  void touch(view_id _id) { bgfx::touch(_id); }
  ...
  void submit(
    view_id _id, program_handle _program, occlusion_query_handle _occlusionQuery, int32_t _depth,
    bool _preserveState) {
    bgfx::submit(_id, _program, _occlusionQuery, _depth, _preserveState);
  }

  void submit(
    view_id _id, program_handle _handle, indirect_buffer_handle _indirectHandle, uint16_t _start,
  ...
  void set_image(
    uint8_t _stage, texture_handle _handle, uint8_t _mip, access _access, texture_format _format) {
  ...
  void
  dispatch(view_id _id, program_handle _handle, uint32_t _numX, uint32_t _numY, uint32_t _numZ) {
  ...
  void discard() { bgfx::discard(); }

  void blit(
    view_id _id, texture_handle _dst, uint16_t _dstX, uint16_t _dstY, texture_handle _src,
  ...
  void blit(
    view_id _id, texture_handle _dst, uint8_t _dstMip, uint16_t _dstX, uint16_t _dstY,
    uint16_t _dstZ, texture_handle _src, uint8_t _srcMip, uint16_t _srcX, uint16_t _srcY,
    uint16_t _srcZ, uint16_t _width, uint16_t _height, uint16_t _depth) {
    bgfx::blit(
      _id,
      _dst,
      _dstMip,
  ...
  const std::string& get_renderer_filename_extension() {
    ...
    if (it != types.cend()) { return it->second; }
    static std::string unknown = ".unknown";
    return unknown;
  }

  bool is_homogeneous_depth() { return bgfx::getCaps()->homogeneousDepth; }

  bool is_origin_bottom_left() { return bgfx::getCaps()->originBottomLeft; }

  std::uint32_t get_max_blend_transforms() { return 128; }
