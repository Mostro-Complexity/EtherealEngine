...
namespace gfx {
  ...
  void topology_sort_tri_list(
    topology_sort _sort, void* _dst, uint32_t _dstSize, const float _dir[3], const float _pos[3],
  ...
  /**/
  const memory_view* make_ref(
    const void* _data, uint32_t _size, release_fn _releaseFn = nullptr, void* _userData = nullptr);
  ...
  /**/
  index_buffer_handle
  create_index_buffer(const memory_view* _mem, uint16_t _flags = BGFX_BUFFER_NONE);
  ...
  vertex_buffer_handle create_vertex_buffer(
    const memory_view* _mem, const vertex_layout& _decl, uint16_t _flags = BGFX_BUFFER_NONE);
  ...
  create_dynamic_index_buffer(uint32_t _num, uint16_t _flags = BGFX_BUFFER_NONE);

  /**/
  dynamic_index_buffer_handle
  create_dynamic_index_buffer(const memory_view* _mem, uint16_t _flags = BGFX_BUFFER_NONE);
  ...
  dynamic_vertex_buffer_handle create_dynamic_vertex_buffer(
    uint32_t _num, const vertex_layout& _decl, uint16_t _flags = BGFX_BUFFER_NONE);
  ...
  /**/
  uint16_t get_shader_uniforms(
    shader_handle _handle, uniform_handle* _uniforms = nullptr, uint16_t _max = 0);
  ...
  /**/
  program_handle
  create_program(shader_handle _vsh, shader_handle _fsh, bool _destroyShaders = false);
  ...
  texture_handle create_texture(
    const memory_view* _mem, uint64_t _flags = BGFX_TEXTURE_NONE | BGFX_SAMPLER_NONE,
    uint8_t _skip = 0, texture_info* _info = nullptr);
  ...
  texture_handle create_texture_cube(
    uint16_t _size, bool _hasMips, uint16_t _numLayers, texture_format _format,
    uint64_t _flags = BGFX_TEXTURE_NONE | BGFX_SAMPLER_NONE, const memory_view* _mem = nullptr);

  /**/
  void update_texture_2d(
    texture_handle _handle, uint16_t _layer, uint8_t _mip, uint16_t _x, uint16_t _y,
    uint16_t _width, uint16_t _height, const memory_view* _mem, uint16_t _pitch = UINT16_MAX);
  ...
  frame_buffer_handle create_frame_buffer(
    uint16_t _width, uint16_t _height, texture_format _format,
    uint64_t _textureFlags = BGFX_SAMPLER_U_CLAMP | BGFX_SAMPLER_V_CLAMP);

  /**/
  frame_buffer_handle create_frame_buffer(
    backbuffer_ratio _ratio, texture_format _format,
    uint64_t _textureFlags = BGFX_SAMPLER_U_CLAMP | BGFX_SAMPLER_V_CLAMP);

  /**/
  frame_buffer_handle
  create_frame_buffer(uint8_t _num, const texture_handle* _handles, bool _destroyTextures = false);

  /**/
  frame_buffer_handle
  create_frame_buffer(uint8_t _num, const attachment* _attachment, bool _destroyTextures = false);

  /**/
  frame_buffer_handle create_frame_buffer(
    void* _nwh, uint16_t _width, uint16_t _height, texture_format _format = texture_format::Count,
  ...
  /**/
  void set_view_scissor(
    view_id _id, uint16_t _x = 0, uint16_t _y = 0, uint16_t _width = 0, uint16_t _height = 0);

  /**/
  void set_view_clear(
    view_id _id, uint16_t _flags, uint32_t _rgba = 0x000000ff, float _depth = 1.0f,
    uint8_t _stencil = 0);

  /**/
  void set_view_clear(
    view_id _id, uint16_t _flags, float _depth, uint8_t _stencil, uint8_t _0 = UINT8_MAX,
    uint8_t _1 = UINT8_MAX, uint8_t _2 = UINT8_MAX, uint8_t _3 = UINT8_MAX, uint8_t _4 = UINT8_MAX,
    uint8_t _5 = UINT8_MAX, uint8_t _6 = UINT8_MAX, uint8_t _7 = UINT8_MAX);
  ...
  /**/
  void
  set_index_buffer(dynamic_index_buffer_handle _handle, uint32_t _firstIndex, uint32_t _numIndices);
  ...
  /**/
  void set_texture(
    uint8_t _stage, uniform_handle _sampler, texture_handle _handle, uint32_t _flags = UINT32_MAX);
  ...
  /**/
  void submit(
    view_id _id, program_handle _program, occlusion_query_handle _occlusionQuery,
    int32_t _depth = 0, bool _preserveState = false);

  /**/
  void submit(
    view_id _id, program_handle _handle, indirect_buffer_handle _indirectHandle,
    uint16_t _start = 0, uint16_t _num = 1, int32_t _depth = 0, bool _preserveState = false);

  /**/
  void set_image(
    uint8_t _stage, texture_handle _handle, uint8_t _mip, access _access,
  ...
  /**/
  void dispatch(
    view_id _id, program_handle _handle, uint32_t _numX = 1, uint32_t _numY = 1,
    uint32_t _numZ = 1);
  ...
  void blit(
    ...
    uint16_t _dstZ, texture_handle _src, uint8_t _srcMip = 0, uint16_t _srcX = 0,
  ...
  std::uint64_t screen_quad(
    float dest_width, float dest_height, float depth = 0.0f, float width = 1.0f,
    float height = 1.0f);
