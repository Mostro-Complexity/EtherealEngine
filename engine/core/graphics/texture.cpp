...
namespace gfx {
  ...
  texture::texture(
    const memory_view* _mem, std::uint64_t _flags, std::uint8_t _skip /*= 0 */,
    texture_info* _info /*= nullptr*/) {
    texture_info* pInfo = _info;
    if (pInfo == nullptr) { pInfo = &info; }

    handle = create_texture(_mem, _flags, _skip, pInfo);

    if (pInfo != nullptr) { info = *pInfo; }
  ...
  texture::texture(
    std::uint16_t _width, std::uint16_t _height, bool _hasMips, std::uint16_t _numLayers,
  ...
  texture::texture(
    std::uint16_t _width, std::uint16_t _height, std::uint16_t _depth, bool _hasMips,
    texture_format _format, std::uint64_t _flags /*= BGFX_TEXTURE_NONE */,
    const memory_view* _mem /*= nullptr */) {
  ...
  texture::texture(
    backbuffer_ratio _ratio, bool _hasMips, std::uint16_t _numLayers, texture_format _format,
  ...
  usize32_t texture::get_size() const {
    if (ratio == backbuffer_ratio::Count) {
      usize32_t size = { static_cast<std::uint32_t>(info.width),
  ...
  bool texture::is_render_target() const { return 0 != (flags & BGFX_TEXTURE_RT_MASK); }
}  // namespace gfx
