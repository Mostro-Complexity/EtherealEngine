...
namespace gfx {
  ...
  frame_buffer::frame_buffer(
    std::uint16_t _width, std::uint16_t _height, texture_format _format,
  ...
  frame_buffer::frame_buffer(
    backbuffer_ratio _ratio, texture_format _format, std::uint32_t _textureFlags)
    : frame_buffer(std::vector<std::shared_ptr<texture>>{
  ...
  frame_buffer::frame_buffer(const std::vector<fbo_attachment>& textures) { populate(textures); }
  ...
  usize32_t frame_buffer::get_size() const {
    if (bbratio_ == backbuffer_ratio::Count) { return cached_size_; }  // End if Absolute
  ...
  std::size_t frame_buffer::get_attachment_count() const { return textures_.size(); }
}  // namespace gfx
