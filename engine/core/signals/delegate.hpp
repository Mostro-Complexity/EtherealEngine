...
template <class R, class... A> class delegate<R(A...)> {
  ...
  template <typename F> struct is_location_invariant : std::is_trivially_copyable<F>::type {};
  ...
  union any_data {
    ...
    template <typename F> F& access() { return *static_cast<F*>(access()); }

    template <typename F> const F& access() const { return *static_cast<const F*>(access()); }
  ...
  template <typename F> struct simple_type_wrapper {
    simple_type_wrapper(F value) : _value(value) {}
  ...
  template <typename F> class base_manager {
  protected:
    ...
    using is_function_pointer = std::integral_constant<
      bool,
  ...
  public:
    ...
    static void destroy(any_data& victim) { destroy_impl(victim, local_storage()); }

    static void init(any_data& _F, F&& f) { init_impl(_F, std::forward<F>(f), local_storage()); }

    template <typename _Signature> static bool not_empty_function(const delegate<_Signature>& f) {
      return static_cast<bool>(f);
    }

    template <typename Func> static bool not_empty_function(Func* const& fp) { return fp; }
    ...
    template <typename Func> static bool not_empty_function(const Func& /*unused*/) { return true; }
  ...
  private:
    ...
    static void
    clone_impl(any_data& /*unused*/, const any_data& /*unused*/, std::false_type /*unused*/) {
    ...
    static void
    clone_impl_local(any_data& dest, const any_data& source, std::true_type /*unused*/) {
    ...
    // that cannot fit into an any_data structure.
    static void
    clone_impl_local(any_data& dest, const any_data& source, std::false_type /*unused*/) {
    ...
    static bool
    compare_impl_function(const any_data& data1, const any_data& data2, std::true_type /*unused*/) {
    ...
    static bool compare_impl_function(
      const any_data& data1, const any_data& data2, std::false_type /*unused*/) {
      const F* ptr1 = get_pointer_typed(data1);
      const F* ptr2 = get_pointer_typed(data2);
    ...
    static bool
    compare_impl(const any_data& data1, const any_data& data2, std::true_type /*unused*/) {
      return compare_impl_pair(data1, data2);
    }

    static bool
    compare_impl(const any_data& data1, const any_data& data2, std::false_type /*unused*/) {
  ...
  struct manager {
    constexpr manager(
      get_pointer_type getter, clone_type cloner, compare_type comparer, destroy_type destructor)
      : get_pointer_type_(getter), clone_type_(cloner), compare_type_(comparer),
...
public:
  ...
  ~delegate() {
    if (manager_) { manager_->destroy_type_(functor_); }
  }
  ...
  delegate(delegate&& d) noexcept { d.swap(*this); }
  ...
  delegate(T&& f) {
    ...
    if (handler::not_empty_function(std::forward<T>(f))) {
      ...
      constexpr static const manager man{
        &handler::get_pointer, &handler::clone, &handler::compare, &handler::destroy
      };
  ...
  bool operator==(delegate const& rhs) const noexcept {
    if (manager_ && manager_->compare_type_(functor_, rhs.functor_)) return true;
  ...
  bool operator<(delegate const& rhs) const noexcept {
    ...
    return (object_ptr < rhs_object_ptr)
           || ((object_ptr == rhs_object_ptr) && (invoker_ < rhs.invoker_));
  }

  bool operator==(std::nullptr_t const) const noexcept { return !invoker_; }

  bool operator!=(std::nullptr_t const) const noexcept { return invoker_; }

  explicit operator bool() const noexcept { return !!invoker_; }
...
private:
  void* get_object_ptr() const {
    if (manager_) { return manager_->get_pointer_type_(functor_); }
    return nullptr;
  }
  ...
  template <typename T>
  static
    typename std::enable_if<!(is_member_pair<T>::value || is_const_member_pair<T>::value), R>::type
