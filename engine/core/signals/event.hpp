...
public:
  ...
  template <class C> slot_key connect(C* const object_ptr, void (C::*const method_ptr)(Args...)) {
  ...
  template <class C> void disconnect(C* const object_ptr, void (C::*const method_ptr)(Args...)) {
  ...
  void disconnect(const slot_key& key) noexcept { disconnect_by_key(key); }
  ...
  void emit(Args... args) const {
    for (auto& slot : slots_) { slot(std::forward<Args>(args)...); }
  }
  ...
  bool operator==(const event& s) const { return slots_ == s.slots_; }

  bool operator!=(const event& s) const { return !operator==(s); }

private:
  void disconnect_by_value(const slot_type& slot) {
    auto it = std::find_if(std::begin(slots_), std::end(slots_), [&slot](const slot_type& other) {
      return slot == other;
    });

    if (it != std::end(slots_)) { slots_.erase(it); }
  }

  void disconnect_by_key(const slot_key& key) noexcept { slots_.erase(key); }
