...
public:
  ...
  void register_command(
    ...
    const std::vector<std::string>& argumentNames, const std::vector<std::string>& defaultArguments,
...
private:
  struct command {
    ...
    explicit command(
      const std::string& name, const std::string& description, unsigned int numArguments,
  ...
  template <typename T> struct argumentConverter { static inline T convert(const std::string& s); };

  static inline std::function<void()> bind_callback(
    std::function<void()> callback, const std::vector<std::string>& arguments, int argumentIndex);
  template <typename T, typename... Args>
  static std::function<void()> bind_callback(
    std::function<void(T, Args...)> callback, const std::vector<std::string>& arguments,
    int argumentIndex);

  template <typename... Args> struct NameArguments {
    static inline std::string get(
      const std::vector<std::string>& argumentNames, unsigned int nextName,
...
void console::register_command(
  ...
  auto cmd = std::make_shared<command>(
    name, description, static_cast<unsigned int>(argCount), argumentNames, defaultArguments);
  ...
  cmd->get_usage = [this, commandRaw]() {
    ...
    return "Usage: " + commandRaw->name
           + NameArguments<Args...>::get(
             commandRaw->argument_names, 0, static_cast<unsigned int>(requiredArguments));
  };
...
inline std::function<void()> console::bind_callback(
  std::function<void()> callback, const std::vector<std::string>& /*unused*/, int /*unused*/) {
  return callback;
}
 ...
 */
template <typename T, typename... Args>
std::function<void()> console::bind_callback(
  std::function<void(T, Args...)> callback, const std::vector<std::string>& arguments,
  int argumentIndex) {
 ...
 */
template <> inline int console::argumentConverter<int>::convert(const std::string& s) {
  return std::stoi(s);
}
 ...
 */
template <> inline float console::argumentConverter<float>::convert(const std::string& s) {
  return std::stof(s);
}
 ...
 */
template <> struct console::NameArguments<> {
  static inline std::string get(
    const std::vector<std::string> argumentNames, unsigned int nextName,
    unsigned int requiredArguments) {
    (void) argumentNames;  // silence the unused compile warnings
    if (nextName > requiredArguments) { return "]"; }
 ...
 */
template <typename T, typename... Args> struct console::NameArguments<T, Args...> {
  static inline std::string get(
    const std::vector<std::string>& argumentNames, unsigned int nextName,
    unsigned int requiredArguments) {
    std::string nameT;
    ...
    if (nextName == requiredArguments) { nameT = "[" + nameT; }

    return " " + nameT
           + NameArguments<Args...>::get(argumentNames, nextName + 1, requiredArguments);
  }
};
