...
namespace cmd_line {
  namespace detail {
    ...
    template <typename T> inline std::uint64_t get_id() {
      static std::uint64_t id = get_internal_id();
      return id;
    }
  }  // namespace detail

  /// Class used to wrap integer types to specify desired numerical base for specific argument
  /// parsing
  template <typename T, int num_base = 0> class numeric_base {
  public:
  ...
  private:
    ...
    public:
      explicit cmd_base(
        const std::string& name, const std::string& alternative, const std::string& description,
        bool required, bool dominant, bool is_variadic)
        : name(name), command(!name.empty() ? "-" + name : ""),
          alternative(!alternative.empty() ? "--" + alternative : ""), description(description),
          required(required), handled(false), arguments({}), dominant(dominant),
    ...
    template <typename T> struct argument_count_checker { static constexpr bool variadic = false; };

    template <typename T> struct argument_count_checker<cmd_line::numeric_base<T>> {
      static constexpr bool variadic = false;
    };

    template <typename T> struct argument_count_checker<std::vector<T>> {
      static constexpr bool variadic = true;
    };

    template <typename T> class cmd_function final : public cmd_base {
    public:
      explicit cmd_function(
        const std::string& name, const std::string& alternative, const std::string& description,
        bool required, bool dominant)
        : cmd_base(
    ...
    template <typename T> class cmd_argument final : public cmd_base {
    public:
      explicit cmd_argument(
        const std::string& name, const std::string& alternative, const std::string& description,
        bool required, bool dominant)
        : cmd_base(
          name, alternative, description, required, dominant, argument_count_checker<T>::variadic) {
    ...
    static int
    parse(const std::vector<std::string>& elements, const int& /*unused*/, int numberBase = 0) {
    ...
    static long double
    parse(const std::vector<std::string>& elements, const long double& /*unused*/) {
      if (elements.size() != 1) { throw std::bad_cast(); }
    ...
    static unsigned int parse(
      const std::vector<std::string>& elements, const unsigned int& /*unused*/,
      int                             numberBase = 0) {
      if (elements.size() != 1) { throw std::bad_cast(); }
    ...
    static unsigned long parse(
      const std::vector<std::string>& elements, const unsigned long& /*unused*/,
      int                             numberBase = 0) {
      if (elements.size() != 1) { throw std::bad_cast(); }
    ...
    static std::string
    parse(const std::vector<std::string>& elements, const std::string& /*unused*/) {
      if (elements.size() != 1) { throw std::bad_cast(); }
    ...
    template <class T>
    static std::vector<T>
    parse(const std::vector<std::string>& elements, const std::vector<T>& /*unused*/) {
      const T                  defval = T();
      std::vector<T>           values{};
    ...
    template <class T> static std::string stringify(const std::vector<T>& values) {
      std::stringstream ss{};
      ss << "[ ";
  ...
  public:
    ...
    template <typename T> void set_default(bool is_required, const std::string& description = "") {
    ...
    void set_required(
      ...
      bool dominant = false) {
      auto command =
        std::make_unique<cmd_argument<T>>(name, alternative, description, true, dominant);
      commands_.emplace_back(detail::get_id<cmd_argument<T>>(), std::move(command));
    }

    template <typename T>
    void set_optional(
      const std::string& name, const std::string& alternative, T defaultValue,
      const std::string& description = "", bool dominant = false) {
      auto command =
        std::make_unique<cmd_argument<T>>(name, alternative, description, false, dominant);
    ...
    template <typename T>
    void set_callback(
      const std::string& name, const std::string& alternative,
      std::function<T(callback_args&)> callback, const std::string& description = "",
      bool dominant = false) {
      auto command =
        std::make_unique<cmd_function<T>>(name, alternative, description, false, dominant);
    ...
    template <typename T> bool try_get(const std::string& name, T& result) const {
      try {
        result = get<T>(name);
        return true;
    ...
    template <typename T> T get_if(const std::string& name, std::function<T(T)> callback) const {
  ...
  protected:
    cmd_base* find(const std::string& name) {
      for (auto& command_pair : commands_) {
        auto& command = command_pair.second;
        if (command->is(name)) { return command.get(); }
      }
    ...
    cmd_base* find_default() {
      for (auto& command_pair : commands_) {
        auto& command = command_pair.second;
        if (command->name.empty()) { return command.get(); }
      }
    ...
    std::string usage() const {
      ...
      for (const auto& command_pair : commands_) {
        ...
        if (command->required) { ss << "\t(required)"; }
