...
namespace fs {
  ...
  namespace detail {
    ...
    template <
      typename Container = std::string, typename CharT = char,
      typename Traits = std::char_traits<char>>
    auto read_stream_into_container(
      std::basic_istream<CharT, Traits>& in, typename Container::allocator_type alloc = {}) {
      static_assert(
        // Allow only strings...
        std::is_same<
          Container,
      ...
      if (std::streamsize(-1) == start_pos) { throw std::ios_base::failure{ "error" }; };
      ...
      if (!in.seekg(start_pos)) { throw std::ios_base::failure{ "error" }; };
  ...
  path resolve_protocol(const path& _path) {
    ...
    if (it == std::end(protocols)) { return path{}; };

    auto result = it->second;
    if (!relative_path.empty()) { result = result / relative_path.make_preferred(); }
    return result;
  }
  ...
  path reduce_trailing_extensions(const path& _path) {
    ...
    for (auto temp = reduced; temp.has_extension(); temp = reduced.stem()) { reduced = temp; }
