...
namespace fs {
  static path executable_path_fallback(const char* argv0) {
    if (nullptr == argv0 || 0 == argv0[0]) { return ""; }
    fs::error_code err;
...
namespace fs {
  path executable_path(const char* argv0) {
    ...
    if (ret == 0 || std::size_t(ret) == buf.size()) { return executable_path_fallback(argv0); }
...
namespace fs {
  path executable_path(const char* argv0) {
    ...
    if (0 != ret) { return executable_path_fallback(argv0); }
    fs::error_code err;
...
namespace fs {
  path executable_path(const char* argv0) {
    ...
    if (size == 0 || size == sizeof(buf)) { return executable_path_fallback(argv0); }
...
#else
namespace fs {
  path executable_path(const char* argv0) { return executable_path_fallback(argv0); }
