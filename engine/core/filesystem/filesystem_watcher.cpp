...
namespace fs {
  ...
  static std::pair<path, std::string> get_path_filter_pair(const fs::path& path) {
    ...
    fs::error_code err;
    if (filter.empty() && !fs::exists(p, err)) { log_path(path); }

    return std::make_pair(p, filter);
  }

  static std::pair<path, std::string> visit_wild_card_path(
    const fs::path& path, bool recursive, bool visit_empty,
  ...
  public:
    ...
    //-----------------------------------------------------------------------------
    impl(
      const fs::path& path, const std::string& filter, bool recursive, bool initial_list,
      clock_t::duration poll_interval, notify_callback list_callback)
      : filter_(filter), callback_(std::move(list_callback)), poll_interval_(poll_interval),
      ...
      if (initial_list) {
        ...
        if (!entries.empty() && callback_) { callback_(entries, true); }
      }
    }
    ...
    void watch() {
      ...
      if (!entries.empty() && callback_) { callback_(entries, false); }
    }

    void process_modifications(
      std::vector<filesystem_watcher::entry>& entries, const std::vector<size_t>& created,
  ...
  std::uint64_t filesystem_watcher::watch(
    const fs::path& path, bool recursive, bool initial_list, clock_t::duration poll_interval,
    notify_callback callback) {
    return watch_impl(path, recursive, initial_list, poll_interval, callback);
  }

  void filesystem_watcher::unwatch(std::uint64_t key) { unwatch_impl(key); }

  void filesystem_watcher::unwatch_all() { unwatch_all_impl(); }
  ...
  filesystem_watcher::~filesystem_watcher() { close(); }
  ...
  std::uint64_t filesystem_watcher::watch_impl(
    const fs::path& path, bool recursive, bool initial_list, clock_t::duration poll_interval,
    notify_callback& list_callback) {
    auto& wd = get_watcher();
    // and start its thread
    if (!wd.watching_) { wd.start(); }
    ...
    if (list_callback) {
      ...
      auto key = free_id++;
      {
        ...
        auto imp = std::make_shared<impl>(
          p, filter, recursive, initial_list, poll_interval, std::move(list_callback));
