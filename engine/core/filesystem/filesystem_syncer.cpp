...
namespace fs {
  ...
  syncer::~syncer() { unsync(); }

  void syncer::set_mapping(
    const std::string& ref_ext, const std::vector<std::string>& synced_ext,
    on_entry_created_t on_entry_created = nullptr, on_entry_modified_t on_entry_modified = nullptr,
    on_entry_removed_t on_entry_removed = nullptr, on_entry_renamed_t on_entry_renamed = nullptr) {
  ...
  void syncer::set_directory_mapping(
    syncer::on_entry_created_t on_entry_created, syncer::on_entry_modified_t on_entry_modified,
    syncer::on_entry_removed_t on_entry_removed, syncer::on_entry_renamed_t on_entry_renamed) {
    std::lock_guard<std::mutex> lock(mutex_);
  ...
  void syncer::unsync() { fs::watcher::unwatch(watch_id_); }
    ...
    const auto on_change = [this](const auto& entries, bool is_initial_listing) {
      for (const auto& entry : entries) {
        ...
        case fs::watcher::entry_status::created: {
          ...
          for (const auto& synced_entry : synced_entries) { ensure_directory_exists(synced_entry); }

          auto callback = this->get_on_created_callback(entry_extension);
          if (callback) { callback(entry.path, synced_entries, is_initial_listing); }
        } break;
  ...
  fs::path syncer::get_synced_directory(const fs::path& path) {
    ...
    fs::error_code err;
    if (fs::is_directory(path, err) || !path.has_extension()) { return result; }
