...
int string_utils::compare(const std::string& s1, const std::string& s2, bool ignore_case) {
  if (!ignore_case) { return s1.compare(s2); }
...
std::string string_utils::to_upper(const std::string& str) {
  std::string s(str);
  std::transform(s.begin(), s.end(), s.begin(), [](char in) { return char(toupper(in)); });
  return s;
}

std::string string_utils::to_lower(const std::string& str) {
  std::string s(str);
  std::transform(s.begin(), s.end(), s.begin(), [](char in) { return char(tolower(in)); });
  return s;
}

bool string_utils::begins_with(
  const std::string& str, const std::string& value, bool ignore_case /*= false*/) {
  // Validate requirements
  if (str.length() < value.length()) { return false; }
  if (str.empty() || value.empty()) { return false; }

  // Do the subsets match?
  return (compare(std::string(str.substr(0, value.length())), value, ignore_case) == 0);
}

bool string_utils::ends_with(
  const std::string& str, const std::string& value, bool ignore_case /*= false*/) {
  // Validate requirements
...
std::string string_utils::word_wrap(
    ...
    default:
      ...
      if (line_length == max_length) {
        ...
        if (last_space == std::string::size_type(-1)) {
          ...
          if (last_space > 0) { wrap_string += current_line.substr(0, last_space) + "\n"; }
          current_line = line_padding;
...
std::string string_utils::command_line_args(int _argc, char* _argv[]) {
  std::string cmd_line;
  for (int i = 1; i < _argc - 1; ++i) { cmd_line.append(_argv[i]).append(" "); }
  cmd_line.append(_argv[_argc - 1]);
  return cmd_line;
}

bool string_utils::parse_command_line(const std::string& cmd_line, std::vector<std::string>& args) {
  ...
  for (auto c : cmd_line) {
    if (in_quotes) {
      ...
      case '\"':
        ...
        if (!construct_argument) { args.resize(args.size() + 1); }
