...
namespace mml {
  window::window() : _impl(NULL), _size({ { 0, 0 } }), _visible(false) {}

  window::window(video_mode mode, const std::string& title, std::uint32_t style)
    : _impl(NULL), _size({ { 0, 0 } }), _visible(true) {
    create(mode, title, style);
  }

  window::window(window_handle handle) : _impl(NULL), _size({ { 0, 0 } }), _visible(true) {
    create(handle);
  }
...
// Check validity of style according to the underlying platform
  ...
  bool window::is_open() const { return _impl != NULL; }
  ...
  void window::set_position(const std::array<std::int32_t, 2>& position) {
    if (_impl) _impl->set_position(position);
  }

  std::array<std::uint32_t, 2> window::get_size() const { return _size; }

  void window::set_size(const std::array<std::uint32_t, 2>& size) {
    if (_impl) {
      if (_size[0] != size[0] || _size[1] != size[1]) { _impl->set_size(size); }
    }
  }

  void window::set_title(const std::string& title) {
    if (_impl) _impl->set_title(title);
  }

  void window::set_icon(unsigned int width, unsigned int height, const std::uint8_t* pixels) {
    if (_impl) _impl->set_icon(width, height, pixels);
  }

  void window::set_visible(bool visible) {
    if (_impl) _impl->set_visible(visible);

    _visible = visible;
  }

  bool window::is_visible() const { return _visible; }

  void window::maximize() {
    if (_impl) _impl->maximize();
  }

  void window::minimize() {
    if (_impl) _impl->minimize();
  }

  void window::restore() {
    if (_impl) _impl->restore();
  }

  void window::set_opacity(float opacity) {
    if (_impl) _impl->set_opacity(opacity);
  }

  void window::set_mouse_cursor_visible(bool visible) {
    if (_impl) _impl->set_mouse_cursor_visible(visible);
  }

  void window::set_mouse_cursor_grabbed(bool grabbed) {
    if (_impl) _impl->set_mouse_cursor_grabbed(grabbed);
  }

  void window::set_mouse_cursor(const cursor& cursor) {
    if (_impl) _impl->set_mouse_cursor(cursor.get_impl());
  }

  void window::set_key_repeat_enabled(bool enabled) {
    if (_impl) _impl->set_key_repeat_enabled(enabled);
  }

  void window::set_joystick_threshold(float threshold) {
    if (_impl) _impl->set_joystick_threshold(threshold);
  }

  void window::request_focus() {
    if (_impl) _impl->request_focus();
  }

  bool window::has_focus() const { return _impl && _impl->has_focus(); }
