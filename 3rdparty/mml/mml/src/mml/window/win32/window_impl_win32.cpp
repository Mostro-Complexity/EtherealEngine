...
namespace mml {
  namespace priv {
    ...
    window_impl_win32::window_impl_win32(window_handle handle)
      : _handle(handle), _callback(0), _cursor_visible(true),  // might need to call GetCursorInfo
        _last_cursor(LoadCursor(NULL, IDC_ARROW)), _icon(NULL), _key_repeat_enabled(true),
        _last_size({ 0, 0 }), _resizing(false), _surrogate(0), _mouse_inside(false),
        _fullscreen(false), _cursor_grabbed(false) {
      ...
      if (_handle) {
        ...
        // message is received
        if (handleCount == 0) joystick_impl::set_lazy_updates(true);

        ++handleCount;

        // We change the event procedure of the control (it is important to save the old one)
        SetWindowLongPtrW(_handle, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(this));
        _callback = SetWindowLongPtrW(
          _handle, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(&window_impl_win32::global_on_event));
      }
    }

    ////////////////////////////////////////////////////////////
    window_impl_win32::window_impl_win32(
      video_mode mode, const std::string& title, std::uint32_t style)
      : _handle(NULL), _callback(0), _cursor_visible(true),  // might need to call GetCursorInfo
        _last_cursor(LoadCursor(NULL, IDC_ARROW)), _icon(NULL), _key_repeat_enabled(true),
        _last_size({ mode.width, mode.height }), _resizing(false), _surrogate(0),
        _mouse_inside(false), _fullscreen((style & style::fullscreen) != 0),
        _cursor_grabbed(_fullscreen) {
      // Set that this process is DPI aware and can handle DPI scaling
      set_process_dpi_aware();

      // Register the window class at first call
      if (windowCount == 0) register_window_class();
      ...
      // In windowed mode, adjust width and height so that window will have the requested client
      // area
      if (!_fullscreen) {
      ...
      // handling)
      DEV_BROADCAST_HDR deviceBroadcastHeader = { sizeof(DEV_BROADCAST_HDR),
                                                  DBT_DEVTYP_DEVICEINTERFACE,
                                                  0 };
      RegisterDeviceNotification(
        _handle,
        &deviceBroadcastHeader,
      ...
      if (_fullscreen) switch_to_fullscreen(mode);
    ...
    window_impl_win32::~window_impl_win32() {
      ...
      if (_handle) {
        ...
        if (handleCount == 0) joystick_impl::set_lazy_updates(false);
      }
      ...
      if (!_callback) {
        ...
        if (windowCount == 0) UnregisterClassW(className, GetModuleHandleW(NULL));
      } else {
    ...
    window_handle window_impl_win32::native_handle() const { return _handle; }
    ...
    void window_impl_win32::set_position(const std::array<std::int32_t, 2>& position) {
      ...
      if (_cursor_grabbed) grab_cursor(true);
    }
    ...
    std::array<std::uint32_t, 2> window_impl_win32::get_size() const {
      ...
      return std::array<std::uint32_t, 2>({ static_cast<std::uint32_t>(rect.right - rect.left),
    ...
    void window_impl_win32::set_icon(
      unsigned int width, unsigned int height, const std::uint8_t* pixels) {
    ...
    void window_impl_win32::maximize() { ShowWindow(_handle, SW_MAXIMIZE); }

    void window_impl_win32::minimize() { ShowWindow(_handle, SW_MINIMIZE); }

    void window_impl_win32::restore() { ShowWindow(_handle, SW_RESTORE); }
    ...
    void window_impl_win32::set_key_repeat_enabled(bool enabled) { _key_repeat_enabled = enabled; }
    ...
    void window_impl_win32::process_event(UINT message, WPARAM wParam, LPARAM lParam) {
      ...
      case WM_SETCURSOR: {
        ...
        if (LOWORD(lParam) == HTCLIENT) SetCursor(_last_cursor);
      ...
      case WM_MOUSEMOVE: {
        ...
        if ((wParam & (MK_LBUTTON | MK_MBUTTON | MK_RBUTTON | MK_XBUTTON1 | MK_XBUTTON2)) == 0) {
          ...
          if (GetCapture() == _handle) ReleaseCapture();
    ...
    LRESULT CALLBACK
    window_impl_win32::global_on_event(HWND handle, UINT message, WPARAM wParam, LPARAM lParam) {
      ...
      window_impl_win32* window =
        handle ? reinterpret_cast<window_impl_win32*>(GetWindowLongPtr(handle, GWLP_USERDATA))
               : NULL;
      ...
      if (window) {
        ...
        if (window->_callback)
          return CallWindowProcW(
            reinterpret_cast<WNDPROC>(window->_callback), handle, message, wParam, lParam);
      }
