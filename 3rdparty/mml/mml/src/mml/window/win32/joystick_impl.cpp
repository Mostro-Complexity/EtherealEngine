...
namespace {
  ...
  std::string get_device_name(unsigned int index, JOYCAPS caps) {
    ...
    if (result != ERROR_SUCCESS) {
      ...
      if (result != ERROR_SUCCESS) {
        mml::err() << "Unable to open registry for joystick at index " << index << ": "
                   << get_error_string(result) << std::endl;
    ...
    if (result != ERROR_SUCCESS) {
      mml::err() << "Unable to query registry key for joystick at index " << index << ": "
                 << get_error_string(result) << std::endl;
      return joystickDescription;
    }
    ...
    if (result != ERROR_SUCCESS) {
      mml::err() << "Unable to open registry key for joystick at index " << index << ": "
                 << get_error_string(result) << std::endl;
      return joystickDescription;
    }
    ...
    if (result != ERROR_SUCCESS) {
      mml::err() << "Unable to query name for joystick at index " << index << ": "
                 << get_error_string(result) << std::endl;
      return joystickDescription;
    }
...
namespace mml {
  namespace priv {
    ...
    bool joystick_impl::is_connected(unsigned int index) {
      if (directInput) return is_connected_dinput(index);

      ConnectionCache&          cache = connectionCache[index];
      auto                      now = std::chrono::steady_clock::now();
      std::chrono::milliseconds delay =
        std::chrono::duration_cast<std::chrono::milliseconds>(now - cache.time);
    ...
    ////////////////////////////////////////////////////////////
    void joystick_impl::set_lazy_updates(bool status) { lazyUpdates = status; }

    ////////////////////////////////////////////////////////////
    void joystick_impl::update_connections() {
      if (directInput) return update_connections_dinput();
    ...
    bool joystick_impl::open(unsigned int index) {
      if (directInput) return open_dinput(index);
    ...
    void joystick_impl::close() {
      if (directInput) close_dinput();
    }

    ////////////////////////////////////////////////////////////
    joystick_caps joystick_impl::get_capabilities() const {
      if (directInput) return get_capabilities_dinput();

      joystick_caps caps;

      caps.button_count = m_caps.wNumButtons;
      if (caps.button_count > joystick::button_count) caps.button_count = joystick::button_count;
    ...
    ////////////////////////////////////////////////////////////
    joystick::identification joystick_impl::get_identification() const { return m_identification; }

    ////////////////////////////////////////////////////////////
    joystick_state joystick_impl::update() {
      if (directInput) return update_dinput();
    ...
    void joystick_impl::cleanup_dinput() {
      ...
      // Unload dinput8.dll
      if (dinput8dll) FreeLibrary(dinput8dll);
    }
    ...
    void joystick_impl::update_connections_dinput() {
      ...
      for (std::size_t i = 0; i < joystickList.size(); ++i) joystickList[i].plugged = false;

      // Enumerate devices
      HRESULT result = directInput->EnumDevices(
        DI8DEVCLASS_GAMECTRL,
        &joystick_impl::device_enumeration_callback,
        NULL,
        DIEDFL_ATTACHEDONLY);

      // Remove devices that were not connected during the enumeration
      for (std::vector<JoystickRecord>::iterator i = joystickList.begin();
           i != joystickList.end();) {
        if (!i->plugged)
    ...
    bool joystick_impl::open_dinput(unsigned int index) {
      ...
      for (int i = 0; i < joystick::axis_count; ++i) m_axes[i] = -1;

      for (int i = 0; i < joystick::button_count; ++i) m_buttons[i] = -1;
      ...
      for (std::vector<JoystickRecord>::iterator i = joystickList.begin(); i != joystickList.end();
        ...
        if (i->index == index) {
          ...
          result = m_device->EnumObjects(
            &joystick_impl::device_object_enumeration_callback,
            this,
          ...
          if (!m_device->GetProperty(DIPROP_PRODUCTNAME, &stringProperty.diph)) {
            ...
            if (length > 0) {
              ...
              mml::utf8::from_wide(
                stringProperty.wsz,
                stringProperty.wsz + length,
                std::back_inserter(m_identification.name));
            }
          }
    ...
    joystick_caps joystick_impl::get_capabilities_dinput() const {
      ...
      for (int i = 0; i < joystick::button_count; ++i) {
        if (m_buttons[i] != -1) ++caps.button_count;
      }

      // Check which axes have valid offsets
      for (int i = 0; i < joystick::axis_count; ++i) caps.axes[i] = (m_axes[i] != -1);
    ...
    ////////////////////////////////////////////////////////////
    BOOL CALLBACK
    joystick_impl::device_enumeration_callback(const DIDEVICEINSTANCE* deviceInstance, void*) {
    ...
    ////////////////////////////////////////////////////////////
    BOOL CALLBACK joystick_impl::device_object_enumeration_callback(
