...
namespace mml {
  namespace priv {
    ...
    private:
      ...
      ::Cursor
        _hidden_cursor;  ///< As X11 doesn't provide cursor hiding, we must create a transparent one
      ::Cursor _last_cursor;  ///< Last cursor used -- this data is not owned by the window and is
                              ///< required to be always valid
      bool _key_repeat;       ///< Is the KeyRepeat feature enabled?
      std::array<std::int32_t, 2>
        _previous_size;  ///< Previous size of the window, to find if a ConfigureNotify event is a
                         ///< resize event (could be a move event only)
