...
namespace {
  ...
  bool is_joystick(udev_device* udevDevice) {
    ...
    if (idClass) {
      ...
      if (
        std::strstr(idClass, "accelerometer") || std::strstr(idClass, "key")
  ...
  void update_plugged_list(udev_device* udevDevice = NULL) {
    ...
    for (joystick_list::iterator record = joystickList.begin(); record != joystickList.end();
         ++record)
      record->plugged = false;
  ...
  unsigned int get_usb_attribute_uint(udev_device* udevDevice, const std::string& attributeName) {
    ...
    if (attribute) value = static_cast<unsigned int>(std::strtoul(attribute, NULL, 16));
  ...
  unsigned int get_joystick_vendor_id(unsigned int index) {
    if (!udevContext) {
      mml::err() << "Failed to get vendor ID of joystick " << joystickList[index].device_node
                 << std::endl;
      return 0;
    }
  ...
  unsigned int get_joystick_product_id(unsigned int index) {
    if (!udevContext) {
      mml::err() << "Failed to get product ID of joystick " << joystickList[index].device_node
                 << std::endl;
      return 0;
    }
...
namespace mml {
  namespace priv {
    ////////////////////////////////////////////////////////////
    joystick_impl::joystick_impl() : _file(-1) { std::fill(_mapping, _mapping + ABS_MAX + 1, 0); }
    ...
    void joystick_impl::initialize() {
      ...
      if (!udevMonitor) {
        ...
        if (error < 0) {
          ...
          if (error < 0) {
            err() << "Failed to enable udev monitor, joystick connections and disconnections won't "
                     "be notified: "
                  << error << std::endl;
    ...
    bool joystick_impl::is_connected(unsigned int index) {
      ...
      if (!udevMonitor) {
        ...
        if (udevDevice) udev_device_unref(udevDevice);
      }

      if (index >= joystickList.size()) return false;
    ...
    ////////////////////////////////////////////////////////////
    joystick::identification joystick_impl::get_identification() const { return _identification; }
