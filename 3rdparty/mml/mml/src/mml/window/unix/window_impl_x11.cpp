...
namespace {
  ...
  struct KeyRepeatFinder {
    ...
    bool operator()(const XEvent& event) {
      return (
        (event.type == KeyPress) && (event.xkey.keycode == keycode)
  ...
  bool ewmhSupported() {
    ...
    if (checked) return ewmhSupported;
    ...
    if (!netSupportingWmCheck || !netSupported) return false;
    ...
    if (!utf8StringType) utf8StringType = XA_STRING;
  ...
  bool getEWMHFrameExtents(::Display* disp, ::Window win, long& xFrameExtent, long& yFrameExtent) {
    if (!ewmhSupported()) return false;

    Atom frameExtents = mml::priv::get_atom("_NET_FRAME_EXTENTS", true);

    if (frameExtents == None) return false;
    ...
    if (
      (result == Success) && (actualType == XA_CARDINAL) && (actualFormat == 32) && (numItems == 4)
      && (numBytesLeft == 0) && (data != NULL)) {
  ...
  bool isWMAbsolutePositionGood() {
    ...
    for (size_t i = 0; i < (sizeof(wmAbsPosGood) / sizeof(wmAbsPosGood[0])); i++) {
      if (wmAbsPosGood[i] == windowManagerName) return true;
    }
...
namespace mml {
  namespace priv {
    ...
    window_impl_x11::window_impl_x11(window_handle handle)
      : _window(0), _screen(0), _input_method(NULL), _input_context(NULL), _is_external(true),
        _old_video_mode(0), _old_rrc_rtc(0), _hidden_cursor(0), _last_cursor(None),
        _key_repeat(true), _previous_size{ -1, -1 }, _use_size_hints(false), _fullscreen(false),
        _cursor_grabbed(false), _window_mapped(false), _icon_pixmap(0), _icon_mask_pixmap(0),
        _last_input_time(0) {
    ...
    window_impl_x11::window_impl_x11(video_mode mode, const std::string& title, unsigned long style)
      : _window(0), _screen(0), _input_method(NULL), _input_context(NULL), _is_external(false),
        _old_video_mode(0), _old_rrc_rtc(0), _hidden_cursor(0), _last_cursor(None),
        _key_repeat(true), _previous_size{ -1, -1 }, _use_size_hints(false),
        _fullscreen((style & style::fullscreen) != 0), _cursor_grabbed(_fullscreen),
        _window_mapped(false), _icon_pixmap(0), _icon_mask_pixmap(0), _last_input_time(0) {
      // Open a connection with the X server
      _display = open_display();
      ...
      XSetWindowAttributes attributes;
      attributes.colormap =
        XCreateColormap(_display, DefaultRootWindow(_display), visual, AllocNone);
      attributes.event_mask = eventMask;
    ...
    window_impl_x11::~window_impl_x11() {
      ...
      // Destroy icon pixmap
      if (_icon_pixmap) XFreePixmap(_display, _icon_pixmap);

      // Destroy icon mask pixmap
      if (_icon_mask_pixmap) XFreePixmap(_display, _icon_mask_pixmap);

      // Destroy the cursor
      if (_hidden_cursor) XFreeCursor(_display, _hidden_cursor);

      // Destroy the input context
      if (_input_context) XDestroyIC(_input_context);
      ...
      if (_input_method) XCloseIM(_input_method);
    ...
    ////////////////////////////////////////////////////////////
    window_handle window_impl_x11::native_handle() const { return _window; }
    ...
    std::array<std::int32_t, 2> window_impl_x11::get_position() const {
      ...
      if (isWMAbsolutePositionGood())
        return std::array<std::int32_t, 2>{ { std::int32_t(xAbsRelToRoot),
                                              std::int32_t(yAbsRelToRoot) } };
      ...
      if (getEWMHFrameExtents(_display, _window, xFrameExtent, yFrameExtent)) {
        ...
        return std::array<std::int32_t, 2>{ { std::int32_t(xAbsRelToRoot - xFrameExtent),
                                              std::int32_t(yAbsRelToRoot - yFrameExtent) } };
      }
      ...
      XGetGeometry(
        _display, ancestor, &root, &xRelToRoot, &yRelToRoot, &width, &height, &borderWidth, &depth);
    ...
    std::array<std::uint32_t, 2> window_impl_x11::get_size() const {
      ...
      return std::array<std::uint32_t, 2>{ { static_cast<std::uint32_t>(attributes.width),
    ...
    void window_impl_x11::set_title(const std::string& title) {
      ...
      Xutf8SetWMProperties(
        _display, _window, title.c_str(), title.c_str(), NULL, 0, NULL, NULL, NULL);
#else
      XmbSetWMProperties(
        _display, _window, title.c_str(), title.c_str(), NULL, 0, NULL, NULL, NULL);
#endif
    ...
    ////////////////////////////////////////////////////////////
    void
    window_impl_x11::set_icon(unsigned int width, unsigned int height, const std::uint8_t* pixels) {
      ...
      XImage*      iconImage = XCreateImage(
        _display, defVisual, defDepth, ZPixmap, 0, (char*) iconPixels, width, height, 32, 0);
      if (!iconImage) {
      ...
      _icon_pixmap =
        XCreatePixmap(_display, RootWindow(_display, _screen), width, height, defDepth);
      XGCValues values;
      ...
      _icon_mask_pixmap = XCreatePixmapFromBitmapData(
        _display, _window, (char*) &maskPixels[0], width, height, 1, 0, 1);
    ...
    void window_impl_x11::set_visible(bool visible) {
      if (visible) {
        ...
        while (!_window_mapped && !_is_external) process_events();
      } else {
        XUnmapWindow(_display, _window);
    ...
    void window_impl_x11::set_mouse_cursor_grabbed(bool grabbed) {
      ...
      if (_fullscreen || (_cursor_grabbed == grabbed)) return;
      ...
      if (grabbed) {
        ...
        if (!_cursor_grabbed) err() << "Failed to grab mouse cursor" << std::endl;
      } else {
    ...
    ////////////////////////////////////////////////////////////
    void window_impl_x11::set_key_repeat_enabled(bool enabled) { _key_repeat = enabled; }
    ...
    void window_impl_x11::request_focus() {
      ...
      {
        std::lock_guard<std::mutex> lock(allWindowsMutex);
        for (std::vector<window_impl_x11*>::iterator itr = allWindows.begin();
             itr != allWindows.end();
             ++itr) {
          if ((*itr)->has_focus()) {
    ...
    void window_impl_x11::grab_focus() {
      ...
      if (ewmhSupported()) netActiveWindow = get_atom("_NET_ACTIVE_WINDOW");
    ...
    void window_impl_x11::set_video_mode(const video_mode& mode) {
      ...
      if (mode == video_mode::get_desktop_mode()) return;
      ...
      for (int i = 0; (i < res->nmode) && !modeFound; i++) {
        ...
        if (
          res->modes[i].width == static_cast<int>(mode.width)
      ...
      if (fullscreenWindow == this) {
        ...
        if (checkXRandR(xRandRMajor, xRandRMinor)) {
          ...
          if ((xRandRMajor == 1 && xRandRMinor >= 3) || xRandRMajor > 1) {
            ...
            if (output == None) output = res->outputs[0];
          } else {
    ...
    void window_impl_x11::initialize() {
      ...
      if (!_input_context)
        err() << "Failed to create input context for window -- text_entered event won't be able to "
                 "return unicode"
              << std::endl;
    ...
    void window_impl_x11::create_hidden_cursor() {
      ...
      _hidden_cursor =
        XCreatePixmapCursor(_display, cursorPixmap, cursorPixmap, &color, &color, 0, 0);
    ...
    bool window_impl_x11::process_event(XEvent& windowEvent) {
      ...
      if (windowEvent.type == KeyRelease) {
        ...
        if (iter != _events.end()) {
          ...
          if (!_key_repeat) _events.erase(iter);
      ...
      case FocusIn: {
        ...
        if (_input_context) XSetICFocus(_input_context);
      ...
      case FocusOut: {
        // Update the input context
        if (_input_context) XUnsetICFocus(_input_context);

        // Release cursor
        if (_cursor_grabbed) XUngrabPointer(_display, CurrentTime);
      ...
      case ConfigureNotify: {
        ...
        // changed
        if (
          (windowEvent.xconfigure.width != _previous_size[0])
          || (windowEvent.xconfigure.height != _previous_size[1])) {
          platform_event event;
      ...
      case ClientMessage: {
        ...
        if (windowEvent.xclient.message_type == wmProtocols) {
          ...
          if (
            ...
            netWmPing && (windowEvent.xclient.format == 32)
      ...
      case KeyPress: {
        ...
        for (int i = 0; i < 4; ++i) {
          ...
          if (key != keyboard::Unknown) break;
        }

        // Fill the event parameters
        // TODO: if modifiers are wrong, use XGetModifierMapping to retrieve the actual modifiers
        // mapping
        platform_event event;
      ...
      case ButtonPress: {
        ...
        if (
          (button == Button1) || (button == Button2) || (button == Button3) || (button == 8)
          || (button == 9)) {
          platform_event event;
      ...
      case LeaveNotify: {
        ...
        // window unmapped
      case UnmapNotify: {
        if (windowEvent.xunmap.window == _window) _window_mapped = false;
      ...
      case VisibilityNotify: {
        ...
        if (windowEvent.xvisibility.window == _window) {
          if (windowEvent.xvisibility.state != VisibilityFullyObscured) _window_mapped = true;
        }
      ...
      case PropertyNotify: {
        if (!_last_input_time) _last_input_time = windowEvent.xproperty.time;
    ...
    bool window_impl_x11::checkXRandR(int& xRandRMajor, int& xRandRMinor) {
      ...
      if (
        !XRRQueryVersion(_display, &xRandRMajor, &xRandRMinor) || xRandRMajor < 1
    ...
    ////////////////////////////////////////////////////////////
    RROutput window_impl_x11::getOutputPrimary(
      ::Window& rootWindow, XRRScreenResources* res, int xRandRMajor, int xRandRMinor) {
    ...
    std::array<std::int32_t, 2> window_impl_x11::getPrimaryMonitorPosition() {
      ...
      // Get xRandr version
      int xRandRMajor, xRandRMinor;
      if (!checkXRandR(xRandRMajor, xRandRMinor)) xRandRMajor = xRandRMinor = 0;
