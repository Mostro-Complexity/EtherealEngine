...
namespace mml {
  namespace priv {
    ...
    ////////////////////////////////////////////////////////////
    std::string clipboard_impl::get_string() { return get_instance().get_string_impl(); }
    ...
    ////////////////////////////////////////////////////////////
    void clipboard_impl::process_events() { get_instance().process_events_impl(); }

    ////////////////////////////////////////////////////////////
    clipboard_impl::clipboard_impl() : m_window(0), m_requestResponded(false) {
    ...
    std::string clipboard_impl::get_string_impl() {
      ...
      while (
        !m_requestResponded
        && (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - last) < std::chrono::milliseconds(1000)))
        process_events();

      // If no response was received within the time period, clear our clipboard contents
      if (!m_requestResponded) m_clipboardContents.clear();
    ...
    void clipboard_impl::process_event(XEvent& windowEvent) {
      ...
      case SelectionNotify: {
        ...
        if ((selectionEvent.property == None) || (selectionEvent.selection != m_clipboard)) break;
      ...
      case SelectionRequest: {
        ...
        if (selectionRequestEvent.selection == m_clipboard) {
          if (selectionRequestEvent.target == m_targets) {
            ...
            (selectionRequestEvent.target == XA_STRING)
