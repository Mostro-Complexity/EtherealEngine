      ...
      if (display) {
        ...
        if (XQueryExtension(display, "RANDR", &version, &version, &version)) {
          ...
          if (config) {
            ...
            XRRScreenSize* sizes = XRRConfigSizes(config, &nbSizes);
            if (sizes && (nbSizes > 0)) {
              desktopMode = video_mode(
                sizes[currentMode].width, sizes[currentMode].height, DefaultDepth(display, screen));
            ...
            // Failed to get the screen configuration
            err() << "Failed to retrieve the screen configuration while trying to get the desktop "
                     "video modes"
                  << std::endl;
          }
        } else {
          // XRandr extension is not supported: we cannot get the video modes
          err() << "Failed to use the XRandR extension while trying to get the desktop video modes"
                << std::endl;
        }
