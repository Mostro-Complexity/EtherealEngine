...
namespace mml {
  namespace priv {
    window_impl*
    window_impl::create(video_mode mode, const std::string& title, std::uint32_t style) {
      return new window_impl_type(mode, title, style);
    }

    window_impl* window_impl::create(window_handle handle) { return new window_impl_type(handle); }

    window_impl::window_impl() : _joystick_threshold(0.1f) {
      ...
      // Disable IME temporarrily
      assert(ImmDisableIME(-1));
    }
    ...
    void window_impl::set_joystick_threshold(float threshold) { _joystick_threshold = threshold; }
    ...
    void window_impl::push_event(const platform_event& event) { _events.push(event); }
    ...
    void window_impl::process_joystick_events() {
      ...
      for (unsigned int i = 0; i < joystick::count; ++i) {
        ...
        if (previousState.connected ^ connected) {
          platform_event event{ .type = connected ? platform_event::joystick_connected
                                                  : platform_event::joystick_disconnected,
                                .joystick_button =
                                  mml::platform_event::joystick_button_event{ .joystick_id = i } };
          push_event(event);
        }
              ...
              if (fabs(currPos - prevPos) >= _joystick_threshold) {
                platform_event event{ .type = platform_event::joystick_moved,
                                      .joystick_move = mml::platform_event::joystick_move_event{
                                        .joystick_id = i, .axis = axis, .position = currPos } };
                push_event(event);
              }
            }
          }
