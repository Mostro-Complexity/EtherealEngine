...
namespace mml {
  namespace priv {
    ...
    ////////////////////////////////////////////////////////////
    bool sensor_manager::is_available(sensor::type sensor) { return m_sensors[sensor].available; }
    ...
    void sensor_manager::set_enabled(sensor::type sensor, bool enabled) {
      if (m_sensors[sensor].available) {
        ...
        err() << "Warning: trying to enable a sensor that is not available (call "
                 "sensor::is_available to check it)"
              << std::endl;
      }
    }

    ////////////////////////////////////////////////////////////
    bool sensor_manager::is_enabled(sensor::type sensor) const { return m_sensors[sensor].enabled; }
    ...
    void sensor_manager::update() {
      for (int i = 0; i < sensor::count; ++i) {
        ...
        if (m_sensors[i].available) m_sensors[i].value = m_sensors[i].sensor.update();
      }
    }
    ...
    sensor_manager::~sensor_manager() {
      // Per sensor cleanup
      for (int i = 0; i < sensor::count; ++i) {
        if (m_sensors[i].available) m_sensors[i].sensor.close();
      }
