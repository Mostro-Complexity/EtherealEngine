...
namespace ImGui {
  ...
  bool ImageButtonWithAspectAndTextDOWN(
    ImTextureID texId, const std::string& name, const ImVec2& texture_size, const ImVec2& imageSize,
    ...
    if (textSize.x > imageSize.x) { istextBig = true; }
    ...
    if (istextBig == false) { start.x += (size.x - textSize.x) * .5f; }
    ...
    if (imgSz.x > imgSz.y) image_bb.Min.y += (max_size - imgSz.y) * 0.5f;
    if (imgSz.x < imgSz.y) image_bb.Min.x += (max_size - imgSz.x) * 0.5f;
  ...
  static void PushMultiItemsWidthsAndLabels(const char* labels[], int components, float w_full) {
    ...
    if (w_full <= 0.0f) w_full = ImGui::GetContentRegionAvail().x;

    const float w_item_one =
      ImMax(
        1.0f, (w_full - (style.ItemInnerSpacing.x * 2.0f) * (components - 1)) / (float) components)
  ...
  bool DragFloatNEx(
    const char* labels[], float* v, int components, float v_speed, float v_min, float v_max,
  ...
  bool DragUIntNEx(
    const char* labels[], unsigned int* v, int components, float v_speed, unsigned int v_min,
    ...
    for (int i = 0; i < components; i++) {
      ...
      value_changed |= DragInt(
        "", &val, v_speed, static_cast<int>(v_min), static_cast<int>(v_max), display_format);
      v[i] = val;
      SameLine(0, g.Style.ItemInnerSpacing.x);
      PopID();
      PopID();
  ...
  bool DragIntNEx(
    const char* labels[], int* v, int components, float v_speed, int v_min, int v_max,
  ...
  bool ImageButtonEx(
    ImTextureID texture, ImVec2 size, const char* tooltip, bool selected, bool enabled) {
    ...
    if (!enabled)
      ...
      if (ImageButton(texture, size, ImVec2(0, 0), ImVec2(1, 1), -1, bg_color)) { ret = true; }
    }
    if (tooltip && IsItemHovered()) { SetTooltip("%s", tooltip); }
  ...
  void ImageWithAspect(
    ImTextureID texture, ImVec2 texture_size, ImVec2 size, const ImVec2& uv0, const ImVec2& uv1,
    ...
    if (size.x > size.y) pos.y += (max_size - size.y) * 0.5f;
    if (size.x < size.y) pos.x += (max_size - size.x) * 0.5f;
  ...
  int ImageButtonWithAspectAndLabel(
    ImTextureID texture, ImVec2 texture_size, ImVec2 size, ImVec2 uv0, ImVec2 uv1, bool selected,
    bool* edit_label, const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags) {
    static bool edit = false;

    if (edit_label != nullptr) edit = *edit_label;
    ...
    BeginGroup();
    {
      ...
      if (!(selected && edit)) {
        float wrap_pos =
          pos.x + size.x
          + image_padding
              * 2.0f;  // CalcWrapWidthForPos(pos, pos.x + size.x + image_padding * 2.0f);
      ...
      if (selected && edit) {
        ...
        if (!inputActive) { SetKeyboardFocusHere(); }
        ...
        if (!inputActive && (IsMouseClicked(0) || IsMouseDragging(0))) { edit = false; }
      }
    }
    EndGroup();
    ...
    if (IsItemHovered() && !IsMouseDragging(0)) {
      if (IsMouseClicked(0)) {
        ...
        if (!inputActive) SetActiveID(id, window);
      }
      ...
      if (selected && label_clicked) { edit = selected; }

      if (IsMouseDoubleClicked(0)) { return_value = 3; }
    }
  ...
  void LabelTextExV(const char* label, const char* fmt, va_list args) {
    ...
    const ImRect total_bb(
      ...
      window->DC.CursorPos
        + ImVec2(
          w + (label_size.x > 0.0f ? style.ItemInnerSpacing.x : 0.0f), style.FramePadding.y * 2)
        + label_size);
  ...
  void PushFont(const std::string& id) { PushFont(GetFont(id)); }
}  // namespace ImGui
