...
namespace ImGui {
  ...
  // Tip: we modify tm (its fields can even be negative!) and then we call this method to retrieve a
  // valid date
  ...
  bool DateChooser(
    const char* label, tm& dateOut, const char* dateFormat, bool* pSetStartDateToDateOutThisFrame,
    const char* leftArrow, const char* rightArrow, const char* upArrowString,
    ...
    if (!BeginCombo(label, currentText, flags)) return false;
    ...
    ImGui::SameLine(
      ImGui::GetContentRegionAvail().x - yearPartWidth - ImGui::GetStyle().WindowPadding.x
      - ImGui::GetStyle().ItemSpacing.x * 4.f);
    ImGui::PushID(1235);
    if (ImGui::Button(arrowLeft)) {
      d.tm_year -= 1;
      if (d.tm_year < 0) d.tm_year = 0;
      RecalculateDateDependentFields(d);
    }
    ImGui::SameLine();
    ...
    int maxDayOfCurMonth = numDaysPerMonth[d.tm_mon];  // This could be calculated only when needed
                                                       // (but I guess it's fast in any case...)
    ...
    for (int dw = 0; dw < 7; dw++) {
      ...
      if (dw == 0 || dw == 6) {
        ...
        ImGui::PushStyleColor(
          ImGuiCol_Text, ImVec4(l * 2.f > 1 ? 1 : l * 2.f, l * .5f, l * .5f, tc.w));
      }
      ImGui::Separator();
      ImGui::AlignTextToFramePadding();
      ...
      ImGui::Spacing();
      //-----------------------------------------------------------------------
      int curDay = dw - d.tm_wday;  // tm_wday is in [0,6]. For this to work here d must point to
                                    // the first day of the month: i.e.: d.tm_mday = 1;
      ...
      //-----------------------------------------------------------------------
      ImGui::Separator();
      if (dw == 0 || dw == 6) ImGui::PopStyleColor();
      ImGui::EndGroup();
      if (dw != 6)
        ImGui::SameLine(
          ImGui::GetContentRegionAvail().x - (6 - dw) * (ImGui::GetContentRegionAvail().x / 7.f));
    }
  ...
  void TestDateChooser(
    const char* dateFormat, const char* leftArrow, const char* rightArrow,
