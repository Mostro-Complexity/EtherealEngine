...
namespace editor {
  ...
  static void unwatch(std::vector<uint64_t>& watchers) {
    for (const auto& id : watchers) { fs::watcher::unwatch(id); }
    watchers.clear();
  };

  template <typename T>
  static std::uint64_t
  watch_assets(const fs::path& dir, const std::string& wildcard, bool reload_async) {
  ...
  template <typename T>
  static void add_to_syncer(
    std::vector<uint64_t>& watchers, fs::syncer& syncer, const fs::path& dir,
    ...
    for (const auto& type : ex::get_suported_formats<T>()) {
      syncer.set_mapping(
        type + ".meta", { ".asset" }, on_modified, on_modified, on_removed, on_renamed);
  ...
  void add_to_syncer<gfx::shader>(
    ...
    auto on_modified = [&ts](
                         const auto& ref_path, const auto& synced_paths, bool is_initial_listing) {
      auto task = ts.push_on_worker_thread([ref_path,
        ...
        if (it == std::end(synced_paths)) { return; }
    ...
    for (const auto& type : ex::get_suported_formats<gfx::shader>()) {
      syncer.set_mapping(
        type + ".meta",
        { ".dx11.asset", ".dx12.asset", ".gl.asset" },
        on_modified,
        on_modified,
        on_removed,
        on_renamed);
  ...
  bool project_manager::open_project(const fs::path& project_path) {
    ...
    setup_meta_syncer(
      app_meta_syncer_, fs::resolve_protocol("app:/data"), fs::resolve_protocol("app:/meta"));
    setup_cache_syncer(
      app_watchers_,
      app_cache_syncer_,
  ...
  void project_manager::setup_meta_syncer(
    fs::syncer& syncer, const fs::path& data_dir, const fs::path& meta_dir) {
    ...
    const auto on_file_modified =
      [](const auto& /*ref_path*/, const auto& synced_paths, bool is_initial_listing) {
        for (const auto& synced_path : synced_paths) {
          fs::error_code err;
          if (is_initial_listing && fs::exists(synced_path, err)) { return; }
    ...
    for (const auto& asset_set : types) {
      for (const auto& type : asset_set) {
        syncer.set_mapping(
          type, { ".meta" }, on_file_modified, on_file_modified, on_file_removed, on_file_renamed);
      }
    }
  ...
  void project_manager::setup_cache_syncer(
    std::vector<uint64_t>& watchers, fs::syncer& syncer, const fs::path& meta_dir,
