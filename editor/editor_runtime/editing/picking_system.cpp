...
namespace editor {
  ...
  void picking_system::frame_render(delta_t dt) {
    ...
    if (input.is_mouse_button_pressed(mml::mouse::left)) {
      ...
      if (imguizmo::is_over() && es.selection_data.object) return;
      ...
      if (!current_camera.viewport_to_world(
            cursor_pos, frustum.planes[math::volume_plane::far_plane], pick_at, true))
        return;
      ...
      ecs.for_each<transform_component, model_component>([this, &pass, &pick_frustum](
        ...
        model.render(
          ...
          0,
          program_.get(),
          [&color_id](auto& p) { p.set_uniform("u_id", &color_id); });
      });
    }

    // If the user previously clicked, and we're done reading data from GPU, look at ID buffer on
    // CPU Whatever mesh has the most pixels in the ID buffer is the one the user clicked on.
    if ((reading_ == 0u) && start_readback_) {
      ...
      // Blit and read
      gfx::blit(
        pass.id, blit_tex_->native_handle(), 0, 0, surface_->get_texture()->native_handle());
        ...
        for (auto& pair : ids) {
          if (pair.second == max_amount) {
            ...
            if (ecs.valid_index(id_key)) {
              ...
              if (picked_entity) es.select(picked_entity);
            }
            break;
          }
        }
