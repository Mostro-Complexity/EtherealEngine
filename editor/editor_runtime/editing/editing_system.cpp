...
namespace editor {
  ...
  void editing_system::save_editor_camera() {
    if (camera)
      ecs::utils::save_entity_to_file(
        fs::resolve_protocol("app:/settings/editor_camera.cfg"), camera);
  }

  void editing_system::load_editor_camera() {
    runtime::entity object;
    if (!ecs::utils::try_load_entity_from_file(
          fs::resolve_protocol("app:/settings/editor_camera.cfg"), object)) {
      auto& ecs = core::get_subsystem<runtime::entity_component_system>();
      object = ecs.create();
    }
    object.set_name("EDITOR CAMERA");

    std::shared_ptr<transform_component> transform_comp;
    if (object.has_component<transform_component>()) {
      transform_comp = object.get_component<transform_component>().lock();
    } else {
      transform_comp = object.assign<transform_component>().lock();
  ...
  void editing_system::select(rttr::variant object) { selection_data.object = object; }
