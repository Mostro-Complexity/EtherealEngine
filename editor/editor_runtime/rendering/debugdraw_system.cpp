...
namespace editor {
  void debugdraw_system::frame_render(delta_t) {
      ...
      auto draw_grid = [&](
        ...
        if (should_render) {
          ...
          const auto grid_size =
            static_cast<std::uint32_t>(math::pow(size_intervals, max_iterations));
          ...
          dd.encoder.drawGrid(
            { normal.x, normal.y, normal.z }, { center.x, center.y, center.z }, sz, float(step));
          dd.encoder.pop();
        }
      };
    ...
    if (!selected || !selected.is_type<runtime::entity>()) return;
