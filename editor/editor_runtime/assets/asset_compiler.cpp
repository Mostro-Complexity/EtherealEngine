...
namespace asset_compiler {
  ...
  static bool run_compile_process(
    ...
    if (!error.isOk()) {
      ...
      if (0 != result) {
        err = std::string(error.getMessage().getPtr());
        if (sz > 0) { err += " " + std::string(buffer.data()); }
        return false;
      }
  ...
  template <> void compile<gfx::shader>(const fs::path& absolute_meta_key, const fs::path& output) {
    ...
    const std::vector<std::string> args_array = {
      "-f",           str_input,   "-o",         str_output,   "-i", str_include,
      "--varyingdef", str_varying, "--platform", str_platform, "-p", str_profile,
      "--type",       str_type,    "-O",         "3",
    };
