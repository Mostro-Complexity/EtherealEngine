...
void process_vertices(aiMesh* mesh, mesh::load_data& load_data) {
  ...
  for (size_t i = 0; i < mesh->mNumVertices; ++i, current_vertex_ptr += vertex_stride) {
    ...
    if (mesh->mVertices != nullptr && has_position) {
      ...
      if (has_position) {
        gfx::vertex_pack(
          position, false, gfx::attribute::Position, load_data.vertex_format, current_vertex_ptr);
      }
    }
    ...
    if (mesh->mBitangents != nullptr && has_bitangent) {
      ...
      float handedness = math::dot(
        math::vec3(bitangent),
        math::normalize(math::cross(math::vec3(normal), math::vec3(tangent))));
      tangent.w = handedness;

      if (has_bitangent) {
        gfx::vertex_pack(
          math::value_ptr(bitangent),
          true,
...
void process_bones(aiMesh* mesh, std::uint32_t subset_offset, mesh::load_data& load_data) {
  if (mesh->mBones != nullptr) {
    ...
    for (size_t i = 0; i < mesh->mNumBones; ++i) {
      ...
      auto it = std::find_if(
        std::begin(bone_influences), std::end(bone_influences), [&bone_name](const auto& bone) {
...
void process_animation(const aiAnimation* assimp_anim, runtime::animation& anim) {
  ...
  if (ticks_per_second < 0.001) { ticks_per_second = 25.0; }

  auto ticks = assimp_anim->mDuration;

  anim.duration = decltype(anim.duration)(ticks * ticks_per_second);

  if (assimp_anim->mNumChannels > 0) { anim.channels.resize(assimp_anim->mNumChannels); }
...
void process_animations(const aiScene* scene, std::vector<runtime::animation>& animations) {
  if (scene->mNumAnimations > 0) { animations.resize(scene->mNumAnimations); }
...
void process_imported_scene(
  const aiScene* scene, mesh::load_data& load_data, std::vector<runtime::animation>& animations) {
...
bool importer::load_mesh_data_from_file(
  const std::string& path, mesh::load_data& load_data,
