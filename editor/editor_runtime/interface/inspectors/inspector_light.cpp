...
bool inspector_light_component::inspect(
  ...
  if (light_val.type == light_type::spot) {
    ...
    if (changed) light_val.spot_data = v.get_value<light::spot>();
  } else if (light_val.type == light_type::point) {
    rttr::variant v = light_val.point_data;
    changed |= inspect_var(v);
    if (changed) light_val.point_data = v.get_value<light::point>();
  } else if (light_val.type == light_type::directional) {
    rttr::variant v = light_val.directional_data;
    changed |= inspect_var(v);
    if (changed) light_val.directional_data = v.get_value<light::directional>();
  }
...
bool inspector_reflection_probe_component::inspect(
  ...
  if (probe.type == probe_type::box) {
    ...
    changed |= inspect_var(v);
    if (changed) probe.box_data = v.get_value<reflection_probe::box>();
  } else if (probe.type == probe_type::sphere) {
    rttr::variant v = probe.sphere_data;
    changed |= inspect_var(v);
    if (changed) probe.sphere_data = v.get_value<reflection_probe::sphere>();
  }
