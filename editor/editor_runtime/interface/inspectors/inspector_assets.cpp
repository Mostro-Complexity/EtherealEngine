...
bool inspector_asset_handle_texture::inspect(
  ...
  auto draw_image = [&]() {
    ...
    if (data) {
      ...
      gui::ItemSize(bb);
      if (!gui::ItemAdd(bb, 0)) return false;
  ...
  if (selected && !selected.is_type<asset_handle<gfx::texture>>()) {
    ...
    if (inspect_var(var_str)) {
      ...
      if (item.empty()) {
        ...
        if (load_future.valid()) { data = load_future.get(); }
      }
      var = data;
      changed = true;
    }
...
bool inspector_asset_handle_material::inspect(
  rttr::variant& var, bool read_only, const meta_getter& get_metadata) {
...
bool inspector_asset_handle_mesh::inspect(
  rttr::variant& var, bool read_only, const meta_getter& get_metadata) {
...
bool inspector_asset_handle_animation::inspect(
  rttr::variant& var, bool read_only, const meta_getter& get_metadata) {
...
bool inspector_asset_handle_sound::inspect(
  rttr::variant& var, bool read_only, const meta_getter& get_metadata) {
...
bool inspector_asset_handle_prefab::inspect(
  rttr::variant& var, bool read_only, const meta_getter& get_metadata) {
