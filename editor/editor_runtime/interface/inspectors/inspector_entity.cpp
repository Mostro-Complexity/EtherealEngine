...
bool inspector_entity::inspect(
  ...
  bool changed = false;
  {
    ...
    if (changed) { data.set_name(var_name.to_string()); }
  }
  ImGui::Separator();
  ...
  for (auto& component_ptr : components) {
    ...
    gui::PopID();
    if (!opened) { component->get_entity().remove(component_ptr.lock()); }
  }
  ...
  if (gui::BeginPopup("COMPONENT_MENU")) {
    ...
    // NOTE: find all component name to display
