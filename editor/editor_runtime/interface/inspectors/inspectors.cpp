  ...
  if (!skip_custom && inspector) {
    ...
    for (auto& prop : properties) {
      ...
      if (open) {
        ...
        if (details && open) gui::TreePop();
      }

      if (prop_changed && !is_readonly) { prop.set_value(object, prop_var); }
...
bool inspect_array(rttr::variant& var, bool read_only, const inspector::meta_getter& get_metadata) {
  ...
  for (std::size_t i = 0; i < size; ++i) {
    ...
    if (changed) view.set_value(i, value);
  }
...
bool inspect_enum(rttr::variant& var, rttr::enumeration& data, bool read_only) {
  ...
  for (const auto& string : strings) cstrings.push_back(string.data());
  ...
  if (read_only) {
    ...
    if (gui::Combo(
          "", &listbox_item_current, cstrings.data(), listbox_item_size, listbox_item_size)) {
...
rttr::variant get_meta_empty(const rttr::variant& other) { return rttr::variant(); }
