...
bool inspector_int8::inspect(rttr::variant& var, bool read_only, const meta_getter& get_metadata) {
  ...
  if (read_only) {
    ...
    auto min_var = get_metadata("min");
    if (min_var) min = min_var.to_int();

    auto max_var = get_metadata("max");
    if (max_var) max = max_var.to_int();
...
bool inspector_string::inspect(
  ...
  if (read_only) {
    ...
    if (gui::InputText(
          "", input_buff.data(), input_buff.size(), ImGuiInputTextFlags_EnterReturnsTrue)) {
