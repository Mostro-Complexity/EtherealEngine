...
bool inspector_audio_source_component::inspect(
  rttr::variant& var, bool read_only, const meta_getter& get_metadata) {
  auto data = var.get_value<audio_source_component*>();

  if (data) {
    gui::PushFont("icons");
    if (gui::Button(ICON_FA_PLAY)) { data->play(); }
    gui::SameLine();
    if (gui::Button(ICON_FA_PAUSE)) { data->pause(); }
    gui::SameLine();
    if (gui::Button(ICON_FA_STOP)) { data->stop(); }
    gui::PopFont();
