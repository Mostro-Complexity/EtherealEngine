...
#include <editor_core/gui/embedded/robotomono_regular.ttf.h>
...
void render_func(ImDrawData* _draw_data) {
  ...
  for (int32_t ii = 0, num = _draw_data->CmdListsCount; ii < num; ++ii) {
    ...
    if (
      !(gfx::get_avail_transient_vertex_buffer(num_vertices, layout) == num_vertices)
    ...
    for (const ImDrawCmd *cmd = draw_list->CmdBuffer.begin(), *cmdEnd = draw_list->CmdBuffer.end();
      ...
      if (cmd->UserCallback != nullptr) {
        ...
        if (nullptr != cmd->TextureId) { tex = reinterpret_cast<gfx::texture*>(cmd->TextureId); }
...
void imgui_handle_event(const mml::platform_event& event) {
  ...
  if (event.type == mml::platform_event::key_pressed) {
    io.AddKeyEvent((ImGuiKey) event.key.code, true);  // TODO: virtual key code µ½imguiµÄÓ³Éä
...
const mml::cursor* map_cursor(ImGuiMouseCursor cursor) {
  ...
  if (cursors.find(id) == cursors.end()) {
    ...
    if (cursor->load_from_system(id)) { cursors.emplace(id, std::move(cursor)); }
  }
...
void imgui_init() {
  ...
  gui::AddFont("default", io.Fonts->AddFontDefault(&config));
  {
    auto font = io.Fonts->AddFontFromMemoryTTF(
      reinterpret_cast<void*>(std::intptr_t(s_robotomono_regular_ttf)),
      sizeof(s_robotomono_regular_ttf),
      20,
      &config);
    font->Scale = 1.05;
    gui::AddFont("standard", font);
  }
  ...
  gui::AddFont(
    ...
    io.Fonts->AddFontFromMemoryTTF(
      ...
      sizeof(fontawesome_webfont_ttf),
      20,
      &config,
      icons_ranges));
...
void gui_system::platform_events(
  const std::pair<std::uint32_t, bool>& info, const std::vector<mml::platform_event>& events) {
  ...
  for (const auto& e : events) {
    if (e.type == mml::platform_event::closed) {
      ...
      if (context != nullptr) { imgui_destroy_context(context); }
