...
namespace imguidock {
  ...
  bool dockspace::dock_with(dock* ddock, dock* dock_to, slot dock_slot, float size, bool active) {
    ...
    if (current_container->splits[0] == nullptr) {
      ...
      case imguidock::slot::left:
        ...
        break;
      case imguidock::slot::right: current_container->vertical_split = true; break;
      case imguidock::slot::top:
        current_container->splits[1] = current_container->splits[0];
        current_container->splits[0] = child_container;
        current_container->vertical_split = false;
        break;
      case imguidock::slot::bottom: current_container->vertical_split = false; break;
      case imguidock::slot::tab: current_container->vertical_split = false; break;
      case imguidock::slot::none: break;
      }
      child_container->active_dock = active                         ? ddock
                                     : child_container->active_dock ? child_container->active_dock
                                                                    : ddock;
        ...
        if (active_dock->draging) {
          if (active_dock->redock_to) {
            ...
            active_dock->redock_from->dock_with(
              active_dock, active_dock->redock_to, active_dock->redock_slot, 0, true);
            active_dock->redock_to = nullptr;
  ...
  dockspace::render_container(uint32_t& idgen, node* container, ImVec2 size, ImVec2 cursor_pos) {
    ...
    if (container->splits[0] == nullptr && container != &root) {
      ...
      ImGui::BeginChild(
        idname.c_str(), calculated_size, false, ImGuiWindowFlags_AlwaysUseWindowPadding);
      ...
      if (split0 && split1) {
        ...
        float bcontsizeX = split0->size != 0.0f   ? size.x - split0->size - splitter_button_width
                           : split1->size != 0.0f ? split1->size
                                                  : size.x / 2 - splitterButtonWidthHalf;
        float bcontsizeY = split0->size != 0.0f   ? size.y - split0->size - splitter_button_width
                           : split1->size != 0.0f ? split1->size
                                                  : size.y / 2 - splitterButtonWidthHalf;

        calculated_size0 = ImVec2(
          container->vertical_split ? acontsizeX : size.x,
          !container->vertical_split ? acontsizeY : size.y);
        calculated_size1 = ImVec2(
          container->vertical_split ? bcontsizeX : size.x,
      ...
      if (split1) {
        ...
        if (ImGui::IsItemHovered() || ImGui::IsItemActive())
          ImGui::SetMouseCursor(
            container->vertical_split ? ImGuiMouseCursor_ResizeEW : ImGuiMouseCursor_ResizeNS);
    ...
    if (current_dock_to_) {
      if (current_dock_action_ == eUndock) {
        ...
        if (current_dock_to_->on_close_func) {
          if (current_dock_to_->on_close_func()) undock(current_dock_to_);
        } else
  ...
  void dockspace::clear() {
    for (auto container : nodes) { delete container; }
    nodes.clear();
  ...
  void activete_dock_impl(node* node, const std::string& title) {
    ...
    if (node->splits[0]) activete_dock_impl(node->splits[0], title);

    if (node->splits[1]) activete_dock_impl(node->splits[1], title);
  }
  ...
  slot dockspace::render_dock_slot_preview(
    std::uint32_t id, const ImVec2& mouse_pos, const ImVec2& cPos, const ImVec2& cSize) {
    ...
    col_preview.w = 0.3f;
    auto check_slot = [convex_slot_preview, &col, &mouse_pos](
                        ImRect rect, slot slot, std::vector<ImVec2>& convex) -> bool {
      if (convex_slot_preview) {
        get_slot_convex(rect, slot, convex);
        if (!convex.empty()) {
          ImGui::GetWindowDrawList()->AddPolyline(
            convex.data(), 4, ImGui::ColorConvertFloat4ToU32(col), true, 2.0f);
        }
        ...
        ImGui::GetWindowDrawList()->AddRectFilled(
          slot_rect.Min, slot_rect.Max, ImGui::ColorConvertFloat4ToU32(col));
    ...
    if (dock_slot != slot::none) {
      ...
      ImGui::GetWindowDrawList()->AddRectFilled(
        preview_rect.Min, preview_rect.Max, ImGui::ColorConvertFloat4ToU32(col_preview), false, 0);
    }
  ...
  bool dockspace::get_min_size(node* container, ImVec2& min) {
    if (container->splits[0] == nullptr) {
      if (min.x < container->active_dock->min_size.x) min.x = container->active_dock->min_size.x;
      if (min.y < container->active_dock->min_size.y) min.y = container->active_dock->min_size.y;
      return true;
    } else {
      if (get_min_size(container->splits[0], min)) {
        if (container->splits[1]) {
          if (get_min_size(container->splits[1], min)) { return true; }
        }
      };
    }
  ...
  void dockspace::render_tab_bar(
    node* container, const ImVec2& sz, const ImVec2& cursor_pos, float tabbar_height) {
    ImGui::SetCursorPos(cursor_pos);
    ...
    for (auto dock : container->docks) {
      ...
      if (is_button_active) {
        if (
          dock->container->parent == &root && root.splits[1] == nullptr
      ...
      if ((is_hovered || is_dock_active) && dock->close_button == true) {
        if (is_dock_active) {
          ...
          ImGui::PushStyleColor(
            ImGuiCol_Button, is_button_active ? buttonColorActive : buttonColorHovered);
        }
  ...
  void dock::initialize(
    const std::string& dtitle, bool close_btn, const ImVec2& min_sz,
