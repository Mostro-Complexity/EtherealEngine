...
static void resource_bar(
  const char* _name, const char* _tooltip, uint32_t _num, uint32_t _max, float _maxWidth,
  float _height) {
  bool itemHovered = false;
  ...
  if (gui::Begin(
    ...
    if (gui::CollapsingHeader(ICON_FA_PUZZLE_PIECE "\tResources")) {
      ...
      resource_bar(
        ...
        itemHeight);
      resource_bar(
        "  P", "Programs", stats->numPrograms, caps->limits.maxPrograms, maxWidth, itemHeight);
      resource_bar(
        "  S", "Shaders", stats->numShaders, caps->limits.maxShaders, maxWidth, itemHeight);
      resource_bar(
        "  T", "Textures", stats->numTextures, caps->limits.maxTextures, maxWidth, itemHeight);
      resource_bar(
        "  U", "Uniforms", stats->numUniforms, caps->limits.maxUniforms, maxWidth, itemHeight);
      resource_bar(
        " VB",
        "Vertex buffers",
          ...
          if (trans_comp) {
            ...
            if (camera_comp->get_camera().viewport_to_world(
                  ...
                  math::plane::from_point_normal(
                    math::vec3{ 0.0f, 0.0f, 0.0f }, math::vec3{ 0.0f, 1.0f, 0.0f }),
                  projected_pos,
                  false)) {
...
void scene_dock::render(const ImVec2& area) {
  ...
  if (size.x > 0 && size.y > 0) {
    ...
    if (gui::IsItemClicked(1) || gui::IsItemClicked(2)) {
      gui::SetWindowFocus();
      if (window != nullptr) { window->set_mouse_cursor_visible(false); }
    }
    ...
    if (gui::IsMouseReleased(1) || gui::IsMouseReleased(2)) {
      if (window != nullptr) { window->set_mouse_cursor_visible(true); }
    }
...
scene_dock::scene_dock(const std::string& dtitle, bool close_button, const ImVec2& min_size) {
  ...
  initialize(
    dtitle, close_button, min_size, std::bind(&scene_dock::render, this, std::placeholders::_1));
}
