...
template <typename T>
static asset_handle<gfx::texture>
get_preview(const T& entry, const std::string& type, editor::editing_system& es) {
  if (entry) { return es.icons[type]; }
  return es.icons["loading"];
};

static asset_handle<gfx::texture> get_preview(
  const asset_handle<gfx::texture>& entry, const std::string&, editor::editing_system& es) {
  if (entry) { return entry; }
  return es.icons["loading"];
};

static asset_handle<gfx::texture>
get_preview(const fs::path&, const std::string& type, editor::editing_system& es) {
    ...
    if (fs::is_directory(absolute_path, err)) {
      ...
      const auto process_drop = [&absolute_path](const std::string& type) {
        ...
        if (payload != nullptr) {
          ...
          if (data != new_name) {
            ...
            if (!fs::exists(new_name, err)) { fs::rename(data, new_name, err); }
          }
        }
        return payload;
      };
      ...
      for (const auto& asset_set : types) {
        for (const auto& type : asset_set) {
          if (process_drop(type) != nullptr) { break; }
        }
      }
      { process_drop("folder"); }
      {
        ...
        if (payload != nullptr) {
          ...
          if (ecs.valid_index(entity_index)) {
            ...
            if (dropped_entity) {
              auto prefab_path =
                absolute_path / fs::path(dropped_entity.to_string() + ".pfb").make_preferred();
...
static bool draw_entry(
  const asset_handle<gfx::texture>& icon, bool is_loading, const std::string& name,
  ...
  ImVec2 texture_size = item_size;
  if (icon) { texture_size = { float(icon->info.width), float(icon->info.height) }; }
  ImVec2 uv0 = { 0.0f, 0.0f };
  ImVec2 uv1 = { 1.0f, 1.0f };
  ...
  if (gui::IsItemHovered()) {
    if (on_double_click) { gui::SetMouseCursor(ImGuiMouseCursor_Hand); }
    if (gui::IsMouseDoubleClicked(0)) { action = entry_action::double_clicked; }
  ...
  if (
    is_selected && gui::GetNavInputAmount(ImGuiNavInput_Input, ImGuiInputReadMode_Pressed) > 0.0f) {
  ...
  if (gui::BeginPopup("ENTRY_RENAME_MENU")) {
    is_popup_opened = true;
    if (open_rename_menu) { gui::SetKeyboardFocusHere(); }
    ...
    if (open_rename_menu) { gui::ActivateItem(gui::GetItemID()); }
    gui::PopItemWidth();
    gui::EndPopup();
  }
  if (is_selected) {
  ...
  if (is_loading) { action = entry_action::none; }
  ...
  case entry_action::renamed: {
    ...
    if (new_name != name && !new_name.empty()) {
      if (on_rename) { on_rename(new_name); }
    }
  } break;
  ...
  if (!process_drag_drop_source(icon, absolute_path)) { process_drag_drop_target(absolute_path); }
    ...
    while (clipper.Step()) {
      for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++) {
        ...
        for (size_t j = start; j < end; ++j) {
          ...
          if (j != end - 1) { ImGui::SameLine(0.0f, style.ItemSpacing.x + extra); }
        }
      }
    }
    gui::PopStyleVar();
    if (!is_popup_opened) { context_menu(); }
...
void project_dock::context_menu() {
  ...
  if (gui::BeginPopupContextWindow()) {
    ...
    if (gui::Selectable("OPEN IN ENVIRONMENT")) { fs::show_in_graphical_env(cache_.get_path()); }

    gui::Separator();

    if (gui::Selectable("IMPORT...")) { import(); }
...
void project_dock::set_cache_path(const fs::path& path) {
  if (cache_.get_path() == path) { return; }
...
project_dock::project_dock(const std::string& dtitle, bool close_button, const ImVec2& min_size) {
  initialize(
    dtitle, close_button, min_size, std::bind(&project_dock::render, this, std::placeholders::_1));
}
