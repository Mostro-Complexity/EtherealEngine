...
style_dock::style_dock(const std::string& dtitle, bool close_button, const ImVec2& min_size) {
  initialize(
    dtitle, close_button, min_size, std::bind(&style_dock::render, this, std::placeholders::_1));
...
void style_dock::render(const ImVec2& /*unused*/) {
  ...
  if (gui::ColorEdit3("MAIN", &rgb.x, ImGuiColorEditFlags_DisplayHSV)) {
  ...
  if (gui::ColorEdit3("AREA", &rgb.x, ImGuiColorEditFlags_DisplayHSV)) {
  ...
  if (gui::ColorEdit3("BACK", &rgb.x, ImGuiColorEditFlags_DisplayHSV)) {
  ...
  if (gui::ColorEdit3("TEXT", &rgb.x, ImGuiColorEditFlags_DisplayHSV)) {
  ...
  if (gui::Button("SAVE")) { style.save_style(); }
  gui::SameLine();
  if (gui::Button("RELOAD")) { style.load_style(); }
  gui::SameLine();
