  ...
  if (ent_trans_comp) {
    ...
    if (ent_model_comp) {
      ...
      if (model.is_valid()) {
        ...
        if (current_mesh) { bounds = current_mesh->get_bounds(); }
      }
    }
...
void focus_entity_on_bounds(runtime::entity entity, const math::bbox& bounds) {
  ...
  camera_comp->set_looking_at_position(cen);
...
static context_action check_context_menu(runtime::entity entity) {
  ...
  if (entity && entity != editor_camera) {
    if (gui::BeginPopupContextItem("Entity Context Menu")) {
      ...
      if (gui::MenuItem("RENAME", "F2")) { action = context_action::rename; }
      ...
      if (gui::MenuItem("DELETE", "DEL")) { entity.destroy(); }

      if (gui::MenuItem("FOCUS", "SHIFT + F")) {  // TODO: follow the view point all the time
        if (
    ...
    if (gui::BeginPopupContextWindow()) {
      ...
      if (gui::BeginMenu("3D OBJECTS")) {
        static const std::map<std::string, std::vector<std::string>> menu_objects = {
          { "BASIC",
            { "SPHERE", "CUBE", "PLANE", "CYLINDER", "CAPSULE", "CONE", "TORUS", "TEAPOT" } },
      ...
      if (payload != nullptr) {
        ...
        if (ecs.valid_index(entity_index)) {
          ...
          if (dropped_entity) {
            ...
            if (trans_comp) { trans_comp->set_parent(entity); }
          }
        }
      }
    }
...
static void check_drag(runtime::entity entity) {
  ...
  if (entity != editor_camera) {
    if (!process_drag_drop_source(entity)) { process_drag_drop_target(entity); }
  }
}
...
void hierarchy_dock::draw_entity(runtime::entity entity) {
  ...
  if (trans_comp) { no_children = trans_comp->get_children().empty(); }

  if (no_children) { flags |= ImGuiTreeNodeFlags_Leaf; }

  auto pos = gui::GetCursorScreenPos();
  gui::AlignTextToFramePadding();

  bool opened = gui::TreeNodeEx(name.c_str(), flags);
  if (!edit_label_) { check_drag(entity); }
  ...
  if (gui::IsItemHovered() && !gui::IsMouseDragging(0)) {
    if (gui::IsMouseClicked(0)) { id_ = window->GetID(trans_comp.get()); }

    if (gui::IsMouseReleased(0) && window->GetID(trans_comp.get()) == id_) {
      if (!is_selected) { edit_label_ = false; }
      es.select(entity);
    }

    // double click the item of hierarchy
  ...
  if (opened) {
    if (!no_children) {
      ...
      for (auto& child : children) {
        if (child.valid()) { draw_entity(child); }
      }
    }
