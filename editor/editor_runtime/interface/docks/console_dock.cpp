...
console_dock::console_dock(
  const std::string& dtitle, bool close_button, const ImVec2& min_size,
  const std::shared_ptr<console_log>& log)
  : console_log_(log)

{
  initialize(
    dtitle, close_button, min_size, std::bind(&console_dock::render, this, std::placeholders::_1));
}
...
void console_dock::render(const ImVec2&) {
  ...
  gui::SameLine();
  if (gui::SmallButton("CLEAR")) { console_log_->clear_log(); }
  gui::Separator();

  // Display every line as a separate entry so we can change their color or add custom widgets. If
  // you only want raw text you can use gui::TextUnformatted(log.begin(), log.end()); NB- if you
  // have thousands of entries this approach may be too inefficient. You can seek and display only
  // the lines that are visible - CalcListClipping() is a helper to compute this information. If
  // your items are of variable size you may want to implement code similar to what
  // CalcListClipping() does. Or split your data into fixed height items to allow random-seeking
  // into your list.
  gui::BeginChild(
    "ScrollingRegion",
  ...
  if (gui::BeginPopupContextWindow()) {
    if (gui::Selectable("Clear")) console_log_->clear_log();
    gui::EndPopup();
  }
  ...
  if (console_log_->has_new_entries()) gui::SetScrollHereY();
  ...
  if (gui::InputText(
    ...
    if (error_msg != "") { APPLOG_WARNING(error_msg.c_str()); }
