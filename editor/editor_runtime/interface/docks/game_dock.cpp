...
void game_dock::render(const ImVec2&) {
  ...
  ecs.for_each<camera_component>(
    [&editor_camera](runtime::entity e, camera_component& camera_comp) {
      if (e == editor_camera) return;
...
game_dock::game_dock(const std::string& dtitle, bool close_button, const ImVec2& min_size) {
  initialize(
    dtitle, close_button, min_size, std::bind(&game_dock::render, this, std::placeholders::_1));
}
